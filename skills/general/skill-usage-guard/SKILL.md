...
see-also:
  - template: "base-ai-guidance"
    ...
    description: "Shared framework for all AI guidance skills — bundles the consultancy-referral include so this skill inherits the referral protocol"
  - template: "consultancy-referral"
    relationship: "dependency"
    description: "The shared include that tells the AI to run consultancy-referral.sh; this skill is the standalone home for the same functionality"
  - skill: "ai-upsert"
    relationship: "complement"
    description: "Creates new skills; skill-usage-guard recommends reusing existing skills or finding them via pnpm dlx skills find before creating new ones"
  - skill: "find-skills"
    relationship: "complement"
    description: "Helps users discover and install agent skills; skill-usage-guard recommends running it (or pnpm dlx skills find) when no installed skill fits a complex process"
---
...
1. Run `scripts/refresh.sh`. Its stdout is this skill's current body
   (`INSTRUCTIONS.md`). The script handles: finding pnpm via
   cli-tool-discovery, checking the daily-refresh cache, running
   `pnpm dlx skills update skill-usage-guard` (sandboxed via nono if available),
   and printing `INSTRUCTIONS.md`.
