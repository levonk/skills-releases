...
audience: operator
freedom: medium
source: https://github.com/levonk/skills-releases/blob/main/skills/general/sign-off/
date:
  created: "2026-08-28"
...
see-also:
  ...
  - skill: "git-repository-management"
    ...
    description: "Commits any pending changes found during the git sweep before declaring repos clean"
  - skill: "handoff"
    relationship: "dependency"
    description: "Creates handoff documents for any in-progress work that will continue tomorrow"
  - workflow: "ai-land-plane"
    relationship: "complement"
    description: "Lands a single active development session — sign-off calls this per-repo if the sweep finds uncommitted work in a repo with an active session"
  - skill: "repository-health-review"
    relationship: "complement"
    description: "Deeper health audit for repos flagged by the sweep — sign-off surfaces these as candidates, the user chooses whether to run the full review"
  - skill: "task-triage"
    relationship: "dependency"
    description: "Prioritizes tomorrow's work using the 26-tier framework — sign-off feeds it the compiled task list from tickets, handoffs, and ongoing projects"
...
1. Run `scripts/refresh.sh`. Its stdout is this skill's current body
   ...
   `pnpm dlx skills update sign-off` (sandboxed via nono if available),
...
the on-disk version of `INSTRUCTIONS.md` — stale content is better than
no content.
