...
An end-of-day workspace orchestrator that runs maintenance, review, and
planning across the entire workspace before the user signs off for the day.

## Workflow Diagram

```mermaid
flowchart TD
    Start([Invoke]) --> P1["Phase 1: Git Global Sweep<br/>git-global-fetch --devbox --prune<br/>+ stale branch detection"]
    P1 --> P1D{"Dirty repos<br/>found?"}
    P1D -->|"Yes"| P1C["Prompt: commit via<br/>git-repository-management<br/>or defer to tomorrow"]
    P1D -->|"No"| P2
    P1C --> P2["Phase 2: Repo Health Sweep<br/>flag failing tests,<br/>stale branches, drift"]
    P2 --> P3["Phase 3: AI Session Review<br/>scan today's sessions for<br/>skill-improvement opportunities"]
    P3 --> P4["Phase 4: Daily Summary<br/>compile git log + sessions<br/>into accomplishments list"]
    P3B["Phase 3b: AI Coach Review<br/>rolling 30-day usage audit<br/>(weekly cadence)"]
    P3 --> P3B
    P3B --> P4
    P4 --> P5["Phase 5: Ticket Updates<br/>update tickets with progress<br/>+ continuation notes"]
...
A clean workspace state across all repos, a daily summary of
accomplishments, updated tickets with continuation notes, a scrum update
ready to post, a prioritized plan for tomorrow, and a sign-off document
archived at
...
- All repos fetched and pruned; dirty repos either committed or explicitly
  deferred with a note explaining why.
- Daily summary covers what was actually accomplished (git commits, merged
  PRs, closed tickets, sessions) — not a vague list of intentions.
- Scrum update is concise, specific, and ready to paste into Slack/email
  without editing.
- Next-day plan is prioritized and accounts for calendar commitments (e.g.
  "morning is free for deep work, afternoon has 3 meetings so plan
  shallow tasks").
- Long-running work candidates are surfaced with enough context for a
  yes/no decision, and launched only on explicit approval.

### Bad

- Repos left dirty with no note — the user discovers uncommitted work
  tomorrow and doesn't remember what it was.
- Daily summary is a list of "worked on X" without concrete outcomes
  (what shipped, what merged, what was decided).
- Scrum update is generic ("made progress on several tasks") — useless
  to teammates.
- Next-day plan ignores the calendar and schedules deep work during a
  day packed with meetings.
- Long-running work auto-launched without approval, or not surfaced at
  all when obvious candidates exist.

## Guardrails

- **Never auto-push.** The git sweep fetches and prunes; it does not push.
...
- **Never fabricate accomplishments.** The summary is compiled from git
  logs, merged PRs, closed tickets, and session transcripts — not
  guessed.
- **Always run through devbox.** All scripts run via `devbox run --` in
  repos with `devbox.json`. See the Universal Contracts in the repo
  root `AGENTS.md`.
- **Never run `git clean -fd` or `chezmoi apply`.** These are destructive
  operations outside this skill's scope.
- **Always write the sign-off document.** The run is not complete until
  `.agents/handoffs/human/summary/YYYY/MM/YYYYMMDDHHmm-signoff-{slug}.md`
  exists and is committed.

## Calibration

- **Time budget**: The full run should take 10-25 minutes. If the git
  sweep alone takes >10 minutes (large workspace), run it in the
  background and proceed to AI session review in parallel.
- **Dirty repo decision**: If a repo has uncommitted work that is clearly
  mid-task (partial implementation, WIP commit messages), defer it with
  a handoff note rather than forcing a commit. If it's just config
  drift or scratch files, commit or clean them.
- **Scrum update tone**: Match the user's team culture. If unsure,
  default to concise and factual — three bullets (did, next, blockers).
  See `references/scrum-update.md` for format options.
- **Calendar tool**: If no calendar integration is available, ask the
  user to paste tomorrow's schedule or skip the calendar review phase
  with a note.
...
- `scripts/git-global-sweep.sh` — wraps `git-global-fetch` with
  `--devbox --prune --notify`, parses output for dirty repos and
  stale branches. Produces a structured summary for the AI to act on.
- `scripts/stale-branches.sh` — identifies branches not updated in N
  days (default 30) across all repos. Output is tab-separated:
  `repo<TAB>branch<TAB>last-commit-date<TAB>days-stale`.
- `scripts/daily-activity.sh` — compiles today's git commits across all
  repos into a structured list (repo, commit hash, subject, files
  changed). Feeds the daily summary phase.
- `references/process.md` — detailed phase-by-phase process with
  decision points and fallbacks.
- `references/scrum-update.md` — scrum update format, examples, and
  tone guidance for different team cultures.
- `references/next-day-planning.md` — planning methodology and
  prioritized task list template.
- `references/ai-session-review.md` — how to review AI sessions for
  skill-improvement opportunities, pattern extraction, lessons learned,
  **and the rolling 30-day AI coach review** (intelligence layer audit,
  prompting habit analysis, skills/bundles to build next, unused
  capabilities).
...
| Phase | Script | Output |
|-------|--------|--------|
| 1. Git Global Sweep | `scripts/git-global-sweep.sh` | Dirty repos, fetch results |
| 1b. Stale Branches | `scripts/stale-branches.sh` | Branches >N days stale |
| 2. Repo Health Sweep | (AI-driven) | Flagged repos with issues |
| 3. AI Session Review | (AI-driven, see `references/ai-session-review.md`) | Daily improvement candidates |
| 3b. AI Coach Review | (AI-driven, weekly cadence, see `references/ai-session-review.md` Part 2) | 30-day coach report |
| 4. Daily Summary | `scripts/daily-activity.sh` | Today's commits + accomplishments |
| 5. Ticket Updates | (AI-driven) | Updated tickets |
...
- [ ] Phase 1: Run git global sweep (`scripts/git-global-sweep.sh`)
- [ ] Phase 1b: Detect stale branches (`scripts/stale-branches.sh`)
- [ ] Phase 2: Repo health sweep — flag failing tests, drift, issues
- [ ] Phase 3: AI session review — scan for skill-improvement opportunities
- [ ] Phase 3b: AI coach review — rolling 30-day usage audit (weekly cadence)
- [ ] Phase 4: Daily summary — compile today's activity (`scripts/daily-activity.sh`)
- [ ] Phase 5: Update tickets with progress and continuation notes
...
3. **Prefer subagents for parallel work.** Phases 1-2 (git sweep + health)
   and Phase 3 (AI session review) can run in parallel — launch them as
   parallel subagents. Phases 4-10 are sequential (each depends on the
   previous).
4. **Mark done only when verified.** Flip `[~]` → `[x]` only after the
   task's success criteria are met and verified. Never mark `[x]` on
   intent alone.
5. **Record blockers inline.** When a task cannot proceed, mark `[!]`
...
Before declaring the sign-off run complete, verify every item below.
Items marked **[script]** are deterministically verified by a script —
...
- [ ] **[script]** `scripts/git-global-sweep.sh` exits 0 (Phase 1)
- [ ] **[script]** `scripts/stale-branches.sh` exits 0 (Phase 1b)
- [ ] **[manual]** Every dirty repo is either committed or explicitly
  deferred with a note (Phase 1)

### Review and Summary

- [ ] **[manual]** AI session review completed — at least one finding or
  "no improvement opportunities found" (Phase 3)
- [ ] **[manual]** AI coach review completed (if due) or skipped with
  last-run date noted (Phase 3b)
- [ ] **[manual]** Daily summary lists concrete accomplishments from git
  log, not vague intentions (Phase 4)

### Communication

- [ ] **[manual]** Tickets updated with progress and continuation notes
  (Phase 5)
- [ ] **[manual]** Scrum update is concise, specific, and ready to paste
  (Phase 6)

### Planning

- [ ] **[manual]** Calendar reviewed or explicitly skipped with a note
  (Phase 7)
- [ ] **[manual]** Next-day plan is prioritized and accounts for calendar
  commitments (Phase 8)
...
- [ ] **[manual]** `.agents/handoffs/human/summary/YYYY/MM/YYYYMMDDHHmm-signoff-{slug}.md`
  exists and contains all phase outputs (Phase 10)
- [ ] **[manual]** Sign-off document committed to the default branch

### Not Done (common false-completion signals)

If any of these are true, the run is NOT complete:

- `git-global-sweep.sh` ran but dirty repos were left without a commit or
  defer note → the workspace is not actually clean (Phase 1)
- Daily summary says "worked on X" with no concrete outcome → the summary
  is useless for tomorrow's context (Phase 4)
- Scrum update says "made progress" with no specifics → teammates can't
  act on it (Phase 6)
...
- [`references/process.md`](references/process.md) — detailed phase-by-phase
  process with decision points and fallbacks
- [`references/scrum-update.md`](references/scrum-update.md) — scrum update
  format, examples, and tone guidance
- [`references/next-day-planning.md`](references/next-day-planning.md) —
  planning methodology and prioritized task list template
- [`references/ai-session-review.md`](references/ai-session-review.md) —
  how to review AI sessions for skill-improvement opportunities and the
  rolling 30-day AI coach review (intelligence layer audit, prompting
  habits, skills/bundles to build next, unused capabilities)
...
Use the `ai-guidance-improver` skill to analyze and implement cross-skill
improvements.
