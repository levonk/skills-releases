...
survive across features. Each requirement is a living spec file in
`current/` with snapshots in `history/` taken before each substantive
change.
...
```markdown
# Requirements Ledger — Current State

_Generated by index-requirements.sh. Do not edit manually._

## {proj}
### {module}
- [{slug}](current/{proj}/{module}/{proj}_{module}_{slug}.md) — {statement summary}
- [{slug}](...) — {statement summary} _(superseded)_
...
- `scripts/index-requirements.sh` — scan `current/` and regenerate
  `INDEX.md`. No args. Reads the first paragraph (Statement) of each
  requirement for the summary line.
- `scripts/validate-ledger.sh` — check consistency. Exit 1 on errors.
  Reports: orphan history, missing creation snapshot, date
  inconsistencies, stale INDEX.md.
...
- **execute-upsert**: Phase 4 reads `internal-docs/reqs/current/` before
  PRD creation to understand existing requirements in scope. Phase 8
  snapshots any requirements that changed during the feature.
...
- **handoff**: includes applicable requirements in handoff documents so
  the next session knows the constraints.
...
- `references/requirement-template.md` — the file template for a
  current requirement (EARS-constrained)
...
- `references/evolution-protocol.md` — detailed snapshot and supersession
  protocol
