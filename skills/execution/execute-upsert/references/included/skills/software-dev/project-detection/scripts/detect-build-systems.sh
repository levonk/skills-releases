...
detect_systems() {
	...
	for system in "${!BUILD_SYSTEMS[@]}"; do
		...
		if [[ "$pattern" == *"*"* ]]; then
			...
			# shellcheck disable=SC2010 # Glob pattern check; non-alphanumeric filenames not expected
			# shellcheck disable=SC2010 # Glob pattern check
	...
	# cli-tool-discovery.sh's --detect-wrapper mode, the single source of
	# truth. It walks up from cwd and checks "already inside" env vars, then
	# config file existence + wrapper functionality (hang-safe devbox probe).
	# No duplicate file checks here.
	# Hermit is NOT listed here because it doesn't use a wrapper command —
	# its tools auto-bootstrap from bin/ and are detected via BUILD_SYSTEMS
	# (bin/hermit.hcl marker) and cli-tool-discovery.sh's repo-root bin/ fallback.
	local cli_discovery="$script_dir/cli-tool-discovery.sh"
	if [[ -f "$cli_discovery" ]]; then
		local result
		result="$(bash "$cli_discovery" --detect-wrapper 2>/dev/null || true)"
		case "$result" in
		WRAPPER:\ *)
			local prefix="${result#WRAPPER: }"
			case "$prefix" in
	...
	# direnv — detected separately (not via --detect-wrapper, which excludes
	# direnv because it's eval-based, not prefix-based). Check .envrc existence.
	if command -v direnv >/dev/null 2>&1; then
		local _dir="$repo_path"
		while [[ "$_dir" != "/" ]]; do
			if [[ -f "$_dir/.envrc" ]]; then
				detected+=("direnv")
				if [[ "$verbose" == "true" ]]; then
					echo "✓ direnv (via .envrc walk)"
				fi
				break
			fi
			_dir="$(dirname "$_dir")"
