...
#   cli-tool-discovery.sh --detect-wrapper              # detect the env wrapper for cwd (no tool needed)
#
...
# Output (detect-wrapper mode, text): WRAPPER: <wrapper-cmd> | NONE
# Output (detect-wrapper mode, json): {"status":"wrapper|none","wrapper":"..."}
#
...
#
# Detect-wrapper mode (--detect-wrapper):
#   Detects the environment wrapper for the current directory WITHOUT requiring
#   a specific tool to exist inside the wrapper. This is needed because the
#   normal resolve mode checks tool existence inside devbox (step 2a), which
#   fails for probe/nonexistent tool names. Detect-wrapper mode checks:
#     1. Already inside a wrapper shell (DEVBOX_SHELL, MISE_SHELL, etc.) → NONE
#        (tools are already on PATH, no prefix needed)
#     2. devbox.json up the tree + devbox functional (hang-safe probe) → devbox run --
#     3. .mise.toml / mise.toml up the tree + mise installed → mise exec --
#     4. flox.nix up the tree + flox installed → flox activate --
#     5. shell.nix / flake.nix up the tree + nix installed → nix develop --command / nix-shell --run
#     6. None of the above → NONE
#   direnv is NOT emitted (eval-based activation, not prefix-based — see
#   resolve_tool() for the same exclusion). Callers that need direnv
#   detection (like detect-build-systems.sh) check .envrc existence separately.
#   Devbox functionality is probed with `devbox run -- true` (hang-safe via
#   devbox_run_timed). If devbox is broken, it falls through to other wrappers.
#   Used by wrapper-helpers.sh's wrapper_prefix() instead of the old
#   __wrapper_probe__ trick (which couldn't detect devbox).
#
...
# --- Parse args: runner mode vs exec mode vs resolve mode vs detect-wrapper mode ---
exec_mode=0
json_output=0
runner_mode=0
detect_wrapper_mode=0
runner_ecosystem=""
tool=""
tool_args=()
...
if [[ "${1:-}" == "--runner" ]]; then
	...
	shift
elif [[ "${1:-}" == "--detect-wrapper" ]]; then
	detect_wrapper_mode=1
	shift
	...
	tool="${1:?Usage: cli-tool-discovery.sh <tool-name> [--json] | -- <tool-name> [args...] | --runner <ecosystem> | --detect-wrapper [--json]}"
	shift
	[[ "${1:-}" == "--json" ]] && json_output=1
fi
...
run_timed() {
	...
	if command -v timeout >/dev/null 2>&1; then
		...
		local _tmp
		_tmp="$(mktemp 2>/dev/null || echo "/tmp/ctd.$$.tmp")"
		"$@" >"$_tmp" 2>&1 &
		local _pid=$!
		local _elapsed=0
...
# --- Detect the environment wrapper for cwd (detect-wrapper mode) ---
# Checks config file existence + wrapper functionality WITHOUT requiring a
# specific tool to exist inside the wrapper. This is the correct way to
# detect devbox (which needs a hang-safe `devbox run -- true` probe, not a
# tool-existence check). For mise/flox/nix, config file existence is sufficient.
# Prints "WRAPPER:<cmd>" or "NONE" to stdout.
detect_wrapper() {
	# 1. Already inside a wrapper shell — no prefix needed (tools on PATH)
	...
	# 2. devbox — check devbox.json existence + functional probe (hang-safe)
	if command -v devbox >/dev/null 2>&1; then
		local devbox_json_dir
		devbox_json_dir="$(walk_up devbox.json 2>/dev/null || true)"
		if [[ -n "$devbox_json_dir" ]]; then
			# Probe devbox functionality with `devbox run -- true` (hang-safe).
			# `true` is a shell builtin — always available inside devbox, so
			# this tests devbox itself, not a specific tool's availability.
			if (cd "$devbox_json_dir" && devbox_run_timed true >/dev/null 2>&1); then
				echo "WRAPPER:devbox run --"
				return 0
			fi
			# Devbox broken — fall through to other wrappers
		fi
	fi

	# 3. mise — check config file existence (no functionality probe needed;
	#    mise is lightweight and doesn't have the hang risk of devbox/nix)
	if command -v mise >/dev/null 2>&1; then
	...
	# 4. flox — check flox.nix existence
	...
	# 5. nix — check flake.nix / shell.nix existence
	...
	# 6. direnv — NOT emitted as a wrapper prefix.
	# direnv activation is eval-based (`eval "$(direnv export bash)"`),
	# not prefix-based. The wrapper_prefix mechanism (`$wrapper "$@"`)
	# cannot activate direnv. When inside direnv (DIRENV_DIR set), tools
	# are already on PATH and the "already inside" check above returns
	# NONE. When outside direnv, callers that need direnv detection
	# (like detect-build-systems.sh) check .envrc existence separately.
	# See cli-tool-discovery.sh resolve_tool() for the same exclusion.

	# 7. No wrapper detected
	echo "NONE"
	return 0
}
...
if [[ "$detect_wrapper_mode" -eq 1 ]]; then
	detect_wrapper_result="$(detect_wrapper)"
	if [[ "$detect_wrapper_result" == "NONE" ]]; then
		if [[ "$json_output" -eq 1 ]]; then
			printf '{"status":"none","wrapper":""}\n'
		else
			echo "NONE"
		fi
	else
		local_wrapper="${detect_wrapper_result#WRAPPER:}"
		if [[ "$json_output" -eq 1 ]]; then
			printf '{"status":"wrapper","wrapper":"%s"}\n' "$local_wrapper"
		else
			echo "WRAPPER: $local_wrapper"
		fi
	fi
	exit 0
fi
...
if [[ "$exec_mode" -eq 1 ]]; then
	...
	FALLBACK)
		...
		if [[ "$tool" == "uv" ]]; then
			pip_cmd="${value#pip:}"
			if [[ "$json_output" -eq 1 ]]; then
				# shellcheck disable=SC2016 # JSON template uses literal backticks, not expansion
