...
see-also:
  ...
  - template: gh-posting-guard
    relationship: "base-framework"
    description: "Shared guard for posting GitHub bodies via gh --body-file"
  - template: forge-template-discovery
    relationship: "base-framework"
    description: "Host-aware issue/PR template directory search order"
dependencies:
  - type: system
    name: gh
  ...
  - type: system
    name: curl
    reason: "Required for fetching contribution standards from the GitHub API"
  - type: system
    name: jq
    url: https://stedolan.github.io/jq/
    reason: "Required for parsing GitHub API JSON responses"
triggers:
  - user
---
...
2. **If no issue exists** — create one inline using the issue creation
   procedure (inlined below). This skill is self-contained: the issue
   procedure is inlined at build time, so you do NOT need the `github-issue`
   skill installed separately to create the orientation issue.
3. **Exception — trivial typo/CI-fix PRs**: If the project does not require
   issues for trivial fixes (check CONTRIBUTING.md), the user may override
...
This procedure is shared between the `github-issue` skill (standalone issue
filing) and the `github-pr` skill (creates an orientation issue before opening
...
1. **Check contribution eligibility**: Before investing time, verify the
   ...
   - Required labels, templates, or formatting

   If the project's `config.yml` has `blank_issues_enabled: false`, do NOT
...
Issue and PR templates live in different directories depending on the forge
(GitHub, GitLab, Forgejo, Gitea, Bitbucket). Before drafting an issue or PR
body, discover the project's own templates and contribution standards so the
submission matches the project's conventions.
...
For local repositories with no `origin` remote, use `local` as the forge —
no templates to discover, fall back to the skill's own templates.

#### Issue template search directories (priority order)

Each forge checks directories in a specific order. The first directory that
contains template files wins.

**GitHub** (checks `.github/` only, plus root and `docs/`):
1. `.github/ISSUE_TEMPLATE/` (directory — multiple `.md`/`.yaml`/`.yml` files)
2. `ISSUE_TEMPLATE/` (root-level directory)
...
**Forgejo** (checks `.forgejo/` → `.gitea/` → `.github/` → `docs/`, each with
`ISSUE_TEMPLATE/` or `issue_template/` subdir):
...
**Gitea** (same as Forgejo minus `.forgejo/`):
...
**GitLab** (checks `.gitlab/` only):
1. `.gitlab/ISSUE_TEMPLATE/`
2. `.gitlab/issue_template/`

**Bitbucket**: No native issue template convention. Skip template discovery;
use the skill's own templates.

#### PR template search directories
...
**Forgejo / Gitea** (single file per forge dir, NOT in `.gitlab/`):
...
**GitLab** (called "merge request templates"):
1. `.gitlab/merge_request_templates/` (directory — multiple `.md` files)

**Bitbucket**: No native PR template convention. Skip.

#### Contribution guidelines search

Beyond templates, check for contribution guidelines and AI agent rules:
...
#### Template config (issue template chooser)

GitHub and Forgejo support a `config.yml` that controls the issue template
chooser (blank issues, contact links, etc.):

- `.github/ISSUE_TEMPLATE/config.yml`
- `.forgejo/ISSUE_TEMPLATE/config.yml`
- `.gitea/ISSUE_TEMPLATE/config.yml`

If `blank_issues_enabled: false` is set, the project does not accept free-form
issues — you MUST use one of the configured templates or contact links. Do not
file a blank issue in this case; present the constraint to the user.

#### YAML issue templates

GitHub and Forgejo support YAML-form issue templates (`.yaml`/`.yml`) with
structured fields (`name`, `description`, `labels`, `body` with input types
like `markdown`, `textarea`, `input`, `dropdown`, `checkboxes`). When a YAML
template is found, parse its `body` fields and fill them in the issue body
rather than using free-form markdown. The `labels` field pre-labels the issue
...
3. **Search for existing issues**: Run
   ...
   Search both open and closed issues. If duplicates are found:
   - Open issue, no PR → comment on it offering to implement; link your work
...
4. **Draft the issue body**: Using the project's own issue template (if found
   in Step 2) and the user's content, draft the complete issue body. If the
   project uses YAML-form issue templates, parse the `body` fields and fill
   them in. If no template exists, use a clear structure:
   ...
   Substitute all placeholders (`$UPSTREAM_OWNER`, `$UPSTREAM_REPO`, etc.) by
   **text replacement** — never shell expansion.

5. **Human review gate**: Present the complete, final issue content (title +
   body, with all placeholders substituted) to the user for review. Do NOT
   auto-post. Wait for explicit approval. The user should review exactly what
   will be posted, not a template with unsubstituted placeholders.
...
6. **Post the issue**: After user approval, write the body to a file and post
   with `--body-file`:
   ```bash
   gh issue create --repo "$UPSTREAM_OWNER/$UPSTREAM_REPO" \
     --title "..." --body-file /tmp/issue-body.md
   ```
   Record the issue number for use in PR bodies (`Resolves #N`).

7. **Validate the posted body**: Run
   `scripts/validate-pr-issue.sh <owner>/<repo> issue <number>`.
   If it exits non-zero, the body is corrupted (literal `\n`, stripped
   backticks, or unsubstituted placeholders). Re-fetch the content, fix the
...
2. **Set up branch**: Sync from upstream (fetch + rebase) to start from a
   ...
   (e.g. `feat-chat-panel-api`). Verify git author is configured with a
   public identity (not private info — use GitHub noreply email).

3. **Validate existing tests**: Run the project's test suite to establish a
   baseline. Document any pre-existing failures — do NOT fix unrelated bugs
   in this PR (minimal scope).

### Phase 3: Work, Commit, Style

4. **Apply the code change**: Make the change the user requested. Keep the
   scope minimal — one completely working change per PR.

5. **Sync before commit**: Run `git fetch upstream && git rebase upstream/main`
   to catch any upstream movement during the work phase. Resolve conflicts
   if any.

6. **Commit the feature**: Squash iterative commits into a single clean
   commit. Use the PR title as the commit message. This commit contains
   ONLY the feature change — no format or lint fixes.

7. **Format and lint**: Run the project's own formatter and linter on the
   files you created/modified. If format/lint produced changes, commit them
   as a separate "style" commit on top of the feature commit. This makes
   them reviewable independently.

8. **Scan for identity leaks**: Run `scripts/scan-artifacts.sh` on the files
   you created. Fix HARD leaks (resolved home paths, usernames, machine
   names). Re-run until clean.
...
9. **CLA gate (pre-push)**: Before pushing, check whether the
   ...
   1. **Determine the target org**: Extract the org login from the
      ...
      (`.owner.login`). For subsidiaries (e.g. VirusTotal → Google),
      use the parent org as the CLA ledger key.

   2. **Consult the CLA ledger**: Run
      ```bash
      SKILL_CONFIG_OWNER=levonk SKILL_CONFIG_REPO=skills-releases \
      SKILL_CONFIG_PATH=software-dev/github-pr \
      scripts/skill-config.sh get "cla.$ORG.signed_at" --default ""
      ```
      Also check `cla.$ORG.expires_at` to determine whether the entry
      is still fresh.

   3. **If the ledger has a fresh entry** (`signed_at` present and
      `expires_at` > today): print a one-line info note
      (`CLA on file for $ORG (signed $signed_at)`) and proceed to step
      10. Do NOT re-warn — the user has already signed.

   4. **If the ledger entry is missing or expired**: surface the CLA
      terms and sign link to the user. Present:
      - The CLA type (Google CLA, Apache CLA, cla-assistant, etc.)
      - The sign link (from the standards file or the CLA bot's
        typical URL pattern)
      - A one-line summary of what the CLA grants the project owner

      Require explicit acknowledgment from the user before proceeding.
      The user may say:
      - **"I'll sign it"** — record a provisional entry
        (`source = "user-acknowledged"`, 365-day TTL) and proceed.
        The post-open CLA-watch (step 16) will confirm or invalidate.
      - **"I already signed it"** — record a confirmed entry
        (`source = "user-acknowledged"`, 365-day TTL) and proceed.
        The post-open CLA-watch will confirm.
      - **"Stop"** — abort the PR. Do not push.

   5. **Record the ledger entry**:
      ```bash
      SKILL_CONFIG_OWNER=levonk SKILL_CONFIG_REPO=skills-releases \
      SKILL_CONFIG_PATH=software-dev/github-pr \
      scripts/skill-config.sh set --layer user "cla.$ORG.signed_at" "$(date +%Y-%m-%d)"

      # Compute expires_at = signed_at + 365 days
      EXPIRES=$(date -d "$(date +%Y-%m-%d) +365 days" +%Y-%m-%d 2>/dev/null || \
        date -v+365d +%Y-%m-%d 2>/dev/null || echo "")
      [ -n "$EXPIRES" ] && \
      scripts/skill-config.sh set --layer user "cla.$ORG.expires_at" "$EXPIRES"

      scripts/skill-config.sh set --layer user "cla.$ORG.source" "user-acknowledged"
      ```

   If no CLA is required (no signals in the standards file), skip this
   step entirely and proceed to step 10.

10. **Validate PR cleanliness**: Verify no merge commits, no unrelated
...
15. **Validate the posted body**: Run
    ...
    If it exits non-zero, the body is corrupted. Re-fetch the content, fix
...
16. **CLA watch (post-open)**: If the pre-push CLA gate (step 9) detected
    ...
    1. **Poll for CLA bot comments and checks** (within ~60 seconds of
       ...
       ```
       Look for:
       - Comments from known CLA bots: `google-cla`, `cla-assistant`,
         `CLAassistant`, `cla-bot`, `facebook-cla`, `apache-cla-bot`
       - Check runs with names containing "CLA", "license", "contributor"

    2. **If a CLA bot comment is found and the CLA check is failing**:
       Surface the comment and sign link to the user immediately. Do NOT
       let it sit as a surprise. Tell the user:
       - The CLA bot name and what it requires
       - The sign link (from the bot comment)
       - That the PR will not be reviewed until the CLA is signed

       If the user confirms they will sign (or have already signed),
       record/update the ledger entry:
       ```bash
       scripts/skill-config.sh set --layer user "cla.$ORG.signed_at" "$(date +%Y-%m-%d)"
       scripts/skill-config.sh set --layer user "cla.$ORG.source" "auto-detected-from-pr-$PR_NUMBER"
       ```

    3. **If the CLA check passes** (either immediately, or after the user
       signs): confirm the ledger entry with `source` upgraded to
       `auto-detected-from-pr-$PR_NUMBER`:
       ```bash
       scripts/skill-config.sh set --layer user "cla.$ORG.source" "auto-detected-from-pr-$PR_NUMBER"
       ```

    4. **If the CLA check fails after a ledger entry existed** (revocation,
       expiry, or per-project-CLA edge case): invalidate the ledger entry
       so the user is re-warned next time:
       ```bash
       scripts/skill-config.sh invalidate --layer user "cla.$ORG"
       ```
       Surface the failure to the user with the reason.

    If no CLA bot activity is detected within 60 seconds, and the pre-push
    gate found no CLA signals, the PR is clean — proceed.

## Cache and Config

### Contribution Standards Cache (regenerable)

Contribution standards are cached per-repository at:
```
${XDG_CACHE_HOME:-~/.cache}/skills/levonk/skills-src/skills/github-pr/<host>/<owner>/<repo>/standards.md
```

The cache has a 7-day TTL. Use `--skill github-pr` when calling
...
```
${XDG_CONFIG_HOME:-~/.config}/skills/levonk/skills-releases/skills/github-pr/config.toml
```

Schema (per-org tables):
```toml
[cla.VirusTotal]
signed_at = "2026-07-26"
expires_at = "2027-07-26"
source = "auto-detected-from-pr-719"  # or "manual" / "user-acknowledged"

[cla.apache]
signed_at = "2025-03-15"
expires_at = "2026-03-15"  # expired — will re-warn
source = "manual"
```

The ledger has a 365-day TTL. Entries are auto-populated by the
post-open CLA-watch (step 16) when the CLA check passes, and
invalidated when a CLA check fails after a prior entry existed.
Manual entries can be created via `scripts/skill-config.sh set`.

Read and write the ledger via `scripts/skill-config.sh` — never edit
the TOML file directly, as the trust gate and three-layer resolution
would be bypassed.

## Knowledge Base

This skill materializes the `upstream-contribution-practices` knowledge bundle
at `references/included/knowledge/upstream-contribution-practices/`. Read the
relevant concept pages before opening a PR:

- [Minimal Scope](references/included/knowledge/upstream-contribution-practices/minimal-scope.md) — one change per PR
- [Feature Branch Only](references/included/knowledge/upstream-contribution-practices/feature-branch-only.md) — never commit to main
- [Test Baseline](references/included/knowledge/upstream-contribution-practices/test-baseline.md) — establish before working
- [Sync Before Push](references/included/knowledge/upstream-contribution-practices/sync-before-push.md) — rebase onto latest
- [Rebase, Never Merge](references/included/knowledge/upstream-contribution-practices/rebase-never-merge.md) — keep linear history
- [Linear History](references/included/knowledge/upstream-contribution-practices/linear-history.md) — squash iterative commits
- [Human Review Gate](references/included/knowledge/upstream-contribution-practices/human-review-gate.md) — present before posting
- [gh --body-file](references/included/knowledge/upstream-contribution-practices/gh-body-file.md) — never `--body` with inline strings
- [Upstream Identity](references/included/knowledge/upstream-contribution-practices/upstream-identity.md) — reference upstream, not your fork
- [Git Author Privacy](references/included/knowledge/upstream-contribution-practices/git-author-privacy.md) — no private hostname/username
- [No User Identity Leak](references/included/knowledge/upstream-contribution-practices/no-user-identity-leak.md) — no local paths in content
- [Format Artifacts](references/included/knowledge/upstream-contribution-practices/format-artifacts.md) — run project formatter
- [Lint Artifacts](references/included/knowledge/upstream-contribution-practices/lint-artifacts.md) — run project linters
- [Separate Style Commit](references/included/knowledge/upstream-contribution-practices/separate-style-commit.md) — keep style fixes separate
- [Massive-Change Guard](references/included/knowledge/upstream-contribution-practices/massive-change-guard.md) — revert over-broad auto-fixes

## Troubleshooting

| Symptom | Cause | Fix |
|---------|-------|-----|
| `gh pr create` fails with no fork | Forgot to fork first | `gh repo fork <owner>/<repo> --clone` |
| PR has merge commits | Merged upstream instead of rebasing | `git rebase upstream/main`, force-push the branch |
| `validate-pr-issue.sh` exits non-zero | Body corrupted (literal `\n` or stripped backticks) | Re-draft, write to file, `gh pr edit --body-file`, re-validate |
| Tests fail on the PR but passed at baseline | Your change broke something | Fix the change; do not disable tests |
| Tests fail at baseline too | Pre-existing failures | Document them in the PR body; do not fix in this PR |
| `scan-artifacts.sh` exits non-zero | Identity leak in generated files | Replace personal references with relative or upstream-relative paths |
| Maintainer asks to match changelog style | Didn't follow project conventions | Re-read the cached standards file; update the PR |
| Surprise CLA bot comment after PR open | Org-level CLA not detected by crawler (no in-repo signal) | The post-open CLA-watch (step 16) should catch this; if it didn't, the org is missing from the known-CLA list — add it to `discover-contribution-standards.sh` |
