...
* [Deterministic Image Versioning](image-versioning.md) - Derive every image version from Git state with scripts/version.sh emitting <epoch>.<semver>-<commit>-<dirty>; inject via build args + OCI labels; never use Git smudge/clean filters; post-deployment override via ConfigMap, bind mount, or env var
* [Volume Ownership Init for Non-Root Containers](volume-ownership-init.md) - Three-phase volume init (chown + in-container verify, then fresh-container verify) before starting any non-root container with a writable volume; catches Docker Desktop WSL2 persistence failures where chown exits 0 but ownership does not persist to the backing store; also covers the entrypoint wrapper image pattern for self-healing containers, the implemented `localnet-volume-init` utility container, and the reusable `localnet-volume-init` Ansible role with intelligent defaults and multi-volume spec support
