# Matrix Output Format (Project-Specific)

## Table of Contents

1. [Overview](#overview)
2. [Project-Specific Meta-Features](#project-specific-meta-features)
3. [Full Example](#full-example)

## Overview

The generic matrix table structure, recommendation framework, and icon legend
are defined in the shared `comparison-methodology` include and the
`feature-matrix` rule. This file adds project-specific meta-features and a
full example.
...
## Project-Specific Meta-Features

The top of the table contains standard meta-features that apply to all
project comparisons. License and coverage icons follow the shared references:
...
| Meta-feature | What to include | Icon guidance |
|---|---|---|
| License | License type | Use the shared license-icons scale (⭐/☑️/⚠️/❌) |
| UX/UI | Quality of CLI, web UI, or GUI | 🏆 polished / ✅ usable / ➖ basic / ⚠️ rough / ❌ none |
| Setup Difficulty | How hard to get started | 🏆 < 5 min / ✅ < 30 min / ➖ < 1 hr / ⚠️ > 1 hr / ❌ very hard |
| Community | Activity of discussions, issues, PRs | 🏆 very active / ✅ active / ➖ moderate / ⚠️ low / ❌ dead |
| Last Commit | Days since last push | From GitHub metadata `pushed_at` |
| Stars | GitHub star count | From GitHub metadata `stargazers_count` |
| Forks | GitHub fork count | From GitHub metadata `forks_count` |
| Year Introduced | Year the repo was created | From GitHub metadata `created_at` |
| Public Repo | Link to the repository | Inline link |
| Run Modes | Server / GUI / CLI / Web App / Library / API | List applicable modes |
| Tech Stack | Primary languages and frameworks | From metadata or `project-detection` |
| Platform Support | OS / runtime support | From docs or metadata |
| Maintainability | Combined rating from `maintainability-scoring.md` | Use the shared coverage-scale icons (🏆/✅/➖/⚠️/❌) |
...
All three follow the same content → build core → template/asset pipeline →
static output flow. Hugo and Eleventy are CLI pipelines; Gatsby adds a
GraphQL data layer and React renderer. The architectures are similar enough
that a single diagram suffices:

```mermaid
graph TD
    Content[Content Files] --> Core[Build Core]
...
|---|---|---|---|
| Process model | CLI pipeline | CLI + dev server | CLI pipeline |
| Extension model | Modules + themes | React plugins | Plugins (JS) |
| Data flow | Pull-based | GraphQL-driven | Pull-based |
| Deployment | Single binary | Node + CDN | Single binary |

## Feature Matrix

| Feature | [Hugo](https://github.com/gohugoio/hugo) | [Gatsby](https://github.com/gatsbyjs/gatsby) | [Eleventy](https://github.com/11ty/eleventy) |
|---|---|---|---|
| **Meta** | | | |
| License | ⭐ FOSS | ⭐ FOSS | ⭐ FOSS |
| UX/UI (CLI) | 🏆 | ✅ | ✅ |
| Setup Difficulty | ✅ < 30 min | ➖ < 1 hr | ✅ < 30 min |
| Community | 🏆 | ✅ | ✅ |
| Last Commit | 2 days ago | 5 days ago | 1 week ago |
| Stars | 78k | 55k | 16k |
| Forks | 7.5k | 10k | 500 |
| Year Introduced | 2013 | 2015 | 2018 |
| Run Modes | CLI | CLI, GraphQL | CLI |
| Tech Stack | Go | React, JS | JS |
...
- **Hugo**: Fastest builds, richest feature set, steepest learning curve for
  templating.
- **Gatsby**: React ecosystem integration, large plugin ecosystem, slower
  builds.
- **Eleventy**: Minimal, flexible, no opinionated framework — great for simple
  sites.
...
- **For beginners / simple sites**: Eleventy — minimal magic, easy to understand, no framework lock-in.
- **For React teams / content-rich sites**: Gatsby — leverages React knowledge, large plugin ecosystem, GraphQL data layer.
- **For large sites (1000+ pages) / build performance critical**: Hugo — build performance is unmatched, richest feature set.
- **Watch**: None currently — all three are well-established.
```
