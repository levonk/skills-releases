...
Mine git history for bug-fix commits, inventory which fixes lack a regression
test, and dispatch `unit-test-writing` to author the missing tests — each
referencing the fix commit SHA that exposed the original bug.

The core contract: **every bug fix in a project's history should have a test
that would have caught the bug.** A fix without a test is a future regression
waiting to happen.

## Quick Start

1. **Mine** — run `scripts/mine-bug-fixes.sh` to collect bug-fix commits from
   `git log` (conventional `fix:` prefixes, `Fixes #N` / `Closes #N` trailers,
   reverts, and `git blame`-identified bug-introducing commits).
2. **Classify** — for each mined commit, determine whether a regression test
   already exists (see [Commit Classification](references/commit-classification.md)).
3. **Inventory** — build the list of fixes that lack a covering test (the
   "test gap" list).
4. **Detect framework** — run `project-detection` to identify the project's
   test framework so generated tests use it (see
   [Framework Detection](references/framework-detection.md)).
5. **Dispatch** — for each gap, dispatch `unit-test-writing` to author a
   regression test that fails against the buggy code and passes against the
   fix. Each test references the fix commit SHA (see
   [SHA Referencing](references/sha-referencing.md)).
6. **Verify** — run the new tests against the current (fixed) code; they
   should pass. Where reproducible, check them out against the parent of the
   fix commit; they should fail.
7. **Report** — summarize: commits mined, gaps found, tests added, gaps
   deferred (with rationale).

## The Mining Pipeline

### Step 1: Mine bug-fix commits

Run `scripts/mine-bug-fixes.sh` from the repository root. The script:

- Collects commits matching conventional `fix:` / `fix(scope):` prefixes
- Collects commits with `Fixes #N`, `Closes #N`, `Resolves #N` trailers
- Collects revert commits (`Revert "..."`)
- Optionally runs `git blame` to find bug-introducing commits (via
  `--blame`, slow on large repos — see
  [Blame-Based Mining](references/blame-mining.md))
- Accepts `--since <date>` and `--max-count <N>` to bound the scan
- Defaults to the full history capped at 1000 commits to avoid unbounded
  runs on large repos
- Emits one commit per line: `<sha>\t<author-date>\t<subject>\t<trailers>`

```bash
# Default: full history, capped at 1000 commits
bash scripts/mine-bug-fixes.sh

# Last 90 days, no cap
bash scripts/mine-bug-fixes.sh --since="90 days ago" --max-count=10000

# Include blame-based bug-introducing commit detection (slow)
bash scripts/mine-bug-fixes.sh --blame --since="180 days ago"

# Feed a known bad commit from git bisect directly
bash scripts/mine-bug-fixes.sh --bisect <sha>

# JSON output for scripting
bash scripts/mine-bug-fixes.sh --json --since="2026-01-01"
```

The script is the single AI→script handoff for history mining. Use its output
as the input to Step 2.

### Step 2: Classify each commit

For each mined commit, determine its test status. See
[Commit Classification](references/commit-classification.md) for the full
decision tree. The four outcomes:

| Status | Meaning | Action |
|--------|---------|--------|
| `COVERED` | A test exists that would have caught the bug | Skip |
| `GAP` | No test exists, or the test exists but does not cover the fixed behavior | Add to gap list |
| `UNREPRODUCIBLE` | The bug depends on environment/state that cannot be reconstructed | Defer with rationale |
| `COSMETIC` | The fix is a typo, formatting, or non-behavioral change | Skip |

### Step 3: Inventory the gaps

Build a structured list of the `GAP` commits. For each:

- The fix commit SHA
- The file(s) and function(s) the fix touched
- A one-line description of the bug the fix addressed
- The detected test framework (from Step 4)
- The proposed test name (Osherove `MethodUnderTest_Scenario_ExpectedOutcome`)

### Step 4: Detect the test framework

Run `project-detection` to identify the project's test framework. The skill
never imposes a framework the project does not use. See
[Framework Detection](references/framework-detection.md) for the detection
matrix and per-framework test file conventions.

### Step 5: Dispatch unit-test-writing

For each gap, dispatch `unit-test-writing` with:

- The fix commit SHA
- The diff of the fix (`git show <sha>`)
- The file(s) and function(s) to test
- The detected test framework
- The proposed test name

`unit-test-writing` authors the test in the Osherove style (readable,
maintainable, trustworthy). Each generated test must reference the fix commit
SHA so the lineage is traceable. See
[SHA Referencing](references/sha-referencing.md) for the per-framework
convention.

### Step 6: Verify

Run the new tests against the current (fixed) code — they should pass. Where
the bug is reproducible, check out the parent of the fix commit and confirm
the test fails there. This is the "fail before, pass after" contract that
makes a regression test trustworthy.

```bash
# Run the new tests against the fixed code
# (use the project's test runner — detected in Step 4)

# Verify the test fails against the buggy code (when reproducible)
git stash                          # save any working changes
git checkout <fix-sha>~1           # the buggy parent
# run the new test — it should FAIL
git checkout -                     # return to the fixed code
git stash pop                      # restore working changes
```

If the test passes against the buggy code, it does not actually test the fix —
return to Step 5 with the feedback.

### Step 7: Report

Summarize the mining run:

- Commits mined (by source: conventional / trailer / revert / blame / bisect)
- Gaps found vs. covered vs. unreproducible vs. cosmetic
- Tests added (with SHAs)
- Tests deferred (with rationale)
- Suggested next steps (e.g., "10 gaps deferred as unreproducible — consider
  integration tests for the environment-dependent ones")

## Input Modes

The skill accepts three input modes:

1. **Full history** (default) — mine `git log` for all bug-fix commits in range.
2. **Single commit** — `--bisect <sha>` when the user has already run
   `git bisect` and identified the bug-introducing commit.
3. **Commit list** — `--from-file <path>` when the user has a pre-filtered
   list of commit SHAs (e.g., from a CI failure log, a security audit, or a
   `git log --author=<user>` query).

See [Input Modes](references/input-modes.md) for the per-mode workflow.

## What This Skill Does Not Do

- **Does not fix bugs** — it mines history for *tests* of already-fixed bugs.
  For active bug fixing, use the project's normal development workflow.
- **Does not author tests in isolation** — it dispatches `unit-test-writing`
  for the actual Osherove-style authoring. This skill identifies *what* to
  test; `unit-test-writing` handles *how*.
- **Does not impose a test framework** — it detects the project's existing
  framework via `project-detection` and uses it.
- **Does not run the full test suite** — it runs only the new regression
  tests for verification. For full suite validation, dispatch
  `code-quality-validation`.
- **Does not mine for code smells or refactoring opportunities** — use
  `refactor-planning` for that.

## See Also

- [Commit Classification](references/commit-classification.md) — the full
  decision tree for classifying a mined commit as COVERED / GAP /
  UNREPRODUCIBLE / COSMETIC
- [Framework Detection](references/framework-detection.md) — the test
  framework detection matrix and per-framework conventions
- [SHA Referencing](references/sha-referencing.md) — how to reference the
  fix commit SHA in each test framework
- [Blame-Based Mining](references/blame-mining.md) — when and how to use
  `git blame` to find bug-introducing commits (slow, opt-in)
...
- [ ] **[script]** `scripts/mine-bug-fixes.sh` exits zero and emits at least one commit line in `<sha>\t<date>\t<subject>\t<trailers>` format (Step 1)
- [ ] **[manual]** The scan range is appropriate — `--since` / `--max-count` was set deliberately, not left at default when the repo exceeds 1000 commits (Step 1)
- [ ] **[manual]** All four commit sources were considered: conventional `fix:` prefixes, `Fixes #N`/`Closes #N`/`Resolves #N` trailers, reverts, and (if `--blame` was used) blame-based bug-introducing commits (Step 1)

### Classification (Step 2)

- [ ] **[manual]** Every mined commit was classified into exactly one of: `COVERED`, `GAP`, `UNREPRODUCIBLE`, `COSMETIC` — no commit left unclassified (Step 2)
- [ ] **[manual]** `UNREPRODUCIBLE` classifications include a rationale explaining why the bug cannot be reconstructed (Step 2)
- [ ] **[manual]** `COSMETIC` classifications are genuinely non-behavioral (typo, formatting) — not a real bug fix mislabeled to avoid writing a test (Step 2)

### Gap Inventory (Step 3)

- [ ] **[manual]** Every `GAP` commit appears in the inventory with: fix commit SHA, file(s)/function(s) touched, one-line bug description, detected test framework, and proposed test name (Step 3)
- [ ] **[manual]** Proposed test names follow the Osherove `MethodUnderTest_Scenario_ExpectedOutcome` pattern (Step 3)

### Framework Detection (Step 4)

- [ ] **[manual]** The project's actual test framework was detected via `project-detection` — no framework was imposed that the project does not use (Step 4)
- [ ] **[manual]** Test file location conventions match the detected framework's expectations (Step 4)

### Test Authoring and Verification (Steps 5-6)

- [ ] **[manual]** Each generated test references the fix commit SHA using the detected framework's SHA-referencing convention (Step 5)
- [ ] **[manual]** Each new test passes against the current (fixed) code (Step 6)
- [ ] **[manual]** Where reproducible: each new test fails against the parent of the fix commit (`git checkout <fix-sha>~1`) — the "fail before, pass after" contract (Step 6)
- [ ] **[manual]** Any test that passes against the buggy code was returned to Step 5 for rework — not accepted as a passing test (Step 6)

### Report (Step 7)

- [ ] **[manual]** The report breaks down commits mined by source (conventional / trailer / revert / blame / bisect) (Step 7)
- [ ] **[manual]** The report counts gaps found vs. covered vs. unreproducible vs. cosmetic (Step 7)
- [ ] **[manual]** Tests added are listed with their SHAs; tests deferred are listed with rationale (Step 7)

### Not Done (common false-completion signals)

If any of these are true, the run is NOT complete:

- `mine-bug-fixes.sh` emits commits but all are classified `COSMETIC` with no rationale → classification was rushed; real bug fixes were skipped to avoid writing tests
- A test passes against the fixed code but was never checked out against the buggy parent → the test may not actually exercise the fix; it is not a trustworthy regression test
- A test passes against the buggy code → it does not test the fix at all; return to Step 5
- The inventory lists gaps but the report's "tests added" count is lower than the gap count with no deferred rationale → gaps were silently dropped
- Generated tests use a framework the project does not use → the tests will not run in the project's CI and provide no protection

## Context Declaration

### File Paths
- Main skill: `src/current/skills/software-dev/regression-test-mining/SKILL.md.tmpl`
- Instructions: `src/current/skills/software-dev/regression-test-mining/INSTRUCTIONS.md.tmpl`
- Mining script: `src/current/skills/software-dev/regression-test-mining/scripts/mine-bug-fixes.sh.tmpl`
- References: `src/current/skills/software-dev/regression-test-mining/references/`
