...
# shellcheck disable=SC2034 # Arrays document the tool preference hierarchy
...
# shellcheck disable=SC2034 # Path constant for future use
...
create_backup() {
	...
	local backup
	backup="$cache_dir/$(basename "$file").$timestamp"
...
validate_file() {
	...
	"code")
		...
		*)
			echo "Warning: No syntax check for ${file##*.}"
			return 0
			;;
		esac
		;;
...
apply_patch_management() {
	...
	if tool_available "quilt"; then
		...
		local patch_name
		patch_name="surgical-edit-$(date +%Y%m%d%H%M%S).patch"
