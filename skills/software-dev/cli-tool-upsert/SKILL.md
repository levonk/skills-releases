...
2. **For embedded scripts: generate from template.** Use
   ...
   the same file `ai-upsert` includes so skills and their scripts share
   one source of truth.
...
| `references/embedded-script-standards.md` | Shared standard for embedded scripts (also included by ai-upsert) |
