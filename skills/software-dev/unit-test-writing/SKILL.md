...
description: "Write unit tests in the style of Roy Osherove's 'The Art of Unit Testing' — readable, maintainable, and trustworthy. Use when writing unit tests, adding test coverage, creating test cases for existing code, or learning unit testing best practices. Triggers on 'write unit tests', 'create tests', 'test this code', 'add test coverage', 'unit test best practices', or 'Roy Osherove style tests'. Do NOT trigger on general coding questions, bug fixes, feature implementation, or end-to-end/integration test setup — this skill is for unit test authoring, not integration or e2e testing."
version: 1.0.0
...
see-also:
  - skill: code-quality-validation
    ...
    description: "Test execution and validation as part of the broader quality gate"
  - skill: code-review-guidance
    relationship: "related"
    description: "Review test code with the same rigor as production code"
  - skill: refactor-planning
    relationship: "related"
    description: "Characterization tests are the foundation of safe refactoring"
  - knowledge: cicd-testing-practices
    relationship: "reference"
    description: "CI/CD testing patterns and pipeline integration"
---
...
Write unit tests in the style of Roy Osherove's "The Art of Unit Testing":
**readable, maintainable, and trustworthy**. A good unit test is the
programmer's safety net — it catches regressions, documents intent, and
...
A good unit test satisfies all three:

| Pillar | Question | Failure mode |
|--------|----------|--------------|
| **Readable** | Can a new team member understand the test in seconds? | Cryptic names, hidden setup, magic values |
| **Maintainable** | Will the test survive unrelated refactors? | Overspecification, brittle assertions, deep coupling to implementation |
| **Trustworthy** | Does the test fail when the code is wrong and pass when the code is right? | Tests that always pass, tests that test nothing, tests with the wrong invariant |

## AAA Pattern (Arrange-Act-Assert)

Every test should have exactly three sections, separated by blank lines:

```typescript
it("calculateTotal_appliesDiscountToEligibleItems_returnsDiscountedTotal", () => {
  // Arrange
  const cart = new Cart([
    new Item("book", 20, { eligibleForDiscount: true }),
    new Item("pen", 5, { eligibleForDiscount: false }),
  ]);
  const discount = new Discount(0.10);

  // Act
  const total = cart.calculateTotal(discount);

  // Assert
  expect(total).toBe(23); // 20 - 2 + 5
});
```
...
| Bad | Good |
|-----|------|
| `testCalculateTotal` | `calculateTotal_emptyCart_returnsZero` |
| `test1` | `applyDiscount_expiredDiscount_throwsError` |
| `shouldWork` | `submitOrder_outOfStockItem_returnsOutOfStockError` |

The name should describe the **behavior**, not the implementation. A reader
should know what the test verifies without reading the body.

## What Makes a Good Unit Test

A good unit test is:

- **Fast** — runs in milliseconds; no network, no disk, no sleeps
- **Isolated** — does not depend on other tests running first (or at all)
- **Repeatable** — gives the same result every run, in any order, on any machine
- **Self-validating** — clearly passes or fails; no manual inspection needed
- **Timely** — written close to (or before) the production code it tests
- **Single-behavior** — tests one logical outcome of one unit
- **Deterministic** — no random data, no time-of-day dependencies, no flakiness

## Common Anti-Patterns

For a full catalog with examples and fixes, see
[Test Anti-Patterns](references/test-anti-patterns.md).

The big ones:

- **Multiple asserts per test** — split into multiple tests
- **Testing private methods** — test through the public API; if a private method needs its own tests, it probably wants to be its own class
- **Overspecification** — asserting on internal state, call counts, or order that doesn't matter to the behavior
- **Shared mutable setup** — leaks state between tests; use fresh fixtures per test
- **Test logic in production code** — `if (isTest)` branches; dependency injection instead
- **Sleep-based timing** — flaky and slow; use fake clocks or async polling

## Test Doubles: Mocks, Stubs, Fakes

For the full guide, see [Test Patterns](references/test-patterns.md).

Quick definitions:

| Double | Purpose | Verifies |
|--------|---------|----------|
| **Stub** | Returns canned answers when called | Nothing — just provides inputs |
| **Mock** | Records calls and lets you assert on them | Interactions (was the method called? with what args?) |
| **Fake** | A working but simplified implementation | Real behavior (e.g., in-memory repository) |

**Prefer stubs and fakes over mocks.** Mocks couple tests to implementation
details; stubs and fakes couple tests to behavior.

## Roy Osherove's Principles

For the full treatment, see
[Roy Osherove Principles](references/roy-osherove-principles.md).

The essentials:

1. **A unit test should test one thing and test it well.**
2. **A unit test should be readable in seconds** — clear name, obvious arrange, single act, single assertion.
3. **A unit test should be maintainable** — change the implementation, not the test, when refactoring.
4. **A unit test should be trustworthy** — fail when the code is wrong, pass when the code is right, never pass for the wrong reason.
5. **Avoid logic in tests** — no `if`, no `for`, no `switch`. If you need logic, you have multiple tests.
6. **Avoid multiple asserts** — one logical outcome per test.
7. **Avoid testing private methods** — test the public API.
8. **Avoid overspecification** — don't assert on things that don't matter to the behavior.

## When to Write Tests

- **TDD (test-first)** — for new features and bug fixes; the test drives the design
- **Characterization tests** — for legacy code before refactoring; capture current behavior (see `refactor-planning` skill)
- **Regression tests** — for every bug fix; a test that fails before the fix and passes after
- **Edge cases** — empty input, null, boundary values, off-by-one, concurrency

## See Also

- [Roy Osherove Principles](references/roy-osherove-principles.md) — full principles from "The Art of Unit Testing"
...
### File Paths
- Main skill: `src/current/skills/software-dev/unit-test-writing/SKILL.md.tmpl`
- References: `src/current/skills/software-dev/unit-test-writing/references/`
