...
get_next_ticket() {
	...
	local next_ticket
	next_ticket=$(tkr ready 2>/dev/null | head -1)
...
status_overview() {
	...
	current_ticket=$(tkr list --status=in_progress --format=short 2>/dev/null | head -1)
	...
	# Queue status
	local open_count
	open_count=$(tkr list --status=open --format=count 2>/dev/null || echo "0")
	local ready_count
	ready_count=$(tkr list --status=ready --format=count 2>/dev/null || echo "0")
...
# shellcheck disable=SC2120 # Arguments passed through to helper script via "$@"
language_aware_loop() {
	local project_type
...
run_ticket_audit() {
	...
	local doc_files=0
	if [[ -f "README.md" ]] || ls docs/* >/dev/null 2>&1; then
