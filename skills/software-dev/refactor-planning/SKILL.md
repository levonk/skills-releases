...
see-also:
  - skill: code-quality-validation
    ...
    description: "For comprehensive code quality checks that validate each refactor step"
  - skill: git-repository-management
    relationship: "related"
    description: "For commit organization and rollback-safe checkpoints between refactor steps"
...
Systematic evolutionary refactor workflow based on Michael Feathers' "Working
Effectively with Legacy Code." Assure a safe starting state, identify code
smells and structural issues, create a prioritized task plan, and execute
refactors in verifiable increments.
...
## Evolutionary Refactor Workflow

### Phase 1 — Assure Clean Starting State

Before touching anything, confirm a safe baseline:

1. **Verify the build**: typecheck, build, lint, unit tests, and run all pass.
2. **Verify the repository**: no untracked, unadded, changed, or staged files (`git status` is clean).
3. **Verify the branch**: working tree is on a feature branch independent of `main`/`master`, `env/dev`, `env/prod`.

If any gate fails, stop and fix the baseline first. A dirty starting state makes
it impossible to tell whether a later failure was caused by your refactor or
pre-existing debt.

### Phase 2 — Identify Issues

Use language-specific annotations or doc strings to mark findings. Scan for:

| Category | What to look for |
|----------|-----------------|
| **Security issues** | Secrets in logs/code, missing input validation, insecure dependencies |
| **Compliance issues** | License incompatibilities, missing data-handling controls |
| **Code smells** | See [Code Smell Catalog](references/code-smell-catalog.md) |
| **Legacy tech / APIs / deps** | EOL frameworks, deprecated APIs, outdated libraries |
| **Standards issues** | Coding convention violations, missing error handling |
| **Pattern opportunities** | Where creational/structural/behavioral patterns would simplify code |
| **Integration improvements** | Missing rate limits, fallbacks, retries, circuit breakers |
| **Package extraction** | Code that belongs in a shared dependency (logging, monitoring, auth) |

For Michael Feathers' legacy code techniques (seams, characterization tests,
dependency breaking), see [Legacy Code Techniques](references/legacy-code-techniques.md).

### Phase 3 — Create Prioritized Task Plan

Organize identified issues into tasks using this priority order:

1. **Urgent** — security incidents, immediate high risks, data-loss dangers
2. **Foundational** — changes that unblock other tasks (e.g., extracting a
   shared interface before replacing call sites)
3. **Dependent** — tasks that depend on foundational work being complete
4. **Low priority** — speculative improvements, nice-to-have cleanups

For the full prioritization procedure (including the business-impact ranking
from the source workflow), see [Refactor Checklist](references/refactor-checklist.md).

### Phase 4 — Execute in Priority Order

For each task:

1. Make the change (smallest viable increment).
2. **Verify**: typecheck, build, lint, unit tests, run — all must pass.
3. **Confirm clean repo**: no untracked, unadded, changed, or staged files
   beyond the intended change.
4. **Commit** the verified change (use `git-repository-management` skill for
   rollback-safe commits with pre/post tags).
...
2. **Verify the repository**: clean working tree, all changes committed.
3. Summarize what was refactored, what was deferred, and any new tech debt
...
| Group | Key smells |
|-------|-----------|
| **Bloaters** | Long Method, Long Parameter List, Primitive Obsession, Data Clumps |
| **OO Abusers** | Alternative Classes w/ Different Interfaces, Refused Bequest, Switch Statements, Temporary Field |
...
| **Couplers** | Feature Envy, Incomplete Library Class, Middle Man, Inappropriate Intimacy, Message Chains |
| **AI-specific** | Full-cycle prompts, heavy prompts without preprocessing, not wired for evals, AI used where deterministic code suffices |

For full descriptions and remediation guidance, see
[Code Smell Catalog](references/code-smell-catalog.md).

## Refactoring Techniques (Summary)

- **Composing Methods**: Extract/Inline Method, Replace Temp with Query, Substitute Algorithm
- **Moving Features**: Move Method/Field, Extract/Inline Class, Hide Delegate
- **Organizing Data**: Replace Data Value with Object, Replace Magic Number with Constant, Replace Type Code with State/Strategy
- **Simplifying Conditionals**: Decompose Conditional, Replace Nested Conditional with Guard Clauses, Replace Conditional with Polymorphism
- **Simplifying Method Calls**: Rename Method, Separate Query from Modifier, Introduce Parameter Object, Replace Constructor with Factory Method
- **Dealing with Generalization**: Pull Up/Push Down Method & Field, Extract Subclass/Superclass/Interface, Collapse Hierarchy
