...
# Issue Watcher: Bulk-Subscribe to GitHub Issues/PRs via GraphQL

Subscribe the user to a list of GitHub issues/PRs (or discover them by topic
keyword), then subscribe via the GitHub GraphQL API with rate-limit-aware
delays and a summary report.

## Outcome

The user is subscribed (or unsubscribed) to a set of GitHub issues/PRs in a
target repository, with a clear summary of what succeeded, what was skipped
(already subscribed), and what failed.

## Good Outcome

- The `notifications` scope was verified before any API call
- In keyword mode, the discovered issues were presented for user confirmation
  before subscribing
- Each subscription used the GraphQL `updateSubscription` mutation with the
  correct node ID
- A 2-second delay was inserted between each subscription
- Already-subscribed items were counted as `skipped`, not `failed`
- Locked issues were handled (subscriptions work on locked issues)
- A summary report listed subscribed/skipped/failed counts

## Bad Outcome

- The script attempted subscriptions without checking the `notifications` scope
- Keyword-mode results were subscribed without user confirmation
- Subscriptions were fired in parallel or with no delay (secondary rate limit)
- Already-subscribed items were reported as errors
- No summary report was printed
- The script crashed on a locked issue instead of continuing

## Guardrails

1. **Must check scope first**: the script runs `gh auth status` and checks for
   the `notifications` scope before any subscription. If missing, it prints
   `gh auth refresh -s notifications` and exits.
2. **Must delay between subscriptions**: a 2-second delay is inserted between
   each GraphQL mutation. Never parallelize subscription mutations.
3. **Must present search results for user confirmation**: in keyword mode, the
   script prints the discovered issues and waits for user confirmation before
   subscribing. No auto-subscribing to search results.
4. **Must handle already-subscribed gracefully**: if the mutation returns
   success on an already-subscribed item, count it as `skipped`, not `failed`.
5. **Must handle locked issues**: issue locking blocks comments, not
   subscriptions. Continue if a locked issue's subscription succeeds.

## Available Resources

- `scripts/watch_issues.py` — PEP 723 Python script (run via
  `uv run --script`). Handles scope check, issue resolution (explicit or
  keyword search), node ID fetch, GraphQL subscription mutation, 2-second
  delays, and summary report.

## Current Process

### Step 1: Check `notifications` Scope

Run `gh auth status` and verify the `notifications` scope is present on the
authenticated token. If missing, print the remediation command and exit:

```bash
gh auth refresh -s notifications
```

The `updateSubscription` GraphQL mutation requires this scope. Without it,
all subscriptions will fail with a 403.

### Step 2: Resolve Issue Numbers

**Mode 1 (explicit list)**: parse `--numbers` into a list of integers.

**Mode 2 (keyword discovery)**: run a search via the gh CLI:

```bash
gh search issues --repo owner/repo "keyword" --state open
```

Parse the results into a list of issue numbers and titles.

### Step 3: Present Discovered Issues for User Confirmation (Keyword Mode)

In keyword mode, print the discovered issues (number + title + state) and
wait for the user to confirm before subscribing. Do not auto-subscribe to
search results — the user may want to filter the list.

In explicit-list mode, skip this step — the user already chose the numbers.

### Step 4: Fetch GraphQL Node IDs

For each issue number, fetch the GraphQL node ID via the REST API:

```bash
gh api repos/owner/repo/issues/42 --jq '.node_id'
```

The `node_id` field is the GraphQL global ID required by the
`updateSubscription` mutation.

### Step 5: Subscribe/Unsubscribe via GraphQL Mutation with 2s Delays

For each node ID, invoke the `updateSubscription` mutation via the gh CLI:

```bash
gh api graphql -f query='mutation Subscribe($id: ID!, $state: SubscriptionState!) {
...
}' -F id="$NODE_ID" -f state="SUBSCRIBED"
```

Use `UNSUBSCRIBED` instead of `SUBSCRIBED` when `--unsubscribe` is set.

Insert a 2-second delay between each mutation. Handle already-subscribed
items (mutation succeeds, count as `skipped`). Handle locked issues
(subscriptions still work on locked issues — continue on success).
...
Summary:
  Subscribed: 5
  Skipped (already subscribed): 2
  Failed: 0
```

List any failed items with their issue numbers and error messages.
...
- [ ] Check `notifications` scope on the gh CLI token (Step 1)
- [ ] Resolve issue numbers — explicit list or keyword search (Step 2)
- [ ] Present discovered issues for user confirmation in keyword mode (Step 3)
- [ ] Fetch GraphQL node IDs for each issue (Step 4)
- [ ] Subscribe/unsubscribe via GraphQL mutation with 2-second delays (Step 5)
- [ ] Report summary — subscribed/skipped/failed counts (Step 6)
...
Before declaring the issue-watcher run complete, verify every item below.
...
- [ ] **[script]** `watch_issues.py` checked the `notifications` scope via `gh auth status` and did not proceed without it (Step 1)
- [ ] **[manual]** If the scope was missing, the script printed `gh auth refresh -s notifications` and exited without attempting subscriptions (Step 1)

### Issue Resolution

- [ ] **[script]** In explicit-list mode, `--numbers` was parsed into a valid list of integers (Step 2)
- [ ] **[script]** In keyword mode, `gh search issues` was run and returned results (Step 2)
- [ ] **[manual]** In keyword mode, the discovered issues were presented to the user for confirmation before subscribing — no auto-subscribing to search results (Step 3)

### Subscriptions

- [ ] **[script]** Each issue's GraphQL node ID was fetched via `gh api repos/{repo}/issues/{num} --jq '.node_id'` (Step 4)
- [ ] **[script]** Each subscription used the `updateSubscription` GraphQL mutation with the correct node ID and state (Step 5)
- [ ] **[script]** A 2-second delay was inserted between each subscription mutation (Step 5, Rate Limit Awareness)
- [ ] **[manual]** Already-subscribed items were counted as `skipped`, not `failed` (Step 5, Rate Limit Awareness)
- [ ] **[manual]** Locked issues were handled — subscriptions succeeded on locked issues, or failures were reported without crashing (Step 5)

### Summary

- [ ] **[script]** A summary report was printed with subscribed/skipped/failed counts (Step 6)
- [ ] **[manual]** Any failed items were listed with their issue numbers and error messages (Step 6)

### Not Done (common false-completion signals)

If any of these are true, the run is NOT complete:

- The script ran subscriptions but never checked the `notifications` scope → all mutations may have failed silently (Step 1)
- Keyword-mode results were subscribed without user confirmation → the user was not given a chance to filter the list (Step 3)
- Subscriptions were fired with no delay or in parallel → secondary rate limit may have been triggered (Step 5, Rate Limit Awareness)
- Already-subscribed items were reported as `failed` → the summary is misleading (Step 5)
- No summary report was printed → the user cannot verify the outcome (Step 6)

## Context Declaration

### File Paths

- `scripts/watch_issues.py` — PEP 723 Python script (run via `uv run --script`)
- `SKILL.md` — skill metadata and quick-start reference

### Variables

- `$REPO` — target repository in `owner/repo` format (e.g. `microsoft/vscode`)
- `$NUMBERS` — comma-separated issue/PR numbers (Mode 1)
- `$SEARCH` — keyword for issue discovery (Mode 2)
- `$STATE` — issue state filter: `open`, `closed`, or `all` (Mode 2, default: `all`)
- `$NODE_ID` — GraphQL node ID fetched per issue via `gh api`
- `$SUBSCRIPTION_STATE` — `SUBSCRIBED` or `UNSUBSCRIBED`

### Related Skills

- issue-sentiment-mapper (companion — identify which issues are worth watching)
- github-issue (issue creation with cross-referencing)
- base-ai-guidance (base-framework)

## References

- [GitHub GraphQL API — updateSubscription](https://docs.github.com/en/graphql/reference/mutations#updatesubscription)
- [GitHub GraphQL API — SubscriptionState](https://docs.github.com/en/graphql/reference/enums#subscriptionstate)
- [gh CLI — auth status](https://cli.github.com/manual/gh_auth_status)
- [gh CLI — search issues](https://cli.github.com/manual/gh_search_issues)
- [gh CLI — api graphql](https://cli.github.com/manual/gh_api_graphql)
