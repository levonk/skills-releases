...
tags: ["ai/skill", "software-development", "github", "issue", "subscription", "graphql", "notifications", "bulk", "watcher"]
see-also:
  - skill: issue-sentiment-mapper
    relationship: "complement"
    description: "Identify which issues are worth watching by sentiment analysis before bulk-subscribing"
  - skill: github-issue
    relationship: "complement"
    description: "Issue creation with cross-referencing — use this after filing an issue to watch related ones"
...
dependencies:
  - type: system
    ...
    reason: "Required for GitHub API calls (auth scope check, issue search, GraphQL mutations)"
  - type: system
    name: uv
    url: https://github.com/astral-sh/uv
    reason: "Required to run the Python watch_issues.py script via uv run --script"
triggers:
  - user
allowed-tools:
  - exec
...
Provide the repository and a comma-separated list of issue/PR numbers.

| Flag | Required | Description |
|------|----------|-------------|
| `--repo <owner/repo>` | yes | Target repository in `owner/repo` format |
| `--numbers <comma-separated-list>` | yes | Issue/PR numbers to subscribe to (e.g. `1,2,3,42`) |

### Mode 2: Keyword Discovery

Provide the repository and a search keyword. The script searches via
`gh search issues`, presents the results for user confirmation, then
subscribes to the confirmed list.

| Flag | Required | Description |
|------|----------|-------------|
| `--repo <owner/repo>` | yes | Target repository in `owner/repo` format |
| `--search <keyword>` | yes | Keyword to search for in issue titles/bodies |
| `--state <open\|closed\|all>` | no | Issue state filter (default: `all`) |

### Optional Flags

| Flag | Description |
|------|-------------|
| `--unsubscribe` | Unsubscribe instead of subscribe (sets mutation state to `UNSUBSCRIBED`) |

## Process

1. **Check `notifications` scope** on the gh CLI token. If missing, print
   `gh auth refresh -s notifications` and exit.
2. **Resolve issue numbers**:
   - Mode 1 (explicit): parse `--numbers` into a list.
   - Mode 2 (keyword): run `gh search issues --repo {repo} "{search}" --state {state}`,
     present results, wait for user confirmation.
3. **Fetch GraphQL node IDs** via `gh api repos/{repo}/issues/{number} --jq '.node_id'`.
4. **Subscribe** via GraphQL `updateSubscription` mutation with
   `state: SUBSCRIBED` (or `UNSUBSCRIBED` with `--unsubscribe`).
5. **2-second delay** between each subscription (rate limit awareness).
6. **Report summary**: subscribed/skipped/failed counts.

## Scope Check

The `updateSubscription` GraphQL mutation requires the `notifications` scope
on the authenticated token. Check it before any API calls:

```bash
# Check for the notifications scope
gh auth status 2>&1 | grep -q 'notifications' \
  && echo "notifications scope present" \
  || { echo "Missing notifications scope. Run:"; echo "  gh auth refresh -s notifications"; exit 1; }
```

If the scope is missing, the script prints the remediation command and exits
without attempting any subscriptions.

## GraphQL Mutation

The `updateSubscription` mutation sets the subscription state for the
authenticated user on a given subscribable node (issue or PR):

```graphql
mutation Subscribe($id: ID!, $state: SubscriptionState!) {
...
}' -F id="$NODE_ID" -f state="$STATE"
```

`$STATE` is `SUBSCRIBED` (default) or `UNSUBSCRIBED` (with `--unsubscribe`).

## Rate Limit Awareness

Bulk subscriptions make sequential GraphQL mutations. GitHub's secondary
rate limits trigger on rapid sequential requests. The script inserts a
2-second delay between each subscription to stay under the threshold.

Already-subscribed items return a success response (no error) — the script
counts them as `skipped` rather than `failed`. Locked issues still accept
...
- Main skill: `config/ai/skills/software-dev/github-engagement/issue-watcher/SKILL.md`
- Script: `scripts/watch_issues.py` — PEP 723 Python script (run via `uv run --script`)
- Instructions: `INSTRUCTIONS.md` — outcome, guardrails, process, definition of done
...
- **issue-sentiment-mapper** — companion skill that identifies which issues are
  worth watching by sentiment analysis. Run it before this skill to prioritize
  which issues to subscribe to.
- **github-issue** — issue creation with cross-referencing. Use this to file a
  new issue, then use issue-watcher to subscribe to related issues on the same
  topic.
