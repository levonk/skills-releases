...
tags: ["ai/skill", "software-development", "github", "sentiment-analysis", "issue-analysis", "community", "read-only", "classification"]
see-also:
  - skill: issue-watcher
    relationship: "complement"
    description: "Companion skill that watches issues for new comments and activity"
  - skill: github-issue
    relationship: "alternative"
    description: "Creates new GitHub issues against upstream repos — use this instead for issue creation"
  - skill: github-pr
    relationship: "alternative"
    description: "Creates pull requests against upstream repos — use this instead for PR creation"
...
dependencies:
  - type: system
    ...
    reason: "Required for GitHub API calls (fetching issue comments via gh api)"
  - type: system
    name: uv
    url: https://docs.astral.sh/uv/
    reason: "Required to run the Python sentiment-mapping script via uv run --script"
triggers:
  - user
allowed-tools:
  - exec
...
Fetch all comments on a GitHub issue or PR, classify each comment's stance
relative to a user-specified position, and output a summary with direct links
to each classified comment. Read-only — no reactions, no comments posted, no
...
|------|----------|-------------|
| `--repo <owner/repo>` | Yes | Target repository in `owner/repo` format |
| `--number <issue-or-pr-number>` | Yes | Issue or PR number to analyze |
| `--stance <keywords>` | Yes | Comma-separated pro-stance keywords (e.g. `"merge,approve,ship it"`) |
| `--anti-stance <keywords>` | Yes | Comma-separated anti-stance keywords (e.g. `"block,reject,concerns"`) |
| `--json` | No | Output structured JSON instead of text summary |

The `--stance` and `--anti-stance` keywords define the classification axes.
Choose keywords that represent the positions in the debate. The classifier
checks for keyword presence in each comment body (case-insensitive, substring
match).

## Classification Logic

Classification is keyword-based — simple, transparent, and reproducible:

| Condition | Classification |
|-----------|---------------|
| Pro keywords match AND anti keywords don't | **PRO** |
| Anti keywords match AND pro keywords don't | **ANTI** |
| Both pro and anti keywords match | **MIXED** |
| Neither pro nor anti keywords match | **NEUTRAL** |

This is intentionally simple. Keyword matching is not NLP sentiment analysis —
it classifies stance relative to a user-specified position, not emotional tone.
A comment that says "I love this idea but it has serious security concerns"
with stance=`love,great` and anti-stance=`concerns,security` would be MIXED.

The transparency is the feature: the user can see exactly which keywords drove
each classification and adjust the keyword lists to refine results.
...
```
Issue Sentiment Summary: owner/repo#123
========================================
Total comments: 47
...
--- PRO (18) ---
1. @alice  2026-08-01  https://github.com/owner/repo/issues/123#issuecomment-1001
   "Strong +1 on this. Let's merge it — the API is clean and the tests pass..."
2. @bob    2026-08-02  https://github.com/owner/repo/issues/123#issuecomment-1002
   "Ship it. This is exactly what we need..."

--- ANTI (9) ---
1. @carol  2026-08-01  https://github.com/owner/repo/issues/123#issuecomment-1003
   "I have concerns about the security implications. We should block this until..."
...
Each comment entry includes: author, date, direct URL (deep link to the
specific comment via `#issuecomment-{id}`), and a 100-character snippet of the
comment body for quick scanning.
...
```json
{
  ...
  "number": 123,
  "total_comments": 47,
  "counts": {"PRO": 18, "ANTI": 9, "MIXED": 5, "NEUTRAL": 15},
  "stance_keywords": ["merge", "approve", "ship it"],
  "anti_stance_keywords": ["block", "reject", "concerns"],
  "comments": [
    {
      "id": 1001,
      "author": "alice",
      ...
      "matched_pro": ["merge"],
      "matched_anti": [],
      "snippet": "Strong +1 on this. Let's merge it — the API is clean and the tests pass..."
    }
  ]
}
```
...
This skill is **strictly read-only**. It does NOT:

- Post reactions (thumbs up, thumbs down, etc.) on comments
- Post new comments or replies
- Edit, delete, or lock any issue or comment
- Modify labels, assignees, or any issue metadata
- Make any authenticated write API call

**Why mass automated reactions were rejected:**

1. **ToS risk** — GitHub's Terms of Service prohibit automated bulk reactions.
   Mass-reacting to comments can trigger automated abuse detection and result
   in account suspension.

2. **Brigading perception** — Even if well-intentioned, a bot that reacts to
   many comments on an issue looks like coordinated brigading to maintainers
   and community members. This damages the reputation of the user and the
   project.

3. **Account risk** — GitHub may flag or suspend accounts that exhibit
   automated reaction patterns. The risk to the user's account outweighs any
   convenience from automating reactions.

4. **Misuse potential** — A reaction-automation feature would be trivially
   repurposed for vote manipulation, harassment, or suppressing dissenting
   opinions. The read-only design prevents this entirely.

**Correct workflow:** Review the sentiment summary output, then manually
react to or reply to specific comments where your human judgment warrants
engagement. The skill gives you the map; you make the moves.
...
### External Resources
- GitHub REST API: https://docs.github.com/en/rest/issues/comments
...
- **issue-watcher** — companion skill that monitors issues for new comments and activity over time
- **github-issue** — creates new GitHub issues against upstream repositories
- **github-pr** — creates pull requests against upstream repositories
