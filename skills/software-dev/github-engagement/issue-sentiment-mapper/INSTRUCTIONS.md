...
Fetch all comments on a GitHub issue/PR, classify each comment's stance
relative to a user-specified position, and output a summary with links for
manual review. Read-only — no mutations.

## Outcome

The user receives a clear, reproducible breakdown of where every commenter
stands on a GitHub issue or PR, with direct links to each classified comment.
The output enables informed manual engagement — no automated reactions, no
mutations.
...
- All comments on the target issue/PR are fetched and classified
- Each comment has a PRO/ANTI/MIXED/NEUTRAL label with the matched keywords shown
- The summary includes counts, a visual bar chart, and per-comment details
  (author, date, URL, snippet)
- Every comment URL deep-links to the specific comment via `#issuecomment-{id}`
- The user can adjust stance keywords and re-run to refine classifications
- No write API calls were made — the skill is strictly read-only

### Bad Outcome

- Comments were fetched but classification keywords were too vague, producing
  mostly NEUTRAL results with no actionable signal
- The script crashed on a locked issue or an issue with no comments, with no
  graceful error message
- The summary lists comment IDs but no URLs, forcing the user to manually
  construct links
- The script made write API calls (reactions, comments) — violating the
  read-only design
- Rate limit was hit mid-fetch with no partial results saved or warning shown

## Guardrails

1. **Read-only — no exceptions.** The script must never call any write API
   endpoint. No reactions, no comments, no edits, no locks, no label changes.
   The `gh api` calls are GET-only (`repos/.../comments`).

2. **No mass reactions.** Mass automated reactions are rejected for ToS risk,
   brigading perception, account risk, and misuse potential. See the Read-Only
   Design section in SKILL.md for the full rationale.

3. **Rate-limit spacing.** Insert 0.5s between paginated reads to avoid
   bursting the API rate budget. Large issues (100+ comments) can consume
   1-5% of the hourly rate limit.

4. **Transparent classification.** The keyword matching logic is simple and
  visible — the user sees exactly which keywords matched for each comment.
   No opaque ML model, no hidden scoring. Reproducible with the same keywords.

5. **Graceful error handling.** Handle: issue not found (404), no comments
   (empty array), locked issues (403), rate limit exceeded (403 with
   rate-limit headers). Never crash without a message.

## Available Resources

| Resource | Purpose |
|----------|---------|
| `scripts/map_sentiment.py.tmpl` | PEP 723 Python script — fetches comments via `gh api`, classifies by keyword, outputs text or JSON summary |

## Current Process

### Step 1: Parse Inputs

Collect and validate the required inputs:

- `--repo <owner/repo>` — must match `owner/repo` format (single slash)
- `--number <issue-or-pr-number>` — must be a positive integer
- `--stance <keywords>` — comma-separated pro-stance keywords
- `--anti-stance <keywords>` — comma-separated anti-stance keywords
- `--json` — optional flag for JSON output

Validate that `gh` is installed and authenticated (`gh auth status`). If `gh`
is missing or not authenticated, exit with a clear error message.

### Step 2: Fetch All Comments

Fetch comments via the GitHub API using `gh`:

```bash
gh api repos/{owner}/{repo}/issues/{number}/comments --paginate
```

The `--paginate` flag follows `Link` headers to fetch all pages. Insert a
0.5-second sleep between paginated reads to respect rate limits (per the
api-rate-limit-awareness guidance inlined above).

The API returns issue comments as JSON. PR comments use the same endpoint
(GitHub treats PRs as issues for comment purposes). Review comments on PRs
(diff-level comments) are NOT included — only the general conversation thread.

Handle these error cases:
- **404**: Issue not found — print "Issue owner/repo#N not found" and exit 1
- **410**: Issue deleted — print "Issue owner/repo#N has been deleted" and exit 1
- **403 (locked)**: Issue is locked — print a warning and exit 1
- **403 (rate limit)**: Rate limit exceeded — print partial results collected
  so far and the reset time, then exit 1
- **Empty array**: No comments — print "No comments found on issue owner/repo#N"
  and exit 0

### Step 3: Classify Each Comment

For each comment, classify its stance using keyword matching:

1. Convert the comment body to lowercase
2. Check which pro-stance keywords appear (substring match)
3. Check which anti-stance keywords appear (substring match)
4. Apply the classification matrix:
   - Pro matches AND no anti matches → **PRO**
   - Anti matches AND no pro matches → **ANTI**
   - Both pro and anti matches → **MIXED**
   - Neither matches → **NEUTRAL**
5. Record the matched keywords for each comment (for transparency)

### Step 4: Aggregate Counts

Count the number of comments in each classification category (PRO, ANTI,
MIXED, NEUTRAL). Calculate percentages of the total.

### Step 5: Output Summary

**Text output (default):**
- Header with repo, issue number, total comment count
- Count summary with visual bar chart (Unicode block characters)
- Per-classification sections listing each comment with:
  - Author (`@username`)
  - Date (ISO 8601)
  - URL (`https://github.com/{repo}/issues/{number}#issuecomment-{id}`)
  - 100-character snippet of the comment body

**JSON output (`--json` flag):**
- Structured object with repo, number, total_comments, counts,
  stance_keywords, anti_stance_keywords, and a comments array
- Each comment object includes id, author, date, url, classification,
  matched_pro, matched_anti, and snippet
...
- [ ] Verify prerequisites — `gh` CLI installed and authenticated (Step 1)
- [ ] Parse and validate inputs — repo, number, stance keywords, anti-stance keywords, json flag (Step 1)
- [ ] Fetch all comments via `gh api repos/{repo}/issues/{number}/comments --paginate` with 0.5s rate-limit spacing (Step 2)
- [ ] Classify each comment as PRO/ANTI/MIXED/NEUTRAL by keyword matching (Step 3)
- [ ] Aggregate counts by stance category (Step 4)
- [ ] Output text summary or JSON with per-comment details (author, date, URL, snippet) (Step 5)

**Mark legend:**
- `[ ]` — task pending (not yet started)
- `[~]` — task in progress (actively being worked)
- `[x]` — task done (verified complete)
- `[!]` — task blocked (cannot the blocker inline)

## Definition of Done

Before declaring the issue-sentiment-mapper run complete, verify every item
...
- [ ] **[script]** `scripts/map_sentiment.py.tmpl` parses `--repo`, `--number`, `--stance`, `--anti-stance` without error (Step 1)
- [ ] **[manual]** The repo is in `owner/repo` format and the number is a positive integer (Step 1)
- [ ] **[manual]** The stance and anti-stance keywords are meaningful for the issue's debate — not so vague that everything is MIXED or NEUTRAL (Step 1)

### Comment Fetching

- [ ] **[script]** `gh api repos/{repo}/issues/{number}/comments --paginate` returns a JSON array (Step 2)
- [ ] **[manual]** Rate-limit spacing (0.5s between paginated reads) was applied — no burst of API calls (Step 2, Rate Limit Awareness)
- [ ] **[manual]** If the issue has 0 comments, the script exits 0 with a "No comments found" message — not a crash (Step 2)
- [ ] **[manual]** If the issue is not found (404) or locked (403), the script exits 1 with a clear error message — not a stack trace (Step 2)

### Classification

- [ ] **[script]** Every fetched comment has a classification of PRO, ANTI, MIXED, or NEUTRAL (Step 3)
- [ ] **[manual]** The classification matrix is correct: PRO = pro-only, ANTI = anti-only, MIXED = both, NEUTRAL = neither (Step 3)
- [ ] **[manual]** Matched keywords are recorded per comment — the user can see which keywords drove each classification (Step 3)

### Output

- [ ] **[script]** Text output includes counts, bar chart, and per-comment entries with author, date, URL, and snippet (Step 5)
- [ ] **[script]** JSON output (when `--json` is passed) is valid JSON with all required fields (Step 5)
- [ ] **[manual]** Every comment URL uses the `#issuecomment-{id}` deep-link format — not just the issue URL (Step 5)
- [ ] **[manual]** Snippets are truncated to 100 characters and do not contain raw newlines (Step 5)

### Read-Only Guarantee

- [ ] **[manual]** No write API calls were made — the script only called GET endpoints via `gh api` (Read-Only Design)
- [ ] **[manual]** No reactions were posted — the skill does not automate reactions under any circumstance (Read-Only Design)
- [ ] **[manual]** No comments were posted or edited — the skill is strictly read-only (Read-Only Design)

### Not Done (common false-completion signals)

If any of these are true, the run is NOT complete:

- The script fetched comments but skipped pagination — only the first 30 comments were classified (Step 2)
- The script crashed on an issue with 0 comments instead of exiting gracefully (Step 2)
- Comments are classified but URLs are missing or point to the issue page instead of the specific comment (Step 5)
- The script posted reactions or comments — the read-only design was violated (Read-Only Design)
- Rate-limit spacing was skipped — the script burst all paginated calls with no delay (Step 2, Rate Limit Awareness)
- Classification keywords were not recorded — the user cannot see which keywords matched (Step 3)

## Context Declaration

### File Paths
- Main skill: `config/ai/skills/software-dev/github-engagement/issue-sentiment-mapper/SKILL.md`
- Script: `scripts/map_sentiment.py.tmpl`

### Variables
- `$REPO` — target repository in `owner/repo` format (e.g. `microsoft/vscode`)
- `$NUMBER` — issue or PR number (e.g. `123`)
- `$STANCE_KEYWORDS` — comma-separated pro-stance keywords
- `$ANTI_STANCE_KEYWORDS` — comma-separated anti-stance keywords

### External Resources
- GitHub REST API (issue comments): https://docs.github.com/en/rest/issues/comments
- `gh` CLI: https://cli.github.com/
- `uv` (script runner): https://docs.astral.sh/uv/
...
- [GitHub REST API — Issue Comments](https://docs.github.com/en/rest/issues/comments) — endpoint documentation for `GET /repos/{owner}/{repo}/issues/{number}/comments`
- [`gh api` documentation](https://cli.github.com/manual/gh_api) — `--paginate` flag follows Link headers
- [PEP 723 — Inline script metadata](https://peps.python.org/pep-0723/) — the script header format for `uv run --script`
