...
see-also:
  ...
  - skill: container-service-deploy
    ...
    description: "Deploys the images this skill builds — compose for dev, Ansible for prod"
  - skill: nixify
    relationship: complement
    description: "Nix flake authoring patterns for Branch C (Nix flake builds)"
---
...
- **Build feasibility check before task creation.** Before creating implementation tasks for a containerized service, verify the build path is feasible on available build hosts. Document the build host, target architectures, and image source (pre-built / Dockerfile / Nix flake) in the task. If no available host can produce the required architectures, block the task until the feasibility issue is resolved.
- **Simplest tool that produces the right artifact wins.** Nix flakes for reproducibility when the runtime needs Nix. Dockerfiles + buildx for portability and multi-arch when it doesn't. Choose based on runtime needs, not build preferences. A Rust binary in a Debian slim image built via `docker buildx --platform` is simpler, smaller, and more portable than a Nix flake container that couples to a Linux build host.
- **Avoid QEMU for heavy compilation.** QEMU/binfmt emulation is 10-24x slower than native and known to segfault on Rust toolchains (rust-lang/rust#147026) and the mold linker (rui314/mold#1550). For Rust, C++, or Go production builds, use cross-compilation toolchains or native build hosts per architecture. QEMU is acceptable only for simple builds, dependency installation, or prototyping.
- **`--push` is required for multi-platform builds.** `docker buildx build --load` only works for a single platform. Multi-platform builds must use `--push` to a registry — the local Docker image store cannot hold a multi-arch manifest.


# Container Image Build

Build container images that run correctly across a mixed-architecture fleet
(x86_64 + aarch64). The decision tree below determines which branch to use.
Each branch has a dedicated reference file with the full workflow.

## Decision Tree

```
1. Does upstream provide a multi-arch image?
   → Check GHCR, Docker Hub, Quay for official/verified images
   → Run: scripts/check-upstream-image.sh <image-name>
   → YES → Branch A (Wrap Pre-Built) — see references/wrap-prebuilt.md
   → NO  → continue

2. Does the service need a Nix store at runtime?
   → Serves nixpkgs to Nix clients? Needs nix CLI inside container?
   → Needs nix-store running inside the container?
   → YES → Branch C (Nix Flake) — see references/nix-flake-build.md
   → NO  → continue

3. Otherwise → Branch B (Dockerfile + buildx)
   → see references/dockerfile-buildx.md
```

**The decision tree is mandatory.** Do not skip to a branch without running
the checks. The most common failure mode is jumping to Branch C (Nix flake)
when Branch A (pre-built) or Branch B (Dockerfile) would have been simpler.

## Steps

1. **Check for pre-built upstream images.** Run
   `scripts/check-upstream-image.sh <image-name>` to search GHCR, Docker Hub,
   and Quay for official multi-arch images. If found and the manifest covers
   all target architectures, go to Branch A.

2. **Determine runtime Nix dependency.** If the service is a binary (Rust, Go,
   C/C++, Python, Node.js) that does not need `nix-store` or `nix` CLI at
   runtime, it does NOT need a Nix flake container. Go to Branch B. If the
   service genuinely needs Nix at runtime (serves nixpkgs, needs nix CLI),
   go to Branch C.

3. **Execute the selected branch.** Follow the reference file for the chosen
   branch:
   - Branch A: `references/wrap-prebuilt.md`
   - Branch B: `references/dockerfile-buildx.md`
   - Branch C: `references/nix-flake-build.md`

4. **Verify multi-arch.** Run `scripts/verify-multi-arch.sh <image:tag>` to
   confirm the built or wrapped image covers all target architectures. See
   `references/multi-arch.md` for manifest management, tagging conventions,
   and verification commands.

5. **Push to registry.** Multi-platform builds require `--push` (the local
   Docker image store cannot hold a multi-arch manifest). Push to the target
   registry and verify with `docker manifest inspect`.

6. **Scan generated files for identity leaks.** Run
   `scripts/scan-artifacts.sh` on the Dockerfile, flake.nix, or CI workflow
   files this skill generated, before committing them. The script resolves
   this machine's `$HOME`, `whoami`, and `hostname` and scans for those
   specific strings. Fix HARD leaks (resolved paths, usernames, hostnames)
   and confirm REVIEW items before committing. If the user confirms the
   files are for private use only, pass `--private` to make HARD findings
...
When upstream provides an official multi-arch image, wrap it: pull, verify
the manifest covers target archs, retag, push to your registry. Do not
rebuild from source. See `references/wrap-prebuilt.md`.

### Branch B — Dockerfile + buildx

When no suitable upstream image exists and the service doesn't need Nix at
runtime, write a multi-stage Dockerfile and build with
`docker buildx build --platform linux/amd64,linux/arm64 --push`. buildx
handles cross-compilation transparently. See `references/dockerfile-buildx.md`.

### Branch C — Nix Flake

When the service genuinely needs a Nix store at runtime, use
`dockerTools.buildLayeredImage` in a flake. Multi-arch requires either
`flake-utils`/Flake Parts with explicit `systems` or `pkgsCross` for
cross-compilation. Nix flake builds are coupled to the build host's system
— document which hosts can build which archs. See
`references/nix-flake-build.md`.

## Key Principles

The rule above (`container-build-principles.md`) is inlined into this skill.
The critical contracts:

- **Check pre-built first** — always, before writing any build definition
- **Multi-arch mandatory** — never single-arch `:latest` for a mixed fleet
- **No Nix unless runtime needs it** — a binary in a slim image is simpler
...
- `scripts/check-upstream-image.sh` — searches registries for pre-built multi-arch images
- `scripts/verify-multi-arch.sh` — verifies an image manifest covers target architectures
- `scripts/scan-artifacts.sh` — scans generated files for identity leaks before upstream commit
