...
see-also:
  ...
  - skill: "container-image-build"
    ...
    description: "Build container images for Java applications"
  - skill: "cicd-upsert"
    relationship: "complement"
    description: "CI/CD pipelines for Java application deployment"
  - skill: "project-adopter"
    relationship: "complement"
    description: "Adopt existing Java projects to standard tooling"
  - skill: "data-pipeline-upsert"
    relationship: "complement"
    description: "Create and update data pipelines that may include Java/Spark components"
---
...
# Java Application Upsert

references/included/knowledge/java-best-practices/
references/included/knowledge/container-best-practices/
references/included/knowledge/devsecops-codeguard/

Create and update Java applications across Maven and Gradle build systems.
Covers project scaffolding, build configuration, testing setup,
containerization, and JVM tuning.

## Decision: Create vs Update vs Convert

Before starting, determine which mode applies:

1. **Check whether the target directory contains a `pom.xml` or
   `build.gradle`/`build.gradle.kts`.**
2. **If no build file exists** → **Mode A: Create a New Java Application**
   (scaffold from scratch).
3. **If a build file exists and the user wants to keep the same build system**
   → **Mode B: Update an Existing Java Application** (audit and improve).
4. **If a build file exists and the user wants to switch build systems**
   → **Mode C: Convert Between Build Systems** (Maven→Gradle or Gradle→Maven).

## Mode A: Create a New Java Application

0. **Research existing projects**: Run the research phase before creating. Skip
...
1. **Determine build system.** Ask the user whether to use Maven or Gradle.
   Default recommendations:
   - **Maven** — convention-over-configuration, best for teams already on
     Maven, stable plugin ecosystem, BOM support.
   - **Gradle (Kotlin DSL)** — faster incremental builds, version catalogs,
     configuration cache, best for multi-project or performance-sensitive
     builds.

2. **Run CLI tool discovery.** Before assuming `mvn` or `gradle` are on PATH,
   run `scripts/cli-tool-discovery.sh` to detect devbox/mise/flox/nix wrappers:
   ```bash
   ./scripts/cli-tool-discovery.sh mvn
   ./scripts/cli-tool-discovery.sh gradle
   ```

3. **Scaffold the project.** Follow the reference for the chosen build system:
   - Maven → `references/maven-project-setup.md`
   - Gradle → `references/gradle-project-setup.md`

4. **Set up testing.** Configure JUnit 5, Mockito, AssertJ, and Testcontainers.
   Split unit tests (surefire) from integration tests (failsafe). See
   `references/java-testing.md`.

5. **Configure containerization.** If the app will be deployed in containers,
   set up a multi-stage Dockerfile with JRE-only runtime, JVM container flags,
   and (for Spring Boot) layered jars. See `references/java-containerization.md`.

6. **Tune the JVM.** Set heap sizing, GC selection, and container-aware flags.
   See `references/jvm-tuning.md`.

7. **Scan generated files for identity leaks.** Run `scan-artifacts.sh` on the
   `pom.xml`, `build.gradle.kts`, `Dockerfile`, and CI workflow files this
   skill generated, before committing them.

## Mode B: Update an Existing Java Application

1. **Audit the existing build.** Read `pom.xml` or `build.gradle.kts` and
   identify:
   - Outdated dependencies or plugin versions
   - Missing BOM / version catalog for dependency alignment
   - Missing reproducible-build settings
   - Test configuration gaps (no surefire/failsafe split, no Testcontainers)
   - Containerization gaps (JDK in runtime image, no container flags)

2. **Run CLI tool discovery.** Use `scripts/cli-tool-discovery.sh` to find the
   build tool wrapper.

3. **Apply improvements** following the relevant references:
   - Build config → `references/maven-project-setup.md` or
     `references/gradle-project-setup.md`
   - Testing → `references/java-testing.md`
   - Containerization → `references/java-containerization.md`
   - JVM tuning → `references/jvm-tuning.md`

4. **Verify the build still passes.** Run the build and test suite after each
   change batch.

5. **Scan modified files for identity leaks** before committing.

## Mode C: Convert Between Build Systems

1. **Inventory the source build.** Read the existing `pom.xml` (for Maven→Gradle)
   or `build.gradle.kts` (for Gradle→Maven) and catalog:
   - All dependencies (group:artifact:version:scope)
   - Plugins and their configurations
   - Profiles / build variants
   - Resource filtering rules
   - Test configuration

2. **Generate the target build file.** Map each dependency, plugin, and
   configuration to the equivalent in the target build system. Follow the
   target reference:
   - Target Gradle → `references/gradle-project-setup.md`
   - Target Maven → `references/maven-project-setup.md`

3. **Verify dependency parity.** Ensure all dependencies, plugins, and
   profiles from the source are represented in the target.

4. **Run the build and tests.** Confirm the converted project compiles and
   all tests pass.

5. **Remove the old build files.** Use `git mv` semantics — remove the old
   `pom.xml` or `build.gradle.kts` and commit the new build file in the same
   commit to preserve history.

6. **Scan for identity leaks** before committing.

## Key Principles

- **Convention over configuration** — prefer standard directory layouts
  (`src/main/java`, `src/test/java`) in both Maven and Gradle.
- **Reproducible builds** — pin plugin versions, use BOMs / version catalogs,
  and enable reproducible-build flags.
- **JRE-only runtime** — never ship a JDK in a production container image;
  use multi-stage builds to separate build and runtime layers.
- **Container-aware JVM** — always set `-XX:+UseContainerSupport` (default on
  JDK 11+) and `-XX:MaxRAMPercentage` instead of hardcoded `-Xmx`.
- **Test split** — unit tests via surefire, integration tests via failsafe
  (Maven) or a separate Gradle source set / Test task.
- **Cache volumes** — use `java-m2-cache` for Maven (`.m2`) and
  `gradle-cache` for Gradle (`.gradle`) in CI and dev containers.
...
### File Paths
- Main skill: `src/current/skills/software-dev/java-app-upsert/SKILL.md` (in the `skills-src` repo at `~/p/gh/levonk/skills-src/`)
- References: `src/current/skills/software-dev/java-app-upsert/references/` (including `maven-project-setup.md`, `gradle-project-setup.md`, `java-testing.md`, `java-containerization.md`, `jvm-tuning.md`)
- Scripts: `src/current/skills/software-dev/java-app-upsert/scripts/` (including `cli-tool-discovery.sh.tmpl`)
- Includes: `src/current/includes/` (shared includes wired into this skill)

### Related Knowledge Bundles
- [java-best-practices](references/included/knowledge/java-best-practices/overview.md) —
  Java/JVM best practices for containers, build tools, testing, and security.
- [container-best-practices](references/included/knowledge/container-best-practices/overview.md) —
  multi-stage builds, base image selection, and runtime hardening for Java apps.
- [devsecops-codeguard](references/included/knowledge/devsecops-codeguard/overview.md) —
  security rules for dependency scanning, crypto, and container hardening.
...
- OpenJDK container images: https://hub.docker.com/_/openjdk
- Eclipse Temurin images: https://hub.docker.com/_/eclipse-temurin
- JFR/JMC documentation: https://docs.oracle.com/en/java/javase/17/jfapi/overview.html
