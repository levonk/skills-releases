...
AUTO_FIX=false
INCREMENTAL=false
# shellcheck disable=SC2034 # Tracked for potential future use
FAILED_PHASES=0
TOTAL_ISSUES=0
...
main() {
	...
	while [[ $# -gt 0 ]]; do
		...
		--debug)
			...
			;;
		--ci)
			# shellcheck disable=SC2034 # Flag set by CLI, reserved for future use
			CI_MODE=true
			shift
			;;
		--format | --format=* | -f=* | --json)
			parse_format_flag "$1" "${2:-}" REPORT_FORMAT
			shift "$SHIFT_COUNT"
			;;
		--fix)
			AUTO_FIX=true
