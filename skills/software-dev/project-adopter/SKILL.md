...
## Submodule-Integrity Pre-Commit Hook

As part of the adoption flow (step 18a), `scripts/install-pre-commit-hooks.sh`
installs a versioned pre-commit hook at `scripts/hooks/pre-commit` in the
target project and sets `git config core.hooksPath scripts/hooks`. The hook
...
parent repo's index. It reads ALL submodule paths from `.gitmodules` (no
hardcoded paths) and skips silently at runtime if `.gitmodules` doesn't
exist or is empty. The installer is idempotent — safe to re-run.
