...
#### Pre-Commit Hook Installation (Submodule-Integrity Protection)

After git initialization (and before the final adoption commit), run
`scripts/install-pre-commit-hooks.sh` to install a versioned pre-commit hook
that protects against the submodule-as-tree class of bug:

```bash
# Install the submodule-integrity pre-commit hook (idempotent)
./scripts/install-pre-commit-hooks.sh /path/to/target
# → creates scripts/hooks/pre-commit
# → sets git config core.hooksPath scripts/hooks
```

The hook reads ALL submodule paths from `.gitmodules` (no hardcoded paths),
writes the staged index to a tree via `git write-tree`, then checks each
submodule path's mode via `git ls-tree`:
...
The hook skips silently at runtime if `.gitmodules` doesn't exist or is empty.
The installer is idempotent — safe to re-run; it overwrites the hook file and
re-sets `core.hooksPath`.
