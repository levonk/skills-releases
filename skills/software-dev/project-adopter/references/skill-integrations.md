...
#### Pre-Commit Hook Installation (Worktree Isolation + Submodule Integrity)

After git initialization (and before the final adoption commit), run
`scripts/install-pre-commit-hooks.sh` to install versioned pre-commit hooks
that enforce worktree isolation and protect against the submodule-as-tree
class of bug:

```bash
# Install pre-commit hooks (idempotent)
./scripts/install-pre-commit-hooks.sh /path/to/target
# → creates scripts/hooks/pre-commit (composite entry point)
# → creates scripts/hooks/pre-commit-worktree-isolation.sh
# → creates scripts/hooks/pre-commit-submodule-integrity.sh
# → sets git config core.hooksPath scripts/hooks
# → scans .gitmodules for file:// URLs and prints advisory if found
```

**Worktree isolation hook** — blocks commits to `main`/`master` (or other
protected branches) when not working inside a linked git worktree. This
enforces the worktree-per-feature workflow that execute-upsert and
git-repository-management expect. Bypass: `SKILL_ALLOW_MAIN_WRITE=1` env
var, or `.agents/config/script-guards.toml` with
`[worktree-isolation] allow_main_write=true`.

**Submodule integrity hook** — reads ALL submodule paths from `.gitmodules`
(no hardcoded paths), writes the staged index to a tree via `git write-tree`,
then checks each submodule path's mode via `git ls-tree`:
- `160000 commit` → OK (correct gitlink)
- `040000 tree` → VIOLATION (submodule converted to regular directory)
- leaked files under a submodule path → VIOLATION

The submodule hook skips silently at runtime if `.gitmodules` doesn't exist
or is empty. The installer is idempotent — safe to re-run; it overwrites the
hook files and re-sets `core.hooksPath`.

**file:// submodule URL advisory** — after installing the hooks, the
installer scans `.gitmodules` for `file://` submodule URLs. If found, it
prints a warning explaining that Git 2.38.1+ defaults
`protocol.file.allow` to `user` (CVE-2022-39253), and environments that set
it to `never` globally will cause `git submodule update` to fail with
`fatal: transport 'file' not allowed`. The warning includes the one-liner
fix (`git config protocol.file.allow always`) and explicitly advises
against setting it globally. The installer detects and informs only — it
never auto-applies the override, because auto-applying would defeat the
deny-by-default security stance. If the advisory fires, surface it to the
user and let them decide whether the repo is trusted enough for the opt-in.
...
- **Reference**: [Shared Quality Scripts ADR](https://github.com/levonk/levonk-base-boilerplate/blob/main/internal-docs/adr/adr-20251218002-shared-quality-scripts.md)
...
- **Reference**: [Vitest for Testing ADR](https://github.com/levonk/levonk-base-boilerplate/blob/main/internal-docs/adr/adr-20251106002-vitest-for-testing.md)
...
- **Reference**: [GitHub Actions CI/CD ADR](https://github.com/levonk/levonk-base-boilerplate/blob/main/internal-docs/adr/adr-20251106014-cicd-strategy.md)
