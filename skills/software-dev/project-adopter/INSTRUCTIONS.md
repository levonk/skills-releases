...
references/included/skills/ai/agent-file-upsert/

references/included/skills/ai/readme-upsert/
...
19a. **Install pre-commit hooks** - Run `scripts/install-pre-commit-hooks.sh {REPO_ROOT}` to install the submodule-integrity pre-commit hook. This creates `scripts/hooks/pre-commit` in the target project (a versioned, committed hook that detects the class of bug where a git submodule is accidentally converted from a gitlink `160000 commit` to a regular tree `040000 tree`) and sets `git config core.hooksPath scripts/hooks`. The hook reads ALL submodule paths from `.gitmodules` (no hardcoded paths) and skips silently at runtime if `.gitmodules` doesn't exist or is empty. The script is idempotent — safe to re-run; it overwrites the hook file and re-sets `core.hooksPath`. Run this AFTER git initialization (step 19) so `core.hooksPath` gets set, and BEFORE the final commit so the hook file itself is included in the adoption changeset. If the repo isn't initialized yet when the script runs, it places the hook file and advises re-running after `git init`. The script also scans `.gitmodules` for `file://` submodule URLs and prints an advisory warning if found — Git 2.38.1+ defaults `protocol.file.allow` to `user` (CVE-2022-39253), and many environments set it to `never` globally; repos with `file://` submodules will hit `fatal: transport 'file' not allowed` until a per-repo override (`git config protocol.file.allow always`) is applied. The script detects and informs only — it never auto-applies the override, because auto-applying would defeat the deny-by-default security stance. If the advisory fires, surface it to the user and let them decide whether the repo is trusted enough for the opt-in.
...
|---------|--------------|-----|
| AGENTS.md creation or update (AI-facing entry point) | **agent-file-upsert** | Owns the AGENTS.md templates (`references/AGENT-project-root-template.md.tmpl`, `AGENT-project-developer-template.md.tmpl`, `AGENT-project-subfolder-template.md.tmpl`), the scaffolder script (`scripts/init-agents-md.py`), and the progressive-disclosure structure (root AGENTS.md as user-facing index → `.agents/knowledge/developer.md` as developer guide → sub-folder AGENTS.md files). Hand-writing AGENTS.md via heredocs in `adopt-project.sh` / `configure-*.sh` produces a flat single-audience doc that mixes user and developer content, has no JIT Index, no Knowledge Bundles section, no Developer Guide link, and no Out of Scope/Improvements/Anti-Patterns references. The two copies diverge over time. agent-file-upsert handles both greenfield (scaffold from template) and brownfield (delta analysis + preserve accurate sections) cases. |
...
```bash
# 1. Generate or update AGENTS.md (agent-file-upsert) — progressive disclosure
#    Read references/included/skills/ai/agent-file-upsert/SKILL.md and follow its workflow.
#    It will call scripts/init-agents-md.py itself — do NOT call it directly from here.

# 2. Generate or update README.md (readme-upsert) — run AFTER AGENTS.md exists
#    Read references/included/skills/ai/readme-upsert/SKILL.md and follow its workflow.
#    It will call scripts/verify_consistency.py itself — do NOT call it directly from here.
...
- [ ] Run repository health review to assess current state (Step 1)
...
- [ ] Install pre-commit hooks via `scripts/install-pre-commit-hooks.sh` — submodule-integrity protection, idempotent, sets `core.hooksPath=scripts/hooks`, scans for file:// submodule URLs and prints advisory if found (Step 19a)
...
- [ ] **[script]** `scripts/install-pre-commit-hooks.sh {REPO_ROOT}` ran and created `scripts/hooks/pre-commit` + set `core.hooksPath=scripts/hooks` — the hook is idempotent and skips silently if no `.gitmodules`; if `.gitmodules` has `file://` URLs, the script printed an advisory (Step 19a)
...
- `install-pre-commit-hooks.sh` printed a `file://` advisory but it was silently ignored → the user was not informed that `git submodule update` will fail under a deny-by-default `protocol.file.allow` stance (Step 19a)
