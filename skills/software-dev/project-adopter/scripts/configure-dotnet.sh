...
create_dotnet_project() {
	...
	if [[ "$mode" == "standardize" ]] && ! ls "$project_path"/*.csproj >/dev/null 2>&1; then
		local project_name
...
EOF
	log_info "OK: Created $project_name.csproj"
}
...
EOF
		log_info "OK: Created Directory.Build.props"
	fi
}
...
EOF
			log_info "OK: Created Tests/Tests.csproj"
		fi
...
EOF
			log_info "OK: Created Tests/UnitTests.cs"
		fi
	fi
}
...
app.Run();
EOF
			log_info "OK: Created Program.cs"
		fi
...
EOF
			log_info "OK: Created Controllers/HealthController.cs"
		fi
...
EOF
			log_info "OK: Created appsettings.json"
		fi
...
EOF
			log_info "OK: Created appsettings.Development.json"
		fi
	fi
}
...
EOF
			log_info "OK: Created App.xaml.cs"
		fi
...
EOF
			log_info "OK: Created MainPage.xaml"
		fi
...
EOF
			log_info "OK: Created MainPage.xaml.cs"
		fi
	fi
}
