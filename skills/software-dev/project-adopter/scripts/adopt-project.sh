...
EOF
    ...
    if [[ ! -f "$PROJECT_PATH/README.md" ]] || [[ ! -s "$PROJECT_PATH/README.md" ]]; then
        log_info "Creating comprehensive README.md..."
        # Detect project characteristics for README generation
        local detection_script="../../project-detection/scripts/detect-build-systems.sh"
        local detected_characteristics=""
        if [[ -f "$detection_script" ]]; then
            detected_characteristics=$("$detection_script" -t characteristics "$PROJECT_PATH" 2>/dev/null || echo "")
        ...
        parse_project_characteristics "$detected_characteristics"
        generate_readme_md "$PROJECT_PATH" "$detected_characteristics"
    else
...
e2e:
    npx playwright test || echo "Playwright not configured"
...
generate_readme_md() {
    ...
    log_info "Generating README.md for project: $project_path"
    ...
    if echo "$detected_systems" | grep -q "pnpm\|npm\|yarn\|bun"; then
        ...
        language="TypeScript"
        if echo "$detected_systems" | grep -q "pnpm"; then
            package_manager="pnpm"
        elif echo "$detected_systems" | grep -q "yarn"; then
            package_manager="yarn"
        elif echo "$detected_systems" | grep -q "bun"; then
            package_manager="bun"
        else
            package_manager="npm"
        fi
        ...
        project_type="Python"
        language="Python"
        if echo "$detected_systems" | grep -q "poetry"; then
        ...
        project_type="Java"
        language="Java"
        if echo "$detected_systems" | grep -q "gradle"; then
...
A $project_type project configured with Devbox and Just for consistent development environments.
...
\`\`\`bash
# List all available devbox scripts
devbox --list

# Run specific devbox scripts
devbox bootstrap    # Bootstrap the project
devbox build        # Build the project
devbox test         # Run tests
devbox dev          # Start development server
devbox lint         # Run linting
devbox typecheck    # Run type checking
devbox clean        # Clean build artifacts
\`\`\`
...
just bootstrap     # Bootstrap the project

# Development loop targets
just loop          # Run bootstrap → build → test → dev
just ci             # Run bootstrap → lint → typecheck → test → build
\`\`\`
...
\`\`\`
$project_name/
├── devbox.json              # Devbox configuration and packages
├── justfile                 # Build commands and targets
├── .envrc                   # Direnv configuration (auto-generated)
├── README.md                # This file
EOF
...
└── .devbox/                 # Devbox nix store (auto-generated)
\`\`\`
...
This project uses Devbox to provide a consistent development environment with:
...
The project includes \`.envrc\` for direnv integration. When you \`cd\` into the project:

1. direnv will automatically load the Devbox environment
2. Devbox will run \`just bootstrap-internal\` to prepare the project
3. All \`just\` and \`devbox\` commands will be available
...
3. Make changes and run tests: \`just ci\`
...
EOF
    ...
    log_info "✓ README.md created with project setup instructions and devbox commands"
}
