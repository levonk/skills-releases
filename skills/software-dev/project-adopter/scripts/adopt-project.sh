...
# Set project path (deferred to adopt_project() after arg parsing)
PROJECT_PATH=""
PROJECT_NAME=""
...
load_project_detection() {
	...
	if [[ -f "$detection_path/detect-build-systems.sh" ]]; then
		...
		source "$detection_path/analyze-workspace-configs.sh"
...
load_surgical_config() {
	...
	if [[ -f "$surgical_path/surgical-edit.sh" ]]; then
		# shellcheck disable=SC2034 # Exported for use by other functions
...
EOF
	...
	*)
	...
	if [[ ! -f "$PROJECT_PATH/AGENTS.md" ]]; then
		log_info "→ NEXT STEP: Run agent-file-upsert to create AGENTS.md"
		log_info "  Read: references/included/skills/ai/agent-file-upsert/SKILL.md"
		log_info "  ⚠ post-adoption-check.sh will FAIL until this is done"
	else
		log_info "→ NEXT STEP: Run agent-file-upsert to update AGENTS.md via delta analysis"
		log_info "  Read: references/included/skills/ai/agent-file-upsert/SKILL.md"
	fi
	...
	if [[ ! -f "$PROJECT_PATH/README.md" ]] || [[ ! -s "$PROJECT_PATH/README.md" ]]; then
		log_info "→ NEXT STEP: Run readme-upsert to create README.md (run AFTER agent-file-upsert)"
		log_info "  Read: references/included/skills/ai/readme-upsert/SKILL.md"
		log_info "  ⚠ post-adoption-check.sh will FAIL until this is done"
	else
		log_info "→ NEXT STEP: Run readme-upsert to update README.md"
		log_info "  Read: references/included/skills/ai/readme-upsert/SKILL.md"
	fi

	# Knowledge bundle installation is delegated to the AI agent.
	# See SKILL.md step 14 and the "Knowledge Bundle Installation" section.
	log_info "→ NEXT STEP: Install knowledge bundles"
	log_info "  Run: uv run --script scripts/install-knowledge-bundles.py ."
	log_info "  ⚠ post-adoption-check.sh will FAIL until this is done"
}
...
adopt_project() {
	...
	# Set project path and name from remaining positional args
	PROJECT_PATH="${1:-.}"
	...
	log_info ""
	log_info "⚠️  DELEGATED STEPS — the AI agent MUST run these before post-adoption-check:"
	log_info "  A. Read references/included/skills/ai/agent-file-upsert/SKILL.md and follow its workflow to create/update AGENTS.md (SKILL.md step 10)"
	log_info "  B. Read references/included/skills/ai/readme-upsert/SKILL.md and follow its workflow to create/update README.md (SKILL.md step 13) — run AFTER step A"
	log_info "  C. Run 'uv run --script scripts/install-knowledge-bundles.py .' with stack-matched bundles (SKILL.md step 14)"
	log_info "  D. Run scripts/post-adoption-check.sh to verify all steps completed"
	log_info ""
	log_info "Next steps (after delegated steps):"
