...
devbox.json          # Devbox with nodejs_24, pnpm, typescript
...
**Step 3: Ignore File Generation (ignorefile-manager skill)**
```bash
# Delegated to ignorefile-manager — do NOT hand-write .gitignore
...
# Generates: .gitignore, .dockerignore, .codeiumignore, .cursorignore,
# .aiexclude, .npmignore, VS Code excludes, ripgrep config
```

**Step 4: README Generation (readme-upsert skill)** — runs AFTER AGENTS.md is in place
```bash
# Delegated to readme-upsert — do NOT hand-write README content
# Greenfield: creates from references/README-project-root-template.md.tmpl
# Brownfield: preserves accurate sections, updates stale ones
# Then runs scripts/verify_consistency.py to check README<->AGENTS.md agreement
# See <readme-upsert>/SKILL.md for the full 5-phase workflow.
```

**Step 5: Repository Init + Adoption Commit (git-repository-management skill)**
```bash
# Delegated to git-repository-management — do NOT call git init/add/commit directly
./<git-repository-management>/scripts/git-collect.sh .                         # emits NOT_A_GIT_REPO if not a repo
...
**Step 6: Development Loop (ai-development-loop skill)**
```bash
...
devbox.json      # Devbox with nodejs_24, pnpm, typescript
justfile         # Standard targets
AGENTS.md        # AI workflow guide (created before README so readme-upsert can link to it)
README.md        # Generated by readme-upsert (from template, greenfield)
LICENSE.md       # Proprietary license
...
.npmignore       # Generated by ignorefile-manager
docker-compose.yml
docker/
└── Dockerfile
.github/
...
# Initialize devbox
devbox init
devbox add just nodejs_24 pnpm typescript
...
RUN nix profile install nixpkgs#devbox nixpkgs#nodejs_24 nixpkgs#pnpm
