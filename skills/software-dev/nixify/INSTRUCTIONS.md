...
flowchart TD
    ...
    R5 -- "C/C++ (CMake)" --> T_CMAKE["source-build/cmake.md"]
    R5 -- "Other" --> T_OTHER["source-build/&lt;lang&gt;.md"]

    S11B --> R6{"nixpkgs superset<br/>+ prebuilt binaries<br/>+ version lags?"}
    R6 -- "Yes" --> T_OVR["nixpkgs-override-attrs.md<br/>(overrideAttrs + per-platform hashes)"]
    R6 -- "No" --> R5
...
12. **Generate flake.nix**: Choose the appropriate template from `references/flake-templates/` based on Step 4 results and the derivation analysis from Step 11. **One build approach per file** — when a language has multiple build approaches (e.g. Node.js `buildNpmPackage` vs `stdenv.mkDerivation`+`fetchNpmDeps`, PHP `buildComposerPackage` vs raw `stdenv.mkDerivation`, Ruby `bundlerEnv` vs `buildRubyGem`), each approach gets its own file (`source-build-<lang>.md` for the common case, `source-build-<lang>-<variant>.md` for alternatives). Do NOT add a second template code block to an existing file — create a new file and add a routing line below. This keeps each file focused on one approach so the agent loads only the relevant context, and follows the one-template-per-file rule from the skill-upsert audit checklist. **In all variants, scope `allSystems`, `assets`, and `meta.platforms` to `target_platforms` from Step 4a** — when `platform_scope` is `darwin_only` or `linux_only`, only include systems from `target_platforms`; do not add the excluded family even if release assets exist for it:
    ...
    - No releases, or `force_source_build=true` -> Source Build Flake by language:
      ...
      - C/C++ (CMake) -> `references/flake-templates/source-build/cmake.md`. Uses `stdenv.mkDerivation` with `cmake` + `ninja` in `nativeBuildInputs`. Derives `version` from the flake's git revision (`unstable-${self.shortRev or "dirty"}`) instead of hardcoding a stale number — C/C++ projects have no language manifest (Cargo.toml, package.json) to read the version from, and CMake projects commonly use `git describe` at build time. See `references/advanced/version-from-git.md` for the rationale and the `self.shortRev` / `self.dirtyRev` / `self.lastModifiedDate` flake attributes.
    ...
    - Project in nixpkgs with complex fixup logic AND prebuilt release binaries AND (nixpkgs version lags upstream OR maintainer wants faster cadence) -> `references/flake-templates/nixpkgs-override-attrs.md` — **store `flake_type=nixpkgs_override_attrs`**. Use when Step 10's `check-nixpkgs.sh` reported `project_in_nixpkgs: true`, Step 11's `inspect-nixpkgs-derivation.sh` confirmed complex fixup logic (patchelf, wrapGApps, desktop-file-fixup, icon symlinks, build patches), Step 11b's `check-nixpkgs-superset.sh` reported `is_superset: true`, AND Step 4b's `check-releases.sh` confirmed prebuilt release binaries exist. This template reuses nixpkgs' packaging via `overrideAttrs` (swapping in upstream version + per-platform SRI hashes) so the repo only maintains `version` + 4 hashes while nixpkgs maintains the ~150 lines of fixup logic. This is the pattern for browsers, Electron apps, and other projects where nixpkgs has mature packaging but lags upstream releases by days or weeks. **Set `version` in flake.nix to the latest release version from Step 4b's `check-releases.sh` output**. The hash automation workflow (Step 16) targets the `hashes` attrset shape (not the `assets` attrset from `prebuilt-tarball.md`) — see `references/advanced/hash-automation.md`.
    ...
    **MANDATORY for source-build flakes — x86\_64-darwin legacy pin**: All source-build templates include a `nixpkgs-darwin-legacy` input pinned to `nixpkgs-26.05-darwin` and a conditional `pkgs` selection that uses it for `x86_64-darwin`. This ensures builds work on Intel Macs — nixpkgs-unstable dropped `x86_64-darwin` in 26.11, so the legacy pin is the only path to a current-version install for Intel Mac users. Do NOT remove the legacy pin when filling in a template. See `references/flake-templates/darwin-legacy-pin.md` for the rationale, branch selection guidance, and EOL timeline. Skip only if the project explicitly targets `aarch64-darwin` only.
    ...
    **Store the `flake_type` value — it determines documentation content at Step 15, advanced features at Step 16, and PR body at Step 27.** Source Build and Prebuilt Tarball flakes have fundamentally different properties: Source Build flakes exist at every git tag (tag-pinning works), while Prebuilt Tarball flakes are bumped *after* the release tag is cut (tag-pinning does NOT work). The `nixpkgs_override_attrs` type shares the Prebuilt Tarball property (bumped after release, tag-pinning does NOT work) since it wraps prebuilt binaries via nixpkgs' packaging. Mixing these up produces broken install instructions.

    **Package extraction (nixpkgs upstreaming candidates)**: When `flake_type=source_build` AND `project_in_nixpkgs=false` (Step 10) AND the flake has a single build definition (no `#prebuilt`/`#source` split, no `hybrid_fallback`), extract the package definition from `flake.nix` into a standalone `package.nix` (callPackage-style) + `default.nix` (non-flake entry point). This makes the flake's system-specific wiring clearer, enables non-flake `nix-build`/`callPackage` workflows, and produces a `package.nix` already in the shape nixpkgs expects for Step 28b — the nixpkgs PR can reuse the exact same file with only `src` and `version` changed. See `references/flake-templates/package-extraction.md` for the three-file structure, release-please marker placement, and workflow updates. **Store `package_extraction=true`** — it is consumed at Step 16 (workflow path filters target `package.nix` instead of `flake.nix` for hash automation), Step 15 (documentation mentions `nix-build default.nix`), and Step 28b (nixpkgs PR reuses `package.nix`). When `flake_type=prebuilt_tarball`, `nixpkgs_wrapper`, `nixpkgs_override_attrs`, or `hybrid_fallback=true`, do NOT extract — the inline structure is load-bearing for the hash automation or multi-output logic.
...
16. **Add advanced features**: See `references/advanced-features.md`. The first two items are required; the rest are optional:
    ...
    - Devenv vs mkShell — when to use devenv (managed services, devcontainer parity) vs plain `mkShell` (build tools only)
    - Version from Git Revision — derive `version` from `self.shortRev` for projects building from `src = self` with no language manifest (C/C++, CMake projects); use `unstable-${self.shortRev or "dirty"}` instead of a hardcoded stale version
...
16b. **Test the generated flake on BOTH ubuntu AND darwin (REQUIRED before pushing)**: Run `scripts/test-with-act.sh <project-dir>`. By default this runs **both** validations sequentially — both must pass:
    - **Action pin validation** (fast first check): Run `scripts/validate-action-pins.sh <project-dir>` to verify every `uses:` line in `.github/workflows/*.yml` is pinned to a 40-char commit SHA — not a mutable `@vN` or `@main` ref. This catches truncated SHAs (e.g. a 39-char SHA from a partial API response) and unresolved placeholders (`<checkout-sha>`) in milliseconds, before the heavier zizmor/actionlint checks. If it exits non-zero, fix the pins (re-run `scripts/resolve-action-shas.sh` from Step 16a) before proceeding — a bad pin will fail CI and zizmor will flag it as unpinned. This is the deterministic catch for the pnpm PR #14255 class of bug where a 39-char SHA broke CI.
    ...
    - **zizmor + actionlint (MANDATORY)**: Run `nix run nixpkgs#zizmor -- .github/workflows/nix.yml .github/workflows/nix-release.yml` and `nix run nixpkgs#actionlint -- .github/workflows/nix.yml .github/workflows/nix-release.yml` on the generated workflows. Both are available via `nix run nixpkgs#<tool>` (no installation needed). **zizmor** catches security issues (unpinned actions, excessive permissions, OIDC token misuse) that the pnpm repo's CI enforces — a zizmor failure on a generated workflow will get the PR blocked with CHANGES_REQUESTED. **actionlint** catches syntax/schema errors in the workflow YAML before they reach GitHub CI. Both must pass with zero findings before pushing. If zizmor reports an unpinned action, re-run `scripts/resolve-action-shas.sh` (Step 16a) and replace the mutable ref with the resolved SHA. If actionlint reports a schema error, fix the workflow template. Do NOT proceed to Step 17 with zizmor or actionlint findings.
...
22b. **Pre-push validation gate (MANDATORY before pushing)**: Run `scripts/validate-pre-push.sh <owner>/<repo> <project-dir> --base-ref <base-ref> --verbose`. This is the deterministic PRE-PUSH gate that catches the six classes of bug that shipped broken on acryl PR #5 — all six were cases where the skill's prose guidance was correct but the agent skipped the validation steps. This script makes the checks enforceable:
    ...
    - **magic-nix-cache-action darwin os guard**: flags `magic-nix-cache-action` when used without an `if: runner.os == 'Linux'` guard (or equivalent os condition). The v14 static binary for `arm64-darwin` fails on the `macos-14` runner with `dyld: Symbol not found` — the binary was built against a newer `libc++` than the runner ships. The job hangs ~20 min then cancels. Gate the action to Linux-only; darwin builds work without the cache, just slower. This was the fourth class of bug shipped on acryl PR #5 (commit `82586e0`). See `references/advanced/github-actions-nix.md`.
    ...
    - **deprecated runner labels**: flags known-deprecated GitHub Actions runner labels (`macos-12`, `macos-13`, `macos-14`, `macos-15`) in `runs-on:` or matrix config. ALWAYS USE THE NEWEST RUNNER — current newest: `macos-26` (ARM), `macos-26-intel` (Intel), `ubuntu-latest` (Linux). Never use `macos-latest` in a matrix — use the explicit label so the runner version is pinned and auditable. See `references/advanced/github-actions-nix.md`.
...
25. **Update changelog (if applicable)**: If CHANGELOG.md exists, add entry under `## Unreleased` -> `### Added`. See `references/changelog-entry.md`. **Changesets must describe user-visible capability ONLY** — no output names (`#default`, `#source`, `#prebuilt`, `#nixpkgs`), no implementation details (build approach, toolchain, crane, fetchurl), no CI/workflow behavior (scheduled updates, hash automation). Just what the user can now do. Example: "Add Nix flake support for installing pnpm from prebuilt releases, source, or nixpkgs." NOT: "Add Nix flake support with prebuilt binary outputs for Linux and macOS. The flake exposes #default (prebuilt standalone binary), #source (from-source build of the Rust CLI), #prebuilt, and #nixpkgs..." CodeRabbit flagged the latter on pnpm PR #14255 for leaking implementation details into a changeset.
...
28c. **Respond to maintainer review feedback (simplification)**: When a maintainer reviews the opened PR and requests simplification — typically citing "overbuilt", "too complex", "maintenance liability", or asking to remove prebuilt outputs / hash automation / extra outputs — follow this pattern:

    1. **Read the review carefully and identify each specific request.** Common simplification requests:
       - "Drop the prebuilt binary path, just use source build"
       - "Remove the hash automation workflow, it's dead code for a source-build flake"
       - "Use one derivation, not a prebuilt+source split"
       - "Read the version from the manifest instead of hardcoding"
       - "Remove dead code" (e.g. leftover logic from a previous iteration)
       - "Simplify the CI matrix"
    2. **Set `force_source_build=true` retroactively** and update `flake_type` to `source_build` if the reviewer asked to drop prebuilt outputs. This affects the PR body template selection — switch from `pr-prebuilt-tarball.md` to `pr-source-build.md` and update the PR description accordingly.
    3. **Strip the requested outputs from `flake.nix`**: Remove `#prebuilt`, the `assets` attrset, `fetchurl` derivations, and the `#source` alias (the source build becomes `#default`). Remove the hash automation workflow (`.github/workflows/nix-release.yml` or similar) entirely — it's dead code for a source-build flake.
    4. **Simplify the CI workflow**: Remove the `#source` build step and its PR guard if the reviewer flagged it. Reduce the matrix if requested, but **do NOT remove `x86_64-darwin` / Intel macOS coverage** — that's a user-facing requirement, not an implementation detail the reviewer can override. If the reviewer asks to remove Intel Mac support, push back in the PR comment explaining why (older Intel Macs depend on it, nixpkgs-unstable no longer builds for x86_64-darwin after 26.11).
    5. **Re-push to the same branch** (do NOT open a new PR — add a commit to the existing branch). The reviewer sees the update in the same PR thread.
    6. **Post a PR comment** summarizing what was changed in response to each review point. Use `gh pr comment <PR-NUMBER> --body-file <file>` (never `--body` for multiline). Address each point individually — don't batch multiple reviewer requests into a vague "addressed feedback" comment.

    **Do NOT** re-open a new PR, re-create the orientation issue, or rebase onto a different base unless the reviewer explicitly asks for it. The simplification is an iteration on the existing PR, not a new contribution.

    **Do NOT** remove the `nixpkgs-darwin-legacy` pin or the `x86_64-darwin` system from the flake, even if the reviewer suggests "simplify to just linux + aarch64-darwin". The legacy pin is the only path to a current-version install for Intel Mac users (nixpkgs-unstable dropped x86_64-darwin in 26.11). Explain this in the PR comment if needed.
...
| macos-14 CI job hangs ~20 min then cancels with `dyld: Symbol not found` (acryl PR #5) | The `magic-nix-cache-action` v14 static binary for `arm64-darwin` was built against a newer `libc++` than the `macos-14` runner ships — the action fails to load before the build starts | Gate the action to Linux-only: add `if: runner.os == 'Linux'` to the `magic-nix-cache-action` step. Darwin builds work without the cache, just slower. `validate-pre-push.sh` (Step 22b) catches missing `if: runner.os == 'Linux'` guards deterministically. See `references/advanced/github-actions-nix.md` |
...
| `overrideAttrs` flake builds but warns about outdated build | nixpkgs' packaging includes an `OutdatedBuildDetector` or version-comparison check that assumes the nixpkgs-pinned version | Disable the check in `postPatch` via `substituteInPlace` or by removing the check call. Since `overrideAttrs` intentionally overrides to a newer upstream version, this check is a false positive. See `references/flake-templates/nixpkgs-override-attrs.md` — `OutdatedBuildDetector` disable |
| `overrideAttrs` flake fails on `x86_64-darwin` but works on other systems | nixpkgs-unstable dropped support for the package on older macOS Intel, so `pkgs.<project-name>` doesn't exist there | Ensure the `nixpkgs-darwin-legacy` input is present and used for `x86_64-darwin` (same as source-build templates). The legacy pin must still have the package. See `references/flake-templates/darwin-legacy-pin.md` and `references/flake-templates/nixpkgs-override-attrs.md` — Darwin legacy pin |
...
3. `magic-nix-cache-action` without `if: runner.os == 'Linux'` guard (v14 static binary fails on `macos-14` with `dyld: Symbol not found` — the acryl PR #5 darwin hang, commit `82586e0`)
4. Missing `timeout-minutes` on nix workflow jobs
5. Deprecated runner labels (`macos-12`/`macos-13`/`macos-14`/`macos-15` — ALWAYS USE THE NEWEST RUNNER: `macos-26`/`macos-26-intel`/`ubuntu-latest`)
6. Stale branch (behind upstream)
7. Action pins (delegates to `validate-action-pins.sh`)
8. PR cleanliness (delegates to `validate-pr-cleanliness.sh`)
...
`magic-nix-cache-action` failures were guidance gaps: (1) the skill said
"DO NOT add" but the action works again — the real issue was omitting
`use-flakehub`, which defaults to `true` and attempts FlakeHub auth without
an org; (2) the v14 static binary fails on `macos-14` with `dyld: Symbol
not found` — the action needs `if: runner.os == 'Linux'` to gate it to
Linux-only. The push guard hook makes the pre-push validation a hard
gate: no push without validation.
...
- [ ] **[manual]** For source-build flakes: the `nixpkgs-darwin-legacy` input pinned to `nixpkgs-26.05-darwin` is present (Step 12 MANDATORY rule) — unless the project is `aarch64-darwin` only
