...
create_backup_tag() {
	...
	local backup_tag
	backup_tag="${BACKUP_TAG_PREFIX}-${project_name}-$(date +%Y%m%d-%H%M%S)"
...
main() {
	...
	local temp_dir
	temp_dir="${TEMP_DIR_BASE}/$(date +%s)-$$"
	mkdir -p "$temp_dir"

	# Ensure cleanup on exit
	# shellcheck disable=SC2064 # $temp_dir must expand now, not at signal time
