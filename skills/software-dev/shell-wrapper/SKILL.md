...
Wrap every bash tool call with an environment wrapper (devbox, mise, flox,
direnv, or nix — whichever the project uses) and rtk (when the command is
rtk-supported) before execution. This is a cheap, deterministic transform that
compounds token savings across a session.

## Why

Two problems this skill solves on every shell invocation:

1. **Wrong environment.** A bare shell lacks the tools a project's environment
   config declares (psql, python, gcloud, node, rtk itself). Running the
   command raw either fails with "command not found" or silently uses a system
   version that differs from the project's. The environment wrapper (`devbox
   run --`, `mise exec --`, `flox activate --`, etc.) loads the project's
   tools for one command, non-interactively, and exits.

2. **Token bloat.** Raw command output (git log, ls -R, cargo build, npm test)
   floods the context window. `rtk <cmd>` filters and compresses output by
   60-90% — same information, fewer tokens. RTK tracks which commands it
   optimizes; for unsupported commands it passes through unchanged.

The two compose: `devbox run -- rtk <cmd>` runs inside the devbox environment
*and* gets token-compressed output. The wrapper script makes this decision in
one call so the model doesn't re-derive it each time.

## Tool Resolution via cli-tool-discovery.sh

Both the environment wrapper and rtk are resolved using
`cli-tool-discovery.sh` — the shared script materialized in `scripts/`. It
checks PATH, environment wrappers (devbox, mise, flox, direnv, nix), 30+
standard PATH locations (tech-stack-aware), and package managers (brew, mise,
asdf). This means:

- **rtk is found even if not on the bare PATH** — e.g. if it's installed via
  cargo, homebrew, or inside a devbox environment
- **The wrapper is detected by probing for a nonexistent tool** — the
  discovery script checks PATH first, then wrappers; a nonexistent tool
  skips PATH and falls through to the wrapper check
- **All five wrappers are supported**, not just devbox

The `cli-tool-discovery.sh` script is materialized into this skill's `scripts/`
directory at build time via a `.tmpl` include, so the skill is self-contained
after installation (per the script-materialization best practice).
...
- **Source repo**: `~/p/gh/levonk/skills-src` (this skill:
  `src/current/skills/software-dev/shell-wrapper/`)
- **RTK source**: https://github.com/rtk-ai/rtk
- **Devbox source**: https://github.com/jetify-com/devbox
- **Script execution standards**: see the `python-script-standards` include
  wired in above for PEP 723, devbox detection, and rtk wrapper patterns
