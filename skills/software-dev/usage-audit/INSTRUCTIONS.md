...
Diagnose coding-agent token usage from session data already on disk. The
script produces numbers. Your job is the diagnosis — a user asking "where
did my tokens go" wants a cause and actionable insight, not a table. Lead
with the finding, then show the evidence.

## Multi-agent architecture

The script supports multiple coding agents via a parser interface. Each
agent has its own parser that reads that agent's local data and produces
common `SessionRecord` objects. The analysis layer (audit, forensics,
report, share) works on merged data from all detected agents.

| Agent | Data location | Parser status | Data granularity |
|-------|---------------|---------------|------------------|
| Devin CLI | `~/.local/share/devin/cli/` | Implemented | Session-level token totals |
| Claude Code | `~/.claude/projects/` | Planned (stub) | Per-request token usage |

Use `--list-agents` to see which agents are detected and which have
implemented parsers. Use `--agent <name>` on any command to filter to one
agent. Without `--agent`, all detected agents with implemented parsers are
merged.

**Adding a new agent parser:** Implement a class inheriting from
`AgentParser` with `name`, `display_name`, `data_paths`, `is_available()`,
`load_sessions()`, `dump_sample()`, and `doctor()`. Add it to
`ALL_PARSERS`. The analysis layer needs no changes — it works on the
common `SessionRecord` model.

## Hard rules

1. **Read-only on the data.** Never modify, move, or delete anything under
   any agent's data directory — it is the evidence being analysed. The only
   files you create are the outputs the user asked for (report, CSV,
   summary), in their working directory or where they specify.
...
4. **Only `share` output is redacted.** Everything else — session ids,
   project names, file paths in the audit — is private. Fine to show the
   user; never to be pasted into anything public.
...
6. **Cost estimates are approximate.** The model rates in the script are
   public list prices, not what the user actually pays. Always caveat cost
   numbers as estimates.

## Data sources

The script reads local data from each detected agent — no network, no
accounts:
...
|--------|------|------------------|
| Transcripts | `~/.local/share/devin/cli/transcripts/*.json` | Per-session `final_metrics` (prompt, completion, cached tokens, step count) |
| Sessions DB | `~/.local/share/devin/cli/sessions.db` | Session metadata (model, working_directory, timestamps, title, agent_mode) |

**Limitation:** Devin stores only aggregate session-level token counts,
not per-request usage. This means per-tool token attribution and per-file
context cost amplification are not available for Devin sessions.

### Claude Code (planned)

| Source | Path | What it provides |
|--------|------|------------------|
| Session logs | `~/.claude/projects/**/*.jsonl` | Per-request `message.usage` (input, output, cache_creation, cache_read) |

When implemented, the Claude Code parser will provide per-request
granularity, enabling per-tool attribution and context amplification
analysis that Devin's aggregate data cannot support.

## Script

The script sits in `scripts/usage_audit.py` relative to this skill's
directory. Stdlib only, no network, no packages to install.

**Resolve the script path before running anything.** Bash commands run
from the user's project directory, not from this skill's directory, so
relative paths will not find the script. Set `AUDIT` to this skill's own
directory — the one this INSTRUCTIONS.md was loaded from. Verify it
before trusting it:

```bash
ls "$AUDIT/scripts/usage_audit.py"
```

If that fails, find the script before doing anything else — do not fall
back to improvising your own analysis.

The script uses `uv run --script` (PEP 723). If `uv` is not available, it
falls back to `python3` directly since it has no external dependencies.

## Step 1 — verify the data before trusting any number

Run this first, every time:

```bash
python3 "$AUDIT/scripts/usage_audit.py" --doctor
```

Check that at least one agent's data sources are found. If no agents are
detected, the script will tell you what it looked for.

Then check which agents are available and their parser status:

```bash
python3 "$AUDIT/scripts/usage_audit.py" --list-agents
```

Then dump a sample to verify the transcript format hasn't changed:

```bash
python3 "$AUDIT/scripts/usage_audit.py" --dump-sample
```

Check that the expected fields are present. If they aren't, stop and tell
the user the parser needs updating — do not present numbers you don't
trust.

## Invocation variants

Bare invocation (or a matching plain-English question) → the full diagnosis
below. With a specific ask, jump straight to the matching command:

- **`usage` / `panel`** — at-a-glance panel: run
  `usage_audit.py panel --days 1` and show its output verbatim in a code
  block, followed by at most two sentences of interpretation.
- **`report [days]`** — build the HTML report (default 7 days) in the
  user's working directory. Open it with `open`/`xdg-open`. Then summarize
  the findings in 2-3 sentences.
- **`share [days]`** — write the redacted summary (default 30 days), say
  what it contains and omits, frame it as asking-for-help, not bug
  evidence.
- **`export` / `spreadsheet`** — run `usage_audit.py audit --days N --csv
  <path>` and tell the user where the CSV is.
- **`week`** — compare this week with last: run audit for 14 days, compare
  the daily breakdown between the two weeks — weight, cache-read share,
  and whether any habit change actually moved the numbers.
- **`agents`** — run `--list-agents` to show which coding agents are
  detected and their parser status.
...
```bash
python3 "$AUDIT/scripts/usage_audit.py" audit --days 30 --csv /tmp/usage.csv
```

Read the output. Key things to look for:

- **Agent breakdown.** If multiple agents are detected, the summary shows
  per-agent session and token counts. Compare across agents to see which
  is consuming more.
- **Total tokens and cache ratio.** A high cache ratio (>90%) means most
  prompt tokens are cache reads — efficient. A low ratio (<50%) means
  contexts are being rebuilt frequently — expensive.
- **Source coverage.** If "Both sources" is much less than "DB metadata"
  (Devin), many sessions lack transcript data. Those sessions show 0
  tokens — the real total is higher than reported.
- **By project concentration.** If one project accounts for >80% of
  tokens, that's the lead — name it.
- **By day discontinuity.** A day with 3-10x more tokens than the previous
  day is the single most informative signal: something changed that day.

## Step 3 — forensics for deeper analysis

```bash
python3 "$AUDIT/scripts/usage_audit.py" forensics /tmp/usage.csv
```

Read the forensics output together:

- **Discontinuity check** — a 3x+ day-over-day spike means something
  changed. Identify what: a new project, a long session, a background
  automation.
- **Session shape** — median duration and step count tell you the typical
  session. A very high max with a low median means one outlier session
  dominates.
- **Concurrency** — peak sessions started in one hour. If 5+ sessions
  started in the same hour, something may be invoking an agent
  programmatically (a script, CI job, batch harness).
- **Swarm detection** — many very short sessions (<5min, <10 steps)
  indicates automation. Every fresh session pays full context-creation
  cost, so a swarm is dramatically more expensive than one long session.
- **Concentration** — if one project or model is >90% of tokens, name it.
  That's the lead.
- **By agent** — if multiple agents are present, the forensics shows
  per-agent token share. Useful for comparing overhead across tools.

## Step 4 — window analysis (if the user asks about recent usage)

```bash
python3 "$AUDIT/scripts/usage_audit.py" forensics /tmp/usage.csv --at "$(date -u +%Y-%m-%dT%H:%M)"
```

This shows how many tokens were consumed in the 5 hours leading up to the
given time. If the user asks "is it safe to start a big task now?", check
whether the recent window is light or heavy relative to their observed
peak.

## Step 5 — form the diagnosis

Lead with the finding, not the numbers. Structure:

1. **The cause** — one sentence identifying what is driving usage.
2. **The evidence** — 2-3 supporting data points from the audit/forensics.
3. **What to change** — one concrete action that would save the most.

Examples:

> **One session in your `2ndbrain` project accounted for 69% of this
> week's tokens.** It ran for 19 hours across 172 steps — a long-running
> session that kept its context cached efficiently (96% cache ratio), but
> the sheer length means it re-sent a large context with every step.
> Starting a fresh session after the first 50 steps would have cut the
> total roughly in half.

> **17 very short sessions (<5 minutes each) suggest something is invoking
> Devin programmatically.** Each fresh session pays full context-creation
> cost. If this is intentional automation, consider batching into fewer,
> longer sessions. If it's not intentional, check what's spawning them.

> **Your usage is proportionate — no anomaly detected.** 125 sessions over
> 7 days with a 95% cache ratio is efficient. The top project (2ndbrain)
> accounts for 69%, which matches your stated work focus. No action
> needed.

## Step 6 — reports and exports (if requested)

- **HTML report:** `usage_audit.py report --days N --html <path>` —
  write to the user's working directory, open with `open`/`xdg-open`.
- **CSV export:** `usage_audit.py audit --days N --csv <path>` —
  for spreadsheet analysis.
- **Shareable summary:** `usage_audit.py share --days N` — redacted,
  contains only totals, dates, agent names, and model names. No session
  IDs, project names, file paths, or titles. Safe to post publicly.

## Calibration

- A single long session (100+ steps, 10M+ tokens) is normal for deep work
  on a complex project. Don't flag it as anomalous unless the user didn't
  expect it.
- A high cache ratio (>90%) is good — it means the context is being reused
  efficiently. Don't flag it as a problem.
- Sessions with 0 tokens are sessions without transcript data (the
  transcript file may not have been written yet, or the session is still
  active). Don't count them as "free" sessions — they did consume tokens,
  the data just isn't available locally.
- Cost estimates use public model rates and assume the user pays per-token.
  If the user is on a subscription plan (not API), the actual cost is the
  subscription fee, not the per-token estimate. Always caveat this.
- When multiple agents are present, different agents may have different
  data granularity. Devin provides session-level totals; Claude Code (when
  implemented) will provide per-request usage. Don't compare granularity —
  compare what each agent's data supports.
