...
Deploy multi-container services. Two branches: docker-compose for local
development and isolated testing, Ansible `community.docker.docker_container`
for production multi-host deployment. A service typically needs BOTH —
compose for development, Ansible for deployment.

## Decision Guidance

```
Local development / isolated testing / single-host dev
  → Branch A (docker-compose) — see references/docker-compose.md

Production deployment / multi-host / infrastructure-as-code
  → Branch B (Ansible docker_container) — see references/ansible-deploy.md
```

**The container-image-build skill produces the image; this skill deploys it.**
They are separate concerns. Build the image first (verify multi-arch), then
deploy using the appropriate branch.

## Steps

1. **Determine deployment context.** Is this local development, or production
   deployment across multiple hosts? Local/dev → Branch A. Production →
   Branch B. A service typically needs both.

2. **Execute the selected branch.** Follow the reference file:
   - Branch A: `references/docker-compose.md`
   - Branch B: `references/ansible-deploy.md`

3. **Verify the deployed image is multi-arch.** Before deploying to a
   mixed-architecture fleet, verify the image manifest covers all target
   architectures. See the container-image-build skill's
   `references/multi-arch.md` for verification commands.

4. **Apply security hardening.** Both branches include security defaults:
   non-root execution, `cap_drop: ALL`, `no-new-privileges`, read-only
   filesystems where possible, no Docker socket mounts. See the branch
   reference for specifics.

5. **Document platform limitations.** Note any Windows/WSL2 network
   restrictions, macOS OrbStack differences, or architecture-specific
   constraints in the service README.

## Branch Overviews

### Branch A — docker-compose (Local/Dev)

For local development and isolated testing. Uses `docker-compose.yml` with
services, networks, volumes, health checks. Follows the file structure
convention (docker/ subdirectory, mounts/templates/, healthcheck/, tests/).
See `references/docker-compose.md`.

### Branch B — Ansible docker_container (Production)

For production deployment, multi-host, infrastructure-as-code. Uses
`community.docker.docker_container` module — never docker-compose or
systemd in production. Includes container restart logic, port collision
detection, env var string conversion, security hardening. See
`references/ansible-deploy.md`.

## Key Principles

The rule above (`container-build-principles.md`) is inlined into this skill.
For deployment, the critical extensions:

- **Compose for dev, Ansible for prod** — not interchangeable. Compose is
  for local development and testing. Ansible `docker_container` is for
  production deployment across multiple hosts.
- **Never docker-compose in Ansible playbooks** — use
  `community.docker.docker_container` directly
- **All ports/IPs/domains must be variables** — never hardcoded in tasks or
  compose files. Follow the `{CATEGORY}_{SERVICE}_{SUB}_{HOST|CONTAINER}_{PORT|IP}`
  naming convention.
