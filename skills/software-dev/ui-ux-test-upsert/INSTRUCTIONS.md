...
- [ ] Step 1 — Detect platform (web, iOS, Android, React Native) and existing test framework via `project-detection`
- [ ] Step 2 — Parse requirements (PRD, user stories, acceptance criteria) into structured UI requirements
- [ ] Step 3 — Generate coverage tests (agent-browser for web, agent-device for mobile) for each requirement
- [ ] Step 4 — Define usability tasks as natural-language goals without step-by-step instructions
- [ ] Step 5 — Generate usability test specs (Stagehand for web, finalrun-agent for mobile) for each task
- [ ] Step 6 — Wire CI (coverage tests to PR pipeline, usability tests to nightly/release with graceful missing-key handling)
- [ ] Step 7 — Verify coverage tests pass against the running app; run usability tests if keys are available
- [ ] Step 8 — Report requirements covered, usability tasks passed, gaps found, tests deferred
