...
# Usage: git-commit-batch.sh [--amend] [--dry-run] [--slug <slug>] [--allow-main-write] [repo_root]
...
# Prevents commits on main/master or outside a linked worktree.
...
main() {
	...
	# Worktree isolation guard — skip in dry-run mode (no mutations).
	...
	while IFS= read -r line || [[ -n "$line" ]]; do
		if [[ "$line" =~ ^COMMIT:(.*)$ ]]; then
			...
			if [[ -n "$current_message" ]] && [[ ${#current_files[@]} -gt 0 ]]; then
				...
				echo "ERROR: FIX: use a heredoc (Format A) instead of printf — see commit-organization.md → Batch Commit Format:" >&2
				echo "ERROR:   cat <<'BATCH' | ./scripts/git-commit-batch.sh --slug <slug>" >&2
				echo "ERROR:   COMMIT:<subject>" >&2
				echo "ERROR:   (blank line)" >&2
				echo "ERROR:   - <body bullet>" >&2
				echo "ERROR:   (blank line)" >&2
				echo "ERROR:   #project-x #module-y #type-feat #skill-grm-created" >&2
				echo "ERROR:   FILES:<path>" >&2
				echo "ERROR:   BATCH" >&2
