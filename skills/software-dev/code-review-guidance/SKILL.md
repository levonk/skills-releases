...
infrastructure, schemas, integrations, security, performance, accessibility, and
cross-cutting concerns so reviewers can catch issues before they reach production.
...
Work through every category. Not every item applies to every PR — skip
categories that are clearly out of scope, but state that you skipped them so the
author knows the review was intentional.

### Infrastructure, Build & Deployment

- Does the change affect build configuration, CI pipelines, or deployment scripts?
- Are new environment variables or secrets introduced? Are they documented and
  injected securely (never committed)?
- Could this change require infrastructure provisioning or capacity adjustments?
- Are container images, base images, or runtime versions changed?

### Schemas & Data

- Were there schema changes that could require a database migration?
- If public APIs or types changed, is backwards compatibility preserved or is the
  API version incremented?
- Are migration scripts reversible and tested against a representative dataset?
- Are nullable fields, defaults, and breaking removals handled explicitly?

### Integrations

- Does the change affect upstream or downstream service contracts?
- Are webhook payloads, event schemas, or message formats modified?
- Do third-party API versions or authentication flows change?
- Are retry, timeout, and circuit-breaker behaviors preserved or improved?
...
- Are there N+1 queries, unnecessary loops over large collections, or redundant
  network calls?
- Are there places we should use caching? Is cache invalidation handled correctly?
- Are expensive operations moved off the hot path or made asynchronous?
- Are batch sizes, pagination, and streaming used where appropriate?

### Accessibility

- For front-end changes, review:
  - Keyboard navigation and focus management
  - ARIA roles, labels, and live regions
  - Color contrast and visible focus indicators
  - WCAG 2.1 AA compliance
- Are loading, empty, error, and offline states handled for user-facing flows?
- Is responsive design preserved across breakpoints?

### Cross-Cutting Concerns

- **Maintainability** — is the code readable, well-named, and appropriately
  documented where intent is non-obvious?
- **Compatibility & portability** — does the change assume a specific OS,
  architecture, or runtime that could break other environments?
- **Licensing & subscriptions** — are new dependencies compatible with the
  project's license? Do they introduce paid-tier requirements?
- **Compliance & legal** — are there GDPR, HIPAA, PCI, or regional data-handling
  implications?
- **Internationalization** — if I18N is set up, are new strings localized and new
  routes internationalized?
- **Feature flags** — does this change require adding a new feature flag?
...
1. **Triage** — classify the PR size and risk. Large or high-risk PRs may warrant
