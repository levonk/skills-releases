...
tags: ["ai/skill", "software-development", "github", "issue", "upstream-contribution", "open-source", "filing", "forge"]
see-also:
  - skill: github-pr
    relationship: "complement"
    description: "Creates PRs against upstream repos; requires an issue number (creates one inline using this skill's procedure if needed)"
  - template: github-issue-procedure
    relationship: "base-framework"
    description: "Shared issue creation procedure inlined at build time"
...
dependencies:
  - type: system
    ...
    reason: "Required for GitHub API calls (issue search, creation, validation)"
  - type: system
    name: curl
  ...
  - type: system
    name: base64
    reason: "Required for decoding GitHub API file content"
triggers:
  - user
---
...
The cache has a 7-day TTL (recorded in `date.knowledge-basis` in the cache
file's frontmatter). On a cache hit, `date.last-used` is updated. On a cache
...
relevant concept pages before filing:

- [Search Before Opening](references/included/knowledge/upstream-contribution-practices/search-before-opening.md) — don't duplicate
...
- [Contribution Eligibility](references/included/knowledge/upstream-contribution-practices/contribution-eligibility.md) — verify contributions accepted

## Troubleshooting

| Symptom | Cause | Fix |
|---------|-------|-----|
| `gh issue create` fails with 403 | No write access or issues disabled | Check repo settings; some repos disable issues |
| `validate-pr-issue.sh` exits non-zero | Body corrupted (literal `\n` or stripped backticks) | Re-draft, write to file, `gh issue edit --body-file`, re-validate |
