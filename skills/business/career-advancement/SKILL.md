...
description: Apply evidence-backed practices for the candidate side of the hiring pipeline — resume writing, ATS/application strategy, interview preparation, salary negotiation, recruiter communication, and age-bias navigation. Materializes the career-advancement-practices knowledge bundle and provides phase-based workflows for resume improvement, interview preparation, and offer negotiation. Use when the user asks to write or improve a resume, prepare for an interview, negotiate a salary, communicate with a recruiter, optimize a LinkedIn profile, or navigate age bias in hiring. Do NOT trigger on general career advice without a specific hiring-pipeline component, internal promotion strategy, or cover letter writing.
version: 2.0.0
user-invocable: true
...
see-also:
  ...
  - template: "trigger-guard"
    ...
    description: "Shared over-triggering guard protocol"
  - knowledge: "career-advancement-practices"
    relationship: "bundled-dependency"
    description: "Bundled via includeTree for offline availability. The canonical evidence-backed practices for the candidate side of the hiring pipeline — resume writing, ATS reality, interview strategy, salary negotiation, recruiter communication, LinkedIn optimization, age-bias navigation. Materialized at references/included/knowledge/career-advancement-practices/ at build time."
---
...
Apply evidence-backed practices for the candidate side of the hiring
pipeline. The canonical reference material lives in the
[`career-advancement-practices`](references/included/knowledge/career-advancement-practices/overview.md)
knowledge bundle — read the relevant bundle page before writing, editing, or
advising, do not restate the bundle's content here.

## When to trigger

- User asks to write, create, edit, improve, review, or optimize a resume.
- User asks to tailor a resume to a specific job description.
- User asks about resume length, format, keywords, metrics, or ATS
  compatibility.
- User asks how to position experience, handle gaps, or address
  overqualification on a resume.
- User asks to prepare for an interview — team composition, cultural fit,
  asking good questions, navigating the fast-track scenario.
- User asks about salary negotiation — bands, sign-on bonuses, answering
  the salary expectations question.
- User asks how to communicate with recruiters or optimize their LinkedIn
  profile for recruiter discovery.
- User asks how to navigate age bias or coded language in hiring.

## When NOT to trigger

- General career advice without a specific hiring-pipeline component.
- Internal promotion strategy or performance review preparation (future
  scope, not yet covered by the bundle).
- Cover letter writing (unless the user explicitly asks for hiring-pipeline
  guidance).
- Recruiter-side or hiring-manager-side guidance (this is the candidate's
  playbook).

## Workflow: Resume Improvement

### Phase 1: AUDIT — Assess the current resume

Read the resume and the target job description (if provided). Evaluate
against the bundle's concepts:

1. **Metrics check** — Are there at least 5 quantified achievements? Do
   bullets follow the X-Y-Z formula? See
   [metrics-and-quantification.md](references/included/knowledge/career-advancement-practices/metrics-and-quantification.md).
2. **Length check** — Is the word count in the sweet spot for the career
   stage? See
   [conciseness-and-length.md](references/included/knowledge/career-advancement-practices/conciseness-and-length.md).
3. **Scannability check** — Does the resume pass the fold test? Are
   company names and titles on the left margin? See
   [visual-scanning-patterns.md](references/included/knowledge/career-advancement-practices/visual-scanning-patterns.md).
4. **Fluff check** — Are there buzzwords, personal pronouns, or adverbs
   that should be replaced with evidence? See
   [fluff-and-buzzword-elimination.md](references/included/knowledge/career-advancement-practices/fluff-and-buzzword-elimination.md).
5. **Verb check** — Do bullets lead with ownership verbs? See
   [ownership-verbs.md](references/included/knowledge/career-advancement-practices/ownership-verbs.md).
6. **Keyword check** — Does the resume match the target JD's terminology?
   Are soft skills embedded naturally? See
   [keyword-engineering.md](references/included/knowledge/career-advancement-practices/keyword-engineering.md).
7. **Positioning check** — Does the resume signal "safe pair of hands"?
   Are risky elements explained on the page? See
   [safe-pair-of-hands-positioning.md](references/included/knowledge/career-advancement-practices/safe-pair-of-hands-positioning.md).
8. **Recency check** — Is experience weighted toward the last 5–7 years?
   See
   [proving-ground-principle.md](references/included/knowledge/career-advancement-practices/proving-ground-principle.md).
9. **ATS check** — Is the layout single-column with standard headers? See
   [ats-reality-vs-myths.md](references/included/knowledge/career-advancement-practices/ats-reality-vs-myths.md).
10. **Age-bias check** — Are there age-signaling details that should be
    removed? See
    [age-bias-coded-language.md](references/included/knowledge/career-advancement-practices/age-bias-coded-language.md).

### Phase 2: PRIORITIZE — Identify the highest-impact changes

Rank the audit findings by impact:

1. **Critical** — Missing metrics, ATS-breaking layout, age-signaling
   content, title gaps
2. **High** — Fluff/buzzwords, junior verbs, missing keywords, poor
   scannability
3. **Medium** — Length adjustments, keyword embedding, soft-skills gaps
4. **Low** — Minor formatting, optional improvements

Present the prioritized list to the user before making changes.

### Phase 3: APPLY — Make the changes

For each change:

1. Read the relevant bundle concept page for the evidence and method.
2. Apply the change to the resume.
3. Verify the change against the concept page's "common mistakes" section.

### Phase 4: VERIFY — Final check

Re-run the audit checklist from Phase 1. Confirm:

- All critical and high-priority findings are resolved.
- No new issues were introduced.
- The resume is consistent with any LinkedIn profile (if applicable — see
  [linkedin-profile-optimization.md](references/included/knowledge/career-advancement-practices/linkedin-profile-optimization.md)).
- The resume passes the fold test and the 7.4-second scan test.

## Workflow: Interview Preparation

### Phase 1: RESEARCH — Understand the team and role

1. Read the job description and identify the team's likely gaps (see
   [interview-strategy.md](references/included/knowledge/career-advancement-practices/interview-strategy.md)
   — Team Composition).
2. Research the company culture and values (see
   [interview-strategy.md](references/included/knowledge/career-advancement-practices/interview-strategy.md)
   — Cultural Fit).
3. Identify what signals the company evaluates (e.g., "Googliness" at
   Google).

### Phase 2: PREPARE — Draft questions and answers

1. Prepare 3–5 clarifying questions for the recruiter about the role and
   team gaps (see
   [recruiter-communication.md](references/included/knowledge/career-advancement-practices/recruiter-communication.md)).
2. Prepare 3–5 questions for the hiring manager about team gaps and
   success criteria.
3. Prepare 3–5 questions for peers about team dynamics and decision-making.
4. If overqualification is a concern, prepare the value-multiplier reframe
   (see
   [value-multiplier-reframe.md](references/included/knowledge/career-advancement-practices/value-multiplier-reframe.md)).
5. If age bias is a concern, prepare proactive surfacing language (see
   [age-bias-coded-language.md](references/included/knowledge/career-advancement-practices/age-bias-coded-language.md)).

### Phase 3: PRACTICE — Rehearse key answers

Rehearse answers to common questions using the ownership-verb and
metrics-quantification patterns from the resume (see
[ownership-verbs.md](references/included/knowledge/career-advancement-practices/ownership-verbs.md)
and
[metrics-and-quantification.md](references/included/knowledge/career-advancement-practices/metrics-and-quantification.md)).

## Workflow: Salary Negotiation

### Phase 1: RESEARCH — Know the band

1. Research the company's compensation philosophy and typical offers for
   the role/level (see
   [salary-negotiation.md](references/included/knowledge/career-advancement-practices/salary-negotiation.md)).
2. Identify what is typically negotiable (base, sign-on, equity,
   relocation, start date).

### Phase 2: DEFLECT — Get their range first

Use the salary expectations scripts to deflect the question and get the
recruiter to share the band (see
[salary-negotiation.md](references/included/knowledge/career-advancement-practices/salary-negotiation.md)
— The Salary Expectations Question).

### Phase 3: NEGOTIATE — Use the right lever

- If base is capped by the band, negotiate a sign-on bonus using the
  ramp-up-time justification.
- Frame the negotiation collaboratively, not adversarially.
- Strong interview performance is the best negotiation leverage.

## Context: Candidate-Specific Resume Systems

If the user has a candidate-specific resume system (e.g., an `AGENTS.md`
file defining a document hierarchy, guardrails, and workflow for a specific
person), **those specifications take precedence** over the general practices
in this bundle. The bundle provides the underlying evidence and reasoning;
the candidate-specific system provides the operational rules for that
candidate.

For example, if a candidate's `AGENTS.md` specifies a 2–3 page executive
