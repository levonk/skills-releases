...
description: >
  ...
  worked examples. (10) URL verification and tracking — every recommended
  product/service in the brief must include a verified direct URL (not a
  homepage), and a tracking page is created under
  `${XDG_DATA_HOME:-$HOME/.local/share}/shopping/` for each item with target
  specs, budget, qualifying features, and status, so research results persist
  across the pipeline and after the conversation ends.
version: 1.10.0
