...
references/included/knowledge/career-advancement-practices/

# Founder Content System

Set up and operate a complete founder content system for a single founder or
executive. The system produces ghostwritten social content across X, LinkedIn,
Instagram, TikTok, YouTube, Facebook, and a blog — built on verified facts, the
founder's verbatim voice, countable style checks, and a feedback ledger that
compounds every correction.

The canonical methodology is codified in the
[`career-advancement-practices`](references/included/knowledge/career-advancement-practices/founder-content-system.md)
knowledge bundle — read that concept page before running the system for a new
client. This skill is the **execution layer**; the bundle is the **why and
what**. Do not restate the bundle here.

**The one-sentence method:** facts come from verified sources, voice comes
from the client's verbatim writing, style is enforced by countable checks, and
every correction compounds in a ledger. Never write from a description of the
voice.
...
the approval stands for the engagement: a request for a setup, a batch, or a
review IS the request for the pipeline, and asking again every round defeats
the purpose.

Never quietly substitute single-context drafting because spawning felt
presumptuous — that is the exact failure Phase 4.3 exists to prevent, and it
has happened. If parallel agents are refused or unavailable, say plainly what
degrades: a batch drafted in one context converges on a single rhythm, and the
cold review in Phase 4.6 is the one step that cannot be dropped without losing
the quality guarantee.

## When to trigger

- User asks to set up, onboard, or start a founder content system for a
  specific founder or executive.
- User asks to draft a batch of founder posts (one or more platforms).
- User asks to review a founder content batch against the gates.
- User asks to run the day-of reactive track for a news peg.
- User asks to update the feedback ledger after a correction round.
- User asks to re-derive voice calibrations for a client.

## When NOT to trigger

- General copywriting or content marketing with no founder-voice component.
- Brand marketing campaigns on the company account (this system is for the
  founder's personal account; company launch content is a separate track).
- Paid ad creative (the ad creative process has its own briefs and legal
  review; this system produces organic content).
- A single one-off post with no system, no batch, and no voice corpus — just
  write the post directly.
- Non-founder personal blogging or hobby writing with no professional
  positioning goal.

## Phase 0 — Intake inventory (run FIRST, report gaps before researching)

Before any research, inventory the raw inputs and report the gaps. Do not
silently skip a research stream because its pointer is missing — every
unprompted research task in this playbook depends on a pointer existing
somewhere findable. **Every input is optional; every gap is named.** The
intake walker helps the user collect what is missing before drafting begins.

Run `scripts/intake-walker.py` to produce the FOUND / MISSING checklist and
the per-gap collection guidance. The script walks the client folder (if one
exists) and prompts for each missing input with: what it is, why the system
needs it, and how to get it (URL to paste, file to drop in, call to schedule).
The user can skip any input — the system proceeds on what is found while the
gaps get filled asynchronously.

Checklist (the script enumerates each):

1. **Recorded insight call** — one 45–60 minute recorded call with the
   founder, transcribed. Carries the origin story, the product in their own
   words, what they have strong opinions about, what they cannot say publicly,
   who reviews. If none exists: schedule and record the call before proceeding;
   the script lists the questions that matter. If a Granola/Zoom/Fathom
   transcript exists, paste the path or URL.
2. **Founder's own accounts** — X handle, LinkedIn URL, Instagram handle,
   TikTok handle, YouTube channel URL, Facebook page URL, blog URL. Confirm
   each. If a handle is unknown, ask; the script accepts partial answers.
3. **Reference links** — a links doc in the client folder: accounts and
   specific posts the founder admires, plus the founder's commentary on WHY
   they admire each. If the transcript mentions admired accounts but no links
   exist, collect them from the client, and search X for the named people
   meanwhile.
4. **Existing posts** — the founder's existing posts, full text. Scrape X via
   `from:handle -filter:replies` in the browser, pull LinkedIn recent
   activity, pull Instagram/TikTok/YouTube captions. Full text, not
   summaries. Note metrics; identify which era of their posting represents the
   voice they WANT.
5. **Published research / technical material** — does the company have a
   research page, papers, benchmarks, a technical blog? If yes, the papers
   download + index is MANDATORY (Phase 1.2), unprompted. If the product is
   technical and no papers exist, flag that accuracy will rest entirely on
   calls + site + docs.
6. **Transcripts they already have** — sales calls, customer calls, internal
   roadmap discussions, podcast appearances, conference talks. Each is a
   voice and fact source.
7. **Everything they have published** — blog posts, podcast episodes, talks,
   newsletter issues. Each is a voice and anchor source.
8. **Verbatim customer quotes and real numbers** — pasted, not paraphrased.
9. **Constraints in writing** — nameable customers, banned topics, competitor
   rules, who approves before publishing.
10. **Reference accounts they admire** — and specifically WHY (what to imitate
    structurally vs. what their persona cannot do).
11. **Brand or positioning doc.**
12. **Research or technical papers.**
13. **Internal docs** — manifesto-equivalents, launch docs, brand/positioning
    docs in the folder or Google Drive. Read everything present; list what was
    promised on calls but never delivered (chase list).

Deliver the inventory as a short checklist with FOUND / MISSING per line and
proceed on what is found while the gaps get filled.

## Phase 1 — Facts layer (before any writing)

1. **Context docs** (3–5 files): company story, product/technical glossary,
   positioning & audience, customers & track record. Sources: onboarding call
   transcripts, client-provided docs, a deep web crawl (background agent over
   site/blog/docs/press). Mark every unverified item `[UNVERIFIED]` inline;
   keep a consolidated flags list. **When two internal sources give different
   numbers (headcount, dates), record BOTH with attribution — never silently
   pick one.** **These are living documents**: every new call transcript
   triggers a careful re-read and doc update, quoting new material verbatim
   where it adds voice value.
2. **Primary sources folder**: download the actual papers/reports/PDFs the
   client's claims rest on, with an index file. Enumerate through EVERY
   channel, not just the research page: company site, Hugging Face org, GitHub
   org READMEs, arXiv author search on every named researcher (a paper can be
   reachable only through a dataset card). Verify downloads are real PDFs.
   Flag papers the site lists that the client did NOT author — research pages
   routinely mix in third-party work — and site errors such as a paper link
   pointing at the wrong ID. Rule learned three times over: agent summaries
   and crawl metadata drift from primary sources in BOTH directions (softer
   claims AND wrong dates). Claims in posts must trace to the primary
   document, not to an intermediate summary.
3. **Live-version check**: for any doc shared as a Google Doc/Drive link, read
   the LIVE version even when a file copy sits in the folder — internal docs
   go stale, and a live version may have cut something the file copy still
   carries.
4. **Anchor bank**: every post starts from a sourced anchor (verbatim client
   quote, verified stat, dated public artifact). No anchor, no draft. Strike
   out used anchors. Section for embargoed claims (heard internally but
   unverifiable — blocked until client confirms).

If a YouTube video is among the inputs (founder talk, podcast appearance,
recorded insight call), run the `youtube-content-analysis` skill to extract
the transcript first, then feed the transcript into the voice corpus and the
anchor bank.

## Phase 2 — Voice layer

1. **Scrape the client's real posts verbatim** (X via search
   `from:handle -filter:replies` in the browser, LinkedIn recent activity,
   Instagram/TikTok/YouTube captions). Full text, not summaries. Note
   metrics; identify which era of their posting represents the voice they WANT
   (one client's older posts were a voice they had moved on from; only the
   most recent year counted).
2. **Capture spoken voice** from call transcripts — founders explain their
   product best out loud; those phrasings are pre-approved material.
3. **Collect the reference accounts they admire** and the client's commentary
   on WHY (what to imitate structurally vs. what their persona cannot do).
4. **Write the Voice Corpus file**: verbatim samples annotated with the
   specific move each makes, a voice-DNA summary derived from (never imposed
   on) the samples, and a post-type table mapping formats to model samples.
5. **Measure the calibrations — never carry another client's numbers across.**
   Per client, measure from their corpus: punchline budget (a restrained
   founder may measure at one per X post and zero on LinkedIn; a theatrical
   one runs higher), hedge density and hedge words, emotional temperature,
   emoji/formatting habits, sentence-length range, analogy type (mechanical
   vs. personal), platform register differences.

## Phase 3 — Rules layer (three files)

1. **Persona constraints**: what the client structurally can/cannot say —
   competitor naming, customer NDA status, banned company narratives (the
   framings the company will not use about its own history), terminology
   preferences, audience priority order, review workflow. Comes from the
   calls, not from guessing.
2. **Banned patterns (slop lint)** — seed with the UNIVERSAL list
   ([`references/banned-patterns.md`](references/banned-patterns.md)), then
   grow client-specific entries via the ledger.
3. **Humanizer (stylometric pass)** — copy the Gate E structure from your
   reference implementation, refill baselines with this client's
   measurements. Includes the rewrite-pair library, seeded empty, grown from
   reviews.

## Phase 4 — Drafting workflow (the gates)

Copy your reference client's skill file as the template; re-point paths,
re-derive calibrations. The pipeline that must survive intact:

0. **Topic plan with adversarial critique, approved BEFORE drafting.** List
   concepts with pillar/anchor/format, then attack your own list: peg older
   than a month? (kill or convert to dated-assessment framing) · does it
   contradict anything the client has publicly said? (a drafted post can
   contradict the company's own published positioning — caught only by
   critique) · competitor adjacency? · does it repeat the client's own recent
   post? · weak ideas get benched with named replacements. Approve the
   surviving list before drafting. **Batch size comes from the client's
   committed cadence** (posts per workweek per platform), counted explicitly —
   a beautiful batch that under-fills the calendar fails.
1. **Anchor gate** — no invented facts; gaps become client questions, never
   filler.
2. **Format worksheet before drafting** — the idea dictates the container;
   batch format budget (≥4x length range, no two posts sharing opener+closer
   type, at most two long essays, at least one sub-50-word post). Containers
   include **comment-thread packages** (a short post plus replies mined from
   the primary source for depth — the paper-thread pattern; hiring posts put
   the roles link in a comment). **Visuals**: flag per post (LinkedIn
   narrative posts want a real photo; data posts may want a paper figure or
   transcript screenshot). Founder-account visuals are working artifacts,
   never branded design — brand polish reads as an ad and is reserved for
   launch content on the company account. **Platform variants of one story
   are allowed but must differ in content and register, not just length**.
3. **Isolated drafting** — one clean-context agent per post with only that
   post's brief. Drafting a batch in one context converges every post to one
   rhythm.
4. **Gate A accuracy** — claim→primary source; dates are claims (false
   recency = accuracy failure); opinions must trace to the client's stated
   positions, new formulations flagged for approval; public facts are OUR job
   to verify via search, client questions reserved for what only the client
   can know.
5. **Gates B–E** — banned patterns, sameness check across the batch, the
   client-calibrated voice pass, stylometric humanizer.
6. **Cold adversarial review (the batch-2 breakthrough)** — a fresh-context
   agent holding ONLY the client's verbatim posts + ban list + rewrite pairs
   reviews the drafts with zero knowledge of their drafting. The writer
   cannot review itself; same model, same instincts, fixes regress to the
   same defaults. This step took the process from four client style-rounds to
   zero on the test batch.
7. **Delivery format**: post text + claims-with-sources block + open items
   (client-only questions). Batch doc opens with the calibration disclaimer
   (expect feedback rounds; evergreen batch vs. day-of reactive track split).
   **Scheduling notes in the doc**: sibling posts (same story, two platforms;
   or same topic twice) at least a week apart · two benchmark/research posts
   never adjacent · thesis-sharing posts separated · launch-window posts
   placed relative to the launch date.

## Phase 5 — Feedback loop

- **Feedback ledger**, entry format: BEFORE (verbatim) / AFTER (verbatim) /
  TYPE / RULE (generalized) / STATUS (promoted where). Update in the SAME
  session that implements any feedback round — client edits in their own hand
  are the highest-value entries. Ask the client to keep suggesting mode and
  version history on in the shared doc so their rewrites are recoverable as
  diffs.
- Patterns appearing twice get promoted to the ban list or constraints.
- **The client's taste, and yours, is the training signal.** The system
  exists to make each correction permanent.
- **The README stays true in the same session as any structural change.** New
  file, new gate, renamed anything: the map updates immediately, same rule as
  the ledger. A stale README was caught on the reference engagement (the
  Humanizer file existed for a day before the README knew about it) — the map
  falling behind the territory is the exact failure this system exists to
  prevent, so it gets the same mandatory treatment.

## Day-of reactive track (separate from batches)

Evergreen batches go through review cycles; news takes decay in 48h. Reactive
process: find the live peg (browse X same day), verify the primary artifact
(whose chart, whose harness, whose claim — one reactive post turned on a
single word in a footnote), draft in-voice with translations for non-expert
readers, deliver same day via Slack. Peg-age rule: nothing older than ~a
month gets reaction framing; older references get dated-assessment framing
("Back in May, X predicted...").

## Platform register guide

Each platform has a different register and constraint set. The voice corpus
captures the founder's per-platform register; the calibrations are measured
per-platform, not carried across. Read
[`references/platform-registers.md`](references/platform-registers.md) for the
per-platform defaults (post length, media expectations, hashtag conventions,
link handling, scheduling cadence) — these are starting points to be
overridden by the client's measured register.

## Context Declaration

### File Paths
- Client folder: per-engagement, set by the user (the skill does not assume a
  fixed path; the intake walker asks for it on first run)
- Voice Corpus, Persona Constraints, Banned Patterns, Humanizer, Anchor Bank,
  Feedback Ledger: all live in the client folder
- Banned patterns reference:
  [`references/banned-patterns.md`](references/banned-patterns.md) (universal
  list, transferable; client entries added per engagement)
- Platform registers reference:
  [`references/platform-registers.md`](references/platform-registers.md)
- Methodology (the why and what):
  [`references/included/knowledge/career-advancement-practices/founder-content-system.md`](references/included/knowledge/career-advancement-practices/founder-content-system.md)

### External Resources
- Methodology distribution: <https://github.com/levonk/skills-releases/blob/main/knowledge/career-advancement-practices/founder-content-system.md>
- YouTube transcript extraction: the `youtube-content-analysis` skill (install
  separately via `pnpm dlx skills add levonk/skills-releases@youtube-content-analysis`)

### Project Information
- Skill: founder-content-system
