...
|---------|------|---------------------|
| `current` | `src/current/skills/<category>/<name>/` | `skills-releases` (public, via `pnpm dlx skills add`) |
...
Every skill uses the **wrapper pattern**: a thin `SKILL.md` wrapper that
runs `scripts/refresh.sh` to ensure the skill is current, then reads
`INSTRUCTIONS.md` for the actual content. This prevents the model from
consuming outdated information and saves tokens (the wrapper is ~40 lines;
the full content is loaded only after the refresh check).

```
skill-name/
├── SKILL.md (required — thin wrapper)
│   ├── YAML frontmatter metadata (required)
│   │   ├── name: (required)
│   │   ├── description: (required)
│   │   ├── skill-type: (outcome | process | hybrid)
│   │   ├── audience: (who/what the skill is for)
│   │   ├── freedom: (high | medium | low)
│   │   ├── source: (full URL to skill source directory)
│   │   └── date: (created, knowledge-basis, last-used)
│   └── Refresh section — runs scripts/refresh.sh, reads INSTRUCTIONS.md
├── INSTRUCTIONS.md (required — actual skill content)
│   ├── base-ai-content include (freshness, reflection, untrusted guard, etc.)
│   ├── trigger-guard include
│   └── Skill body (Outcome, Good vs Bad, Guardrails, Calibration, etc.)
└── Bundled Resources (optional)
    ├── scripts/
    │   ├── refresh.sh (required — daily refresh + sandbox + print body)
    │   ├── cli-tool-discovery.sh (required — tool discovery)
    │   ├── resolve-reference.sh (required — three-tier reference resolver)
    │   └── ... (skill-specific scripts)
    ├── references/
    │   ├── nono-profile.json (required — sandbox profile for refresh.sh)
    │   ├── process.md (optional — detailed process for hybrid skills)
...
1. Model triggers skill (frontmatter description, always in context)
2. Model loads SKILL.md — the thin wrapper
3. Wrapper instructs model to run `scripts/refresh.sh`
4. `refresh.sh` checks daily-refresh cache, runs `pnpm dlx skills update`
   (sandboxed via nono if available), updates `last-used` in SKILL.md to
   today, prints `INSTRUCTIONS.md` to stdout
5. Model reads the refresh.sh output (the current INSTRUCTIONS.md)
6. Model proceeds with the skill's actual content

This ensures the model never reads stale instructions — the update runs
before the content is loaded.

#### last-used Ordering Bug (Prevention)

`skills update` overwrites the entire skill directory, including `SKILL.md`
and its frontmatter. The `self-update-requirement` include (in
`base-ai-wrapper.md`) instructs the AI to update `last-used` — but if the AI
does this BEFORE running `refresh.sh`, the update destroys the change and
`last-used` reverts to the published value. This was the root cause of
persistent stale `last-used` dates across all wrapper-pattern skills.

**Prevention (both layers required for new skills):**

1. **`refresh.sh` updates `last-used` deterministically** — after
   `skills update` completes, `refresh.sh` sets `last-used` to today via
   `sed`. This is the single source of truth; the AI-side instruction is a
   fallback for when refresh is skipped.
2. **`self-update-requirement.md.tmpl` documents the ordering** — the
   include text tells the AI to run `refresh.sh` FIRST, then update
   `last-used`, and explains why (the overwrite destroys pre-refresh
   updates).

**When creating or modifying a wrapper-pattern skill, NEVER**:
- Move `self-update-requirement` to fire before `refresh.sh` in the
  rendered SKILL.md order
- Remove the `last-used` update from `refresh.sh` (the deterministic
  update is the fix, not the AI instruction)
- Add a "update last-used before proceeding" instruction without
  qualifying it with "after refresh" for wrapper-pattern skills

### skill-type Frontmatter

| Value | Meaning | When to use |
|-------|---------|-------------|
| `hybrid` | Guardrails in body + detailed process in references/process.md | Default for new skills — no fidelity loss |
| `outcome` | Guardrails + calibration only, no Current Process | When the recipe doesn't matter, just the outcome |
| `process` | Current Process is the body itself (old paradigm) | When determinism is the point (e.g., surgical-config) |
...
Every skill ships `scripts/refresh.sh` (materialized from
`includes/refresh.sh.tmpl` at build time). The script:

1. Checks skip conditions (`SKIP_SKILL_REFRESH=1`, inside skills-src)
2. Checks daily-refresh cache (once per day per skill, stored in
   `${XDG_CACHE_HOME:-~/.cache}/skills/levonk/skills-releases/skills/<name>/refresh.date`)
3. Finds pnpm via `cli-tool-discovery.sh`
4. Detects existing nono session (skips sandbox if already inside one)
5. Runs `pnpm dlx skills update <skill-name>` — sandboxed via nono with
   the bundled `references/nono-profile.json` if available
6. Writes today's date to the cache file regardless of outcome
7. Updates `last-used` in `SKILL.md` frontmatter to today (deterministic
   fix for the last-used ordering bug — `skills update` overwrites
   `SKILL.md`, so the AI-side `self-update-requirement` instruction would
   be destroyed if it fired before refresh)
8. Prints `INSTRUCTIONS.md` to stdout

See the ADR:
[Skill Refresh Sandbox Selection](https://github.com/levonk/skills-src/blob/main/internal-docs/adr/2026/08/adr-202608030953-skill-refresh-sandbox-selection.md)
for the sandbox selection rationale and platform coverage matrix.
...
skill-type: <enum>      # hybrid (default) | outcome | process — controls body structure
audience: <string>      # who/what the skill is for (operator, developer, agent-author, etc.)
freedom: <enum>         # high | medium | low — degrees-of-freedom default
source: <url>           # full URL to skill source directory in GitHub
...
**Wrapper-pattern fields** (new in the wrapper pattern):

| Field | Values | Purpose |
|-------|--------|---------|
| `skill-type` | `hybrid` (default), `outcome`, `process` | Controls INSTRUCTIONS.md body structure. `hybrid` = guardrails + process in references. `outcome` = guardrails only. `process` = process in body (old paradigm). |
| `audience` | free text | Who/what the skill is for. Helps the model calibrate tone and depth. |
| `freedom` | `high`, `medium`, `low` | Degrees-of-freedom default. How much latitude the model has to deviate from the process. |
| `source` | full URL | Full URL to the skill source directory in GitHub (e.g., `https://github.com/levonk/skills-releases/blob/main/skills/ai/readme-upsert/`). Used for traceability and freshness checks. |

### Body (Markdown)

The SKILL.md body is a thin wrapper. The actual skill content lives in
`INSTRUCTIONS.md`.

**SKILL.md includes** (top of body, after frontmatter):
...
- `base-ai-wrapper`: wrapper-level framework (self-update, CLI tool discovery)
- `trigger-guard`: guard against over-triggering

**INSTRUCTIONS.md includes** (top of body):
...
- `base-ai-content`: content-level framework (freshness check, post-task
  reflection, untrusted content guard, reference resolution, AI guidance
  creation, content principles, subagent delegation, project overrides)
- `trigger-guard`: guard against over-triggering (also in SKILL.md wrapper)
...
**Backwards compatibility**: `base-ai-guidance.md.tmpl` (the original hub
include) remains available for skills that have not yet migrated to the
wrapper pattern. It bundles both wrapper-level and content-level includes
into a single file. New skills should use `base-ai-wrapper` + `base-ai-content`
instead.

**Section order** (INSTRUCTIONS.md body, after includes):
1. Title (`# Skill Name`) — one line
2. Outcome — what the skill produces, 1-3 sentences
3. Good vs Bad — 2-3 examples of each, calibrates the model
4. Guardrails — inviolable rules that must hold during execution
...
1. Title (`# Skill Name`) — one line
2. Refresh — run `scripts/refresh.sh`, read its output
3. References — links to `references/*.md` files (minimal — most go in INSTRUCTIONS.md)
...
See `references/skill/script-execution-standards.md` for full detection and wrapper patterns (bash and python).
...
`references/skill/progressive-disclosure.md` — Anti-Patterns for the embedded-code
