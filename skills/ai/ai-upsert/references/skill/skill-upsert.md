...
2. **Audit against the skill guidelines.** Check the existing skill for compliance with:
   ...
   - **last-used ordering (wrapper-pattern skills)**: If the skill uses the wrapper pattern (has `scripts/refresh.sh` and `INSTRUCTIONS.md`), verify that `refresh.sh` updates `last-used` in `SKILL.md` AFTER `skills update` completes (deterministic fix). Verify that `self-update-requirement` does NOT instruct the AI to update `last-used` before running `refresh.sh` — that update would be destroyed by `skills update` overwriting the file. See `references/skill/anatomy.md` — last-used Ordering Bug (Prevention).
   - **Task List** (see `references/skill/anatomy.md` — Task List + Definition of Done Sections): Does the skill have a `## Task List` section? Are items checkbox-tracked with status marks (`[ ]`/`[~]`/`[x]`/`[!]`)? Is the mark legend present? Is the maintenance protocol present? Flag missing or non-standard Task List sections.
   - **Definition of Done** (see `references/skill/anatomy.md` — Task List + Definition of Done Sections): Does the skill have a `## Definition of Done` section? Are items tagged `[script]`/`[manual]`? Are items grouped into subsections by deliverable category? Is there a "Not Done (common false-completion signals)" anti-checklist? If the skill produces deterministic outputs (scripts, builds, configs), are `[script]` items present for the verifiable checks? Flag missing or non-standard DoD sections.
