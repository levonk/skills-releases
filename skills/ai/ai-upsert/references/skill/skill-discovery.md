...
uv run --script scripts/skill/discover_skills.py "skill comparison and evaluation"

# Search only local skills
uv run --script scripts/skill/discover_skills.py "code review" --sources local

# Search skills.sh and GitHub only (skip local)
uv run --script scripts/skill/discover_skills.py "pdf rotation" --sources skills.sh,github

# Verbose output (prints progress to stderr)
uv run --script scripts/skill/discover_skills.py "git workflow" --verbose

# Dry run (show what would be searched without API calls)
uv run --script scripts/skill/discover_skills.py "testing" --dry-run
```
