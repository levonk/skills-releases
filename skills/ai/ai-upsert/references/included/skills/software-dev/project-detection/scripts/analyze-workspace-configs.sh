...
analyze_nx_workspace() {
    ...
    log_info "Analyzing Nx workspace: $file (preferred per ADR 20260419001)"
    
    echo "  ✓ Found Nx configuration (preferred monorepo tool)"
...
analyze_workspace_configs() {
    ...
    # NX is preferred over Turbo per ADR 20260419001
    if [[ -f "nx.json" ]]; then
        analyze_nx_workspace "nx.json" "$target_project"
        configs_found+=("nx.json")
    fi
    
    # Turbo is legacy - superseded by NX
