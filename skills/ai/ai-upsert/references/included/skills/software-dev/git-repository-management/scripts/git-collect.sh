...
# Wrapper: use 'jj log' when jj is available and working, otherwise 'git log'.
# Produces cleaner, more structured log output for AI analysis. Falls back to
# git log on any jj runtime failure (config error, corrupt repo, version
# incompatibility) — not just on binary absence — so a broken jj install never
# silently blanks RECENT_LOG.
jj_log() {
	if [[ "$JJ_AVAILABLE" -eq 1 ]]; then
		devbox_run jj log "$@" 2>/dev/null && return
	fi
	git_cmd log "$@"
}
...
emit_json() {
	...
	parse_status() {
		local s="$1"
		if [[ "$s" == *"STATUS:PASS"* ]]; then
			printf 'PASS'
		elif [[ "$s" == *"STATUS:FAIL"* ]]; then
			printf 'FAIL'
		elif [[ "$s" == *"NOT_AVAILABLE"* ]]; then
			printf 'NOT_AVAILABLE'
		elif [[ "$s" == *"NOT_CONFIGURED"* ]]; then
