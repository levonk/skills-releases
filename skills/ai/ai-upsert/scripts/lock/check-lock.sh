...
# --- Lock directory resolution (mirrors acquire-lock.sh) ---
resolve_lock_dir() {
...
if [ -z "$REPO" ] || [ -z "$SKILL" ]; then
    echo "usage: check-lock.sh --repo <path> --skill <name> [--json]" >&2
    exit 1
fi
...
if [ -d "$CURRENT_DIR" ]; then
    for lock_file in "$CURRENT_DIR"/*-"${LOCK_KEY}".sh; do
        ...
        # Stale locks self-archive during the check
    done
fi
