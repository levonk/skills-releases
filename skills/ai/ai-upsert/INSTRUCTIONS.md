...
1. **Initialize the run log** (see "Run Log" section above). Create
   ...
   to it.
2. **Run the clean-repo check** using the bundled `git-repository-management`
...
3. **If the working tree is clean** (no uncommitted changes): append
   `Phase 0: CLEAN` to the run log, proceed to Phase 1.
4. **If the working tree is dirty**: do NOT abort. The
...
5. **If the target is not a git repo** (e.g., creating a skill in a fresh
