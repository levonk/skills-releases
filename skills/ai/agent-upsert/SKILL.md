...
see-also:
  ...
  - template: "agent-template"
    relationship: "structure-standard"
    description: "Standard agent definition template with frontmatter, i/o schema, workflow, guardrails, and contracts"
  - skill: "ai-upsert"
    relationship: "sibling"
    description: "Full lifecycle management for skills (create/update/convert/eval). Use when the target is a skill, not an agent."
  - skill: "ai-workflow-upsert"
    relationship: "sibling"
    description: "Full lifecycle management for workflows. Use when the target is a workflow, not an agent."
  - skill: "agent-file-upsert"
    relationship: "complement"
    description: "Generates AGENTS.md hierarchy for AI agents working in codebases — use for agent documentation, not agent definitions"
...
A skill for creating expert agents that channel specific expertise and work autonomously, and for auditing and improving existing agent definitions. Handles the full agent lifecycle: create from scratch, update existing, and audit for relevance.

## Workflow Diagram

```mermaid
flowchart TD
    Start([Invoke]) --> Mode{"Agent file<br/>exists?"}
    Mode -->|"No"| Create["Mode A:<br/>Create"]
    Mode -->|"Yes"| Update["Mode C:<br/>Update"]

    Create --> R0["Step 0:<br/>Research existing agents"]
    R0 --> R1["Step 1:<br/>init-agent.py scaffold"]
    R1 --> R2["Step 2:<br/>Customize frontmatter"]
    R2 --> R3["Step 3:<br/>Define design focus"]
    R3 --> R4["Step 4:<br/>Apply agent guidelines"]
    R4 --> R5["Step 5:<br/>verify-agent.py"]
    R5 --> R6["Step 6:<br/>Deliver to internal-docs/agents/"]
    R6 --> DoneC([Done])

    Update --> U1["1. Read existing agent"]
    U1 --> U2["2. Audit vs checklist"]
    U2 --> U3["3. Propose changes<br/>(do not apply)"]
    U3 --> U4{"User approves?"}
...
1. **Check whether the target agent file already exists** at `internal-docs/agents/<name>.md` (or the user-specified location).
2. **If no agent file exists** → **Mode A: Create a New Agent from Scratch**.
3. **If the agent file already exists** → **Mode C: Update an Existing Agent (Upsert)**.

## Mode A: Create a New Agent from Scratch

0. **Research existing agents**: Run the research phase before creating. Skip
   only if the user explicitly says "skip research" or "don't search".

   **Agent-specific search** — see `references/agent-search.md` for the full
   search workflow (local `internal-docs/agents/`, skills.sh / GitHub, and
   cross-check with prompts/templates).
...
3. **Define the agent design focus**: Embody specific expertise, ensure autonomous operation after initial questioning, and define clear boundaries and capabilities. See `references/agent-design.md` for agent-specific design guidance.
...
## Mode C: Update an Existing Agent (Upsert)

When the target agent file already exists, switch to update mode. The goal is to bring the existing agent into compliance with the agent guidelines without blindly overwriting the author's intent.

**Research phase**: Run Step 0 above before improving — understand the
landscape and whether better-maintained agent patterns exist. Skip only if
...
**Agent-specific audit checklist** — in addition to the shared audit
methodology above, check:

- **Capabilities still relevant**: Are the declared `capabilities` still
  accurate for the agent's domain? Have any capabilities become obsolete or
  been superseded by new tools/workflows?
- **Template still valid**: Does the agent definition still conform to the
  current `templates/meta/agent-template.md.tmpl` structure? Are any new
  required fields missing?
- **Agent still used**: Is `date.last-used` recent? If the agent hasn't been
  used in a long time, flag it as a candidate for deprecation (set
  `agent-status: "deprecated"`).
- **Model level appropriate**: Is the `model-level` still the best choice for
  the agent's tasks? Consider whether a newer model would perform better or
  whether a lower-cost model would suffice.
- **Tools up to date**: Are the declared `tools` still available and
  relevant? Have any tool contracts changed?
- **Personality and voice**: Is the `personality` configuration still
  appropriate for the agent's role and audience?
- **Frontmatter compliance**: Does the frontmatter include all required
  fields (`agent`, `description`, `use`, `personality`, `categories`,
  `capabilities`, `model-level`, `version`, `date`, `tags`)?

**High-level steps:**

1. **Read the existing agent fully** — frontmatter, body, i/o schema, workflow,
   guardrails, and contracts.
2. **Audit against the agent guidelines** using the checklist above.
...
7. **Consistency verification** — after applying changes, run the verification
   ...
   - The agent definition renders correctly (if `.tmpl`)

**Never silently overwrite.** The author may have intentionally deviated from
a guideline. Propose, explain the benefit, and let them decide.

## Cross-Cutting Concerns

### Agent Design Principles

- Create agents that embody specific expertise (e.g., tax strategist, software
  architect, spiritual advisor)
...
- Use the standard agent frontmatter structure — see `references/agent-structure.md`
...
Ensure no secrets, keys, or sensitive paths are exposed in agent definitions.
