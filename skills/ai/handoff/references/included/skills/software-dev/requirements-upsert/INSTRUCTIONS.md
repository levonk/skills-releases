...
# Requirements Upsert — Detailed Instructions

## Identity

A requirements ledger tracks durable constraints and capabilities that
survive across features. The ledger uses a four-state lifecycle:
`proposed/` (draft plans), `todo/` (ready change descriptions),
`current/` (how the system works now), and `history/` (evolution log
with completed plans and superseded snapshots). See
`references/granularity-guidance.md` for the full lifecycle description.

## Prototype Over Ask

Before asking the user a clarifying question about a requirement, check
whether the answer can be inferred from the codebase or a prototype. If
it can, infer it — do not ask. Reserve requirement elicitation for
genuine product or preference decisions that cannot be inferred from
the code.

This is the "prototype over ask" principle (pstack poteto-mode): the
codebase is the source of truth for technical constraints. A question
that the code can answer is a question that wastes the user's time.

### When to Infer (Do Not Ask)

- **The constraint is already encoded in the codebase.** The existing
  code follows a pattern (all endpoints are versioned, all config is
  in TOML, all errors use the Result type). The requirement is to
  document the existing constraint, not to decide it. Read the code,
  infer the constraint, write the requirement.
- **The constraint can be discovered by building a prototype.** A
  quick prototype (a sketch, a minimal implementation, a test) reveals
  whether an approach works. The prototype answers the question; the
  user does not need to.
- **The constraint is a consequence of an existing decision.** If the
  project chose PostgreSQL, the requirement "all timestamps must be
  stored as timestamptz" is a consequence, not a decision. Infer it
  from the existing schema.

### When to Ask (Genuine Product or Preference Decisions)

- **The constraint is a product decision.** "Should the session timeout
  be 15 minutes or 30 minutes?" is a product decision — the codebase
  cannot answer it. Ask.
- **The constraint is a user preference.** "Should we use tabs or
  spaces?" is a preference (unless the codebase already has a
  convention). Ask only if no convention exists.
- **The constraint has tradeoffs the user must weigh.** "Should we
  prioritize consistency or availability for this service?" is a
  tradeoff decision. Ask, presenting the tradeoff.

### How to Infer

1. **Read the codebase.** Search for existing patterns, conventions,
   and constraints. The code is the evidence.
2. **Build a prototype if needed.** If the constraint depends on
   whether an approach works, build a minimal prototype to find out.
   The prototype is disposable — its purpose is to answer the
   question, not to become the implementation.
3. **Document the inference.** When writing the requirement from an
   inference, note the evidence: "Inferred from existing schema where
   all timestamps use timestamptz." This makes the inference auditable
   — a reviewer can verify the evidence.

## Granularity

A requirement is something that **outlives the feature that created it**.
If a statement would not survive its feature, it belongs in the PRD, not
the ledger.

Examples of requirements:
- "All API endpoints must be versioned (`/v1/`, `/v2/`)"
- "User authentication must support MFA via TOTP"
- "CI build must complete in under 60 seconds"

Examples that are NOT requirements (belong in PRDs):
- "Add a settings page to the dashboard" (feature-scoped)
- "Fix the login redirect bug" (task-scoped)
- "Refactor the auth module to use the Result pattern" (refactor-scoped)

See `references/granularity-guidance.md` for the full decision guide.
...
- `{proj}` defaults to the repo name when single-project
- `{proj}`, `{module}`, `{slug}` are each kebab-case
- `_` joins the three components in current filenames (composite-identity
  delimiter, not snake_case — each component is kebab-case internally)
- History filenames use `req-` prefix + date + slug only (path carries
  proj/module)

## Current Requirement Frontmatter
...
date:
  ...
  superseded: ""            # set when status → superseded
supersedes: ""              # history path this one replaced (empty for first version)
see-also:
  - adr: "internal-docs/adr/YYYY/MM/adr-..."
  - feature: "internal-docs/feature/archive/YYYY/MM/{slug}/feat-..."
---
```

## Current Requirement Body

1. **Statement** — the requirement, one paragraph
2. **Rationale** — why it exists
3. **Constraints** — specific thresholds, versions, scope boundaries
4. **Verification** — how to check compliance (commands, tests, grep
   patterns)
5. **Change Log** — append-only: `YYYY-MM-DD — what changed — link to
   history snapshot`

## History Snapshot Frontmatter
...
module: {module}
slug: {slug}
status: superseded          # or "created" for the first snapshot
date:
  created: "YYYY-MM-DD"     # = the filename timestamp
  superseded: "YYYY-MM-DD"  # when the next version replaced it
superseded-by: "internal-docs/reqs/current/{proj}/{module}/{proj}_{module}_{slug}.md"
---
```

## Process: Create

1. Determine `{proj}`, `{module}`, `{slug}`.
   - `{proj}`: the project name. In a monorepo, this is the project
     directory or logical project name. In a single-project repo, this
     is the repo name.
   - `{module}`: the module or package within the project. If the
     project has no module subdivision, use `_root`.
   - `{slug}`: kebab-case, 3-6 words, describing the requirement (not
     the feature that created it).
2. Check `internal-docs/reqs/current/{proj}/{module}/` for an existing
   file with the same slug. If one exists, this is an update — see
   Process: Update below.
3. Create directories: `mkdir -p internal-docs/reqs/current/{proj}/{module}/`
4. Write the file using `references/requirement-template.md` as the
   template. Set `status: active`, `date.created` and `date.last-revised`
   to today's date.
   The template uses EARS patterns (5 templates with SHALL). Pick the
   template that fits the requirement type. See `references/ears-patterns.md`.
   For provability-critical requirements (idempotency, money-moving, auth,
   concurrency, safety-critical), also create a parallel `.fsl` spec — see
   `references/fsl-integration.md`.
5. Run `scripts/snapshot-requirement.sh --project {proj} --module {module}
   --slug {slug}` to create the initial history entry with
   `status: created`.
6. Run `scripts/index-requirements.sh` to regenerate `INDEX.md`.
7. Commit: `git add internal-docs/reqs/ && git commit -m "docs(reqs): add
   {proj}/{module}/{slug} requirement"`

## Process: Update

1. Run `scripts/snapshot-requirement.sh --project {proj} --module {module}
   --slug {slug}` — copies the current file to
   `history/YYYY/MM/{proj}/{module}/req-YYYYMMDDHHmm-{slug}.md` with
   `status: superseded` and `superseded-by` pointing to the current file.
2. Edit the current file in place.
3. Update `date.last-revised` to today.
4. Append a Change Log entry:
   ```
   - 2026-08-29 — Changed threshold from 2.5s to 2.0s — [snapshot](history/2026/08/{proj}/{module}/req-202608291430-{slug}.md)
   ```
5. Run `scripts/index-requirements.sh` to regenerate `INDEX.md`.
6. Commit: `git add internal-docs/reqs/ && git commit -m "docs(reqs):
   update {proj}/{module}/{slug} requirement"`

## Process: Supersede

1. Run `scripts/snapshot-requirement.sh` (final snapshot).
2. Set `status: superseded` in the current file. Set `date.superseded` to
   today.
3. The current file stays in `current/` — it is the latest state, even
   if that state is "no longer active". The snapshots in `history/` are
   the evolution record.
4. Run `scripts/index-requirements.sh`.
5. Commit: `git add internal-docs/reqs/ && git commit -m "docs(reqs):
   supersede {proj}/{module}/{slug} requirement"`

## Process: Propose

Create a draft plan for a potential change. The plan is not ready to be
implemented yet — it has open questions or unresolved dependencies.

1. Determine `{proj}`, `{module}`, `{slug}` as in Process: Create.
2. Create directories: `mkdir -p internal-docs/reqs/proposed/{proj}/{module}/`
3. Write the file using `references/proposed-template.md` as the template.
   Set `status: proposed`, `date.created` to today. Leave `date.ready` empty.
4. Run `scripts/index-requirements.sh` to regenerate INDEX.md.
5. Commit: `git add internal-docs/reqs/ && git commit -m "docs(reqs):
   propose {proj}/{module}/{slug}"`

## Process: Mature (Proposed → Todo)

When a proposed plan is validated and ready to be implemented:

1. Set `date.ready` in the frontmatter to today's date.
2. Set `status: todo`.
3. Move the file from `proposed/{proj}/{module}/` to
   `todo/{proj}/{module}/` using `git mv`.
4. Run `scripts/index-requirements.sh`.
5. Commit: `git add internal-docs/reqs/ && git commit -m "docs(reqs):
   mature {proj}/{module}/{slug} from proposed to todo"`

## Process: Process Todo (Todo → History + Current Updated)
...
3. The script moves the todo to
   `history/YYYY/MM/{proj}/{module}/todo-YYYYMMDDHHmm-{slug}.md` via
   `git mv` (preserving git history) and updates its frontmatter to
   `status: implemented` with `date.implemented` set.
4. **If no `current/` file existed** (new requirement): create one
   fresh from `references/requirement-template.md`. The archived todo
   is reference material only — read it to understand what was
   ...
   - Set `status: active`, `date.created` and `date.last-revised` to
     today
   - Add a Change Log entry referencing the archived todo
   - Run `snapshot-requirement.sh` for the initial history snapshot
5. **If `current/` already existed** (changed requirement): update it
   to reflect the new behavior, bump `date.last-revised`, and append a
   Change Log entry:
   ```
   - 2026-09-03 — Implemented planned change — [todo](history/2026/09/{proj}/{module}/todo-202609031430-{slug}.md)
   ```
6. Run `scripts/index-requirements.sh` (the script already does this,
   but run again after editing current/).
7. Commit: `git add internal-docs/reqs/ && git commit -m "docs(reqs):
   process todo {proj}/{module}/{slug}"`

## Process: Validate

Run `scripts/validate-ledger.sh` to check:
- Every current requirement has at least one history snapshot
- No orphan history entries
- Frontmatter dates are consistent
- `INDEX.md` is up to date
- Proposed/todo files have valid status fields
- Todo files have a ready date set

Check 5 (EARS pattern validation) runs `lint-ears.py` which checks that
every requirement's Statement and Constraints use a valid EARS pattern
with SHALL. See `references/ears-patterns.md` for the 5 valid patterns.

## INDEX.md and INDEX.html Format

Auto-generated by `scripts/index-requirements.sh`:

- **`INDEX.md`** — pure markdown, viewable on github.com. Shows all four
  states (proposed, todo, current, history) grouped by project and module,
  with status badges and summary lines. Includes a summary table with
  counts per state.

- **`INDEX.html`** — standalone HTML with embedded JavaScript. Renders an
  interactive view with search, state filter, and project filter. Works
  when viewed locally (`open INDEX.html`) or via GitHub Pages, but not on
  github.com directly (GitHub strips `<script>` tags). The HTML reads a
  JSON blob embedded in the page — no external dependencies, no server
  needed.

Both files are committed to the repo and version-controlled.

## Scripts

- `scripts/snapshot-requirement.sh` — copy current → history with
  timestamp. Args: `--project`, `--module`, `--slug`. Reads the current
  file, writes the history snapshot with `status: superseded` (or
  `status: created` if no prior history exists).
- `scripts/process-todo.sh` — process a todo file: snapshot current,
  archive todo to history with `status: implemented`, regenerate INDEX.
  Args: `--project`, `--module`, `--slug`. Called by execute-upsert
  Phase 8 after implementing a planned change.
- `scripts/index-requirements.sh` — scan all four states and regenerate
  `INDEX.md` (pure markdown) and `INDEX.html` (interactive JS view).
  No args (uses `--root` optional).
- `scripts/validate-ledger.sh` — check consistency across all four
  states. Exit 1 on errors. Reports: orphan history, missing creation
  snapshot, date inconsistencies, stale INDEX.md, invalid proposed/todo
  status fields, missing todo ready dates.
- `scripts/lint-ears.py` — PEP 723 linter that checks every requirement's
  Statement and Constraints use a valid EARS pattern with SHALL. Run via
  `uv run --script scripts/lint-ears.py --root <path>`. Wired into
  validate-ledger.sh as Check 5.

## Wiring with Other Skills

- **execute-upsert**: Phase 4 reads `internal-docs/reqs/todo/` and
  `current/` before PRD creation to understand planned and existing
  requirements in scope. Phase 8 processes todo/ files via
  `process-todo.sh`, updates current/, and archives todos to history/
  after feature completion.
- **agent-file-upsert**: reads the ledger when generating AGENTS.md so
  agent documentation reflects the project's durable constraints.
- **handoff**: bundles requirements-upsert write machinery via
  includeTree. Writes to `todo/` (ready plans) and `proposed/` (draft
  plans) during context capture when new durable constraints are
  identified. Also includes applicable requirements in handoff documents
  so the next session knows the constraints.
- **ai-upsert**: bundles requirements-upsert write machinery via
  includeTree. In skills-src, writes to `current/` (the skill IS the
  product, active immediately). In consumer repos, writes to `todo/`
  using `{proj}` = `skills` convention for skill-imposed constraints.
- **All ai/* upsert skills**: consult the ledger during creation/update
  to ensure generated guidance is consistent with project requirements.

## Formal Requirements Languages

The requirements ledger supports three formal/near-formal languages that close
specific gaps in the requirements stack:

- **EARS** (Easy Approach to Requirements Syntax) — constrains authoring syntax.
  Every requirement's Statement and Constraints use one of 5 EARS templates with
  SHALL as the modal verb. This eliminates vague requirements at authoring time.
  See `references/ears-patterns.md` for the 5 templates and examples.

- **FSL** (AI-Native Formal Specification Language) — optional parallel `.fsl`
  spec for the provability-critical subset (idempotency, money-moving, auth,
  concurrency, safety-critical). FSL uses Z3 bounded model checking to verify
  consistency and generate counterexamples. See `references/fsl-integration.md`
  for when to use FSL, the write→verify→repair loop, and installation.

- **AgentContract** — formalizes the runtime binding contract in execute-upsert
  as `.contract.yaml` with must/must-not/may clauses + severities. This is the
  runtime enforcement layer; requirements-upsert produces the constraints that
  AgentContract enforces. See execute-upsert's `references/agent-contract.md`.

The EARS linter (`scripts/lint-ears.py`) validates that every requirement in the
ledger uses a valid EARS pattern. It is wired into `validate-ledger.sh` as
Check 5.

For the full feature matrix comparing these tools against the current stack and
alternatives (SpeQ, Simplex), see `references/formal-languages-overview.md`.

## References

- `references/requirement-template.md` — the file template for a
  current requirement (EARS-constrained, pure description)
- `references/proposed-template.md` — the file template for a proposed
  plan (draft, not ready to implement)
- `references/todo-template.md` — the file template for a todo plan
  (ready to implement, flexible format)
- `references/ears-patterns.md` — the 5 EARS templates with examples and
  implied test shapes
- `references/formal-languages-overview.md` — feature matrix comparing EARS,
  FSL, AgentContract, SpeQ, Simplex against the current stack; workflow
  diagram from requirement → PRD → task → execution → verification
- `references/fsl-integration.md` — when to use FSL, the write→verify→repair
  loop, 3-tier install strategy (Docker verified working), example .fsl spec
- `references/redundancy-audit.md` — audit of overlaps between new formal
  language tools and existing validation/review infrastructure
- `references/evolution-protocol.md` — detailed four-state lifecycle,
  snapshot, process-todo, and supersession protocol
- `references/granularity-guidance.md` — what is a requirement vs a PRD
  bullet vs an ADR; four-state lifecycle; {proj}=skills convention for
  consumer repos
