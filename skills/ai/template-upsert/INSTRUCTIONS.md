...
2. **Customize frontmatter**: Fill in `template`, `slug`, `description`, `use`, `engine`, `outputs_to`, `date` (`created`, `knowledge-basis`, `last-used`), `tags`, `see-also`. The `description` should state what the template structures and when to use it. See `references/template-guidelines.md` for required fields and variable schema format.
