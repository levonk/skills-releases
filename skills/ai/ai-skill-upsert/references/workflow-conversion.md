...
Skills created from workflows must NOT be auto-loaded into the model's context. Set `disable-model-invocation: true` and `user-invocable: true` so the skill is only invoked on explicit user request (e.g., `/skill-name`), never auto-triggered by the model:
...
This excludes the skill's description from the system prompt entirely (zero baseline token cost). The skill only loads when the user types `/skill-name`. This keeps workflow-derived skills opt-in, avoiding context bloat from skills the user hasn't explicitly chosen to activate.
...
4. **Apply skill-based optimizations as separate commits**, one logical change per commit, so each optimization is independently reviewable and revertable:
   - **Frontmatter**: Add/adjust `name`, `description`, `user-invocable: true`, `disable-model-invocation: true`, `date`, `tags`, `see-also`. Strip workflow-only fields (`workflow`, `slug`, `use`, `role`, `inputs`, etc.) that don't apply to skills. Add a "Do NOT trigger on..." clause to the description listing cases where the skill would waste effort. Wire in the trigger-guard include (`---
...
`) right after the `base-ai-guidance` include.
...
` if the skill should inherit the shared framework.
