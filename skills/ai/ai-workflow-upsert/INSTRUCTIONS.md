...
- [ ] Decide mode: check whether the target wrapper exists (Mode A: Create / Mode B: Convert Skill → Workflow / Mode C: Update)
- [ ] Select the workflow location (skills-src repo / current project / user directory)
- [ ] Run the workflow-specific research (local `config/ai/workflows/`, skills.sh / GitHub, cross-check with skills)
- [ ] Mode A Step 1: Initialize — run `scripts/init_workflow.py` to scaffold the wrapper and content template
- [ ] Mode A Step 2: Plan — map steps and dependencies; identify deterministic vs AI-judgment phases
- [ ] Mode A Step 3: Apply — implement the Template/Wrapper pattern; fill in placeholders; set `date.last-used`
- [ ] Mode A Step 4: Verify — validate step sequencing and template syntax (`just validate`)
- [ ] Mode A Step 5: Deliver — save to the chosen location and update `date.last-used`
- [ ] Mode A Step 6: Packaging verification — frontmatter valid, no forbidden files, Template/Wrapper split confirmed
- [ ] Mode B Step 1: Verify conversion is appropriate (no scripts/references/evals/assets needed)
- [ ] Mode B Step 2-3: `git mv` the skill's SKILL.md to the wrapper path (preserve history)
- [ ] Mode B Step 4: Commit the rename as a standalone commit
- [ ] Mode B Step 5: Apply workflow-based optimizations as separate commits
- [ ] Mode C Step 1: Read the existing workflow fully (wrapper, content template, see-also)
- [ ] Mode C Step 2: Audit against the workflow guidelines checklist
- [ ] Mode C Step 3: Propose changes (Critical / Important / Nice to have) — do not apply yet
