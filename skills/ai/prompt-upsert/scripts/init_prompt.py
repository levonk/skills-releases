...
# ///
"""
Initialize a new prompt file with the correct naming convention and a
companion README scaffold.

Creates:
  1. <output-dir>/<project-slug>-prompt-<YYYYMMDDHHMM>-01-01-<prompt-slug>.md
  2. <doc-dir>/<project-slug>-readme-<YYYYMMDDHHMM>-01-01-<prompt-slug>.md

Usage:
    python init_prompt.py <prompt-slug> --path <output-directory> [--project <project-slug>] [--doc-dir <documentation-directory>]

Examples:
    # Create in the standard prompts todo directory
    python init_prompt.py gather-job-history --path ./internal-docs/prompts/todo --project resume

    # Create with a custom documentation directory
    python init_prompt.py analyze-market --path ./internal-docs/prompts/todo --project launch --doc-dir ./internal-docs/prompts/doc
"""
...
def create_prompt_structure(
    ...
    """Create the prompt file and companion README with TODO placeholders."""
    created_date = datetime.now().strftime("%Y-%m-%d")
    timestamp = datetime.now().strftime("%Y%m%d%H%M")
    prompt_title = prompt_slug.replace("-", " ").title()

    # 1. Prompt file: <output>/<project>-prompt-<timestamp>-01-01-<slug>.md
    output_dir = Path(output_path)
    output_dir.mkdir(parents=True, exist_ok=True)
    prompt_filename = f"{project_slug}-prompt-{timestamp}-01-01-{prompt_slug}.md"
    prompt_path = output_dir / prompt_filename

    if prompt_path.exists():
        print(f"Error: Prompt already exists: {prompt_path}")
        sys.exit(1)
...
date:
  ...
  knowledge-basis: "{created_date}"
  last-used: "{created_date}"
tags:
  - "ai/prompt/{project_slug}/{prompt_slug}"
---
...
[TODO: Why this task matters, what it's for, who will use it, and the end goal.]

## Instructions

[TODO: Clear, unambiguous language about what the AI should do and in what order.]
...
[TODO: Use explicit, relative paths when files are involved (usually under
./internal-docs/ or ./src/ as appropriate).]

## Success / Verification

[TODO: Inline success criteria and any verification steps (tests, validation
commands, review prompts).]

<!-- vim: set ft=markdown -->
"""
    prompt_path.write_text(prompt_content)

    # 2. Companion README: <doc-dir>/<project>-readme-<timestamp>-01-01-<slug>.md
    documentation_dir = Path(doc_dir)
    documentation_dir.mkdir(parents=True, exist_ok=True)
    readme_filename = f"{project_slug}-readme-{timestamp}-01-01-{prompt_slug}.md"
    readme_path = documentation_dir / readme_filename

    if readme_path.exists():
        print(f"Warning: README already exists: {readme_path} (leaving as-is)")
    else:
        readme_content = f"""# {prompt_title} — Design Documentation

## Design Decisions

[TODO: Document the key design decisions behind this prompt.]

## References

[TODO: List references, templates used, and related prompts.]

## Future Adjustments

[TODO: Note future adjustments that do not belong in the executable prompt
itself.]
...
"""
    ...
    if readme_path.exists():
        print(f"✓ README exists (left as-is): {readme_path}")
    else:
        print(f"✓ Companion README created: {readme_path}")
    print(f"\nNext steps:")
    print(f"1. Edit the prompt frontmatter (description, tags)")
    print(f"2. Fill in the prompt body (Context, Instructions, File/Output, Success/Verification)")
    print(f"3. Document design decisions in the companion README")
    print(f"4. Update date.last-used on each use (YYYY-MM-DD)")


def main():
    parser = argparse.ArgumentParser(
        description="Initialize a new prompt file with naming convention and companion README"
    )
    parser.add_argument("prompt_slug", help="Slug for the prompt (kebab-case)")
    parser.add_argument(
        "--path",
        default="./internal-docs/prompts/todo",
        help="Output directory for the prompt file",
    )
    parser.add_argument(
        "--project",
        default="project",
        help="Project slug used in the filename prefix",
    )
    parser.add_argument(
        "--doc-dir",
        default="./internal-docs/prompts/doc",
        help="Output directory for the companion README",
    )

    args = parser.parse_args()

    if not args.prompt_slug.replace("-", "").isalnum():
        print("Error: Prompt slug should use kebab-case (letters, numbers, hyphens only)")
        sys.exit(1)

    if not args.project.replace("-", "").isalnum():
        print("Error: Project slug should use kebab-case (letters, numbers, hyphens only)")
        sys.exit(1)

    create_prompt_structure(args.prompt_slug, args.path, args.project, args.doc_dir)
