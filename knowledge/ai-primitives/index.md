...
* [LLM Model Assessment](comparison/model-assessment.md) - Twenty-two-criterion framework for assessing an LLM before adoption (benchmark methodology, open weights vs open model, active vs total MoE parameters, on-disk size, license and weights availability, per-million-token pricing, independent evals, languages, tools and runtime integration, modalities, knowledge cutoff, Hugging Face stats, context window, data privacy, throughput and latency, rate limits, compliance, fine-tuning, structured output, safety and refusal, vendor viability and deprecation, release cadence)
...
* [Agent Plugins Specification](cross-domain/agent-plugins-spec.md) - Cross-link to the agentplugins/agent-plugins-spec v1.0.0; an open, vendor-neutral packaging standard for Agent Skills and MCP servers. Maps the plugin package model, manifest, and component discovery onto the skills-src primitive system and identifies the gaps (no agent component type, divergent SKILL.md frontmatter, different distribution mechanism)
