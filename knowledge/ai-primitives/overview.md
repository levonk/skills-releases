...
- [Agent Plugins Specification](cross-domain/agent-plugins-spec.md) —
  an open, vendor-neutral packaging standard (v1.0.0) for Agent Skills and
  MCP servers. Defines a portable `plugin.json` manifest, fixed component
  locations, and containment rules. Maps onto the skills-src primitive
  system with identified gaps: no agent component type (agents are
  explicitly excluded as too client-specific), divergent SKILL.md
  frontmatter (skills-src is a rich superset), and a different distribution
  mechanism (vercel-labs/skills CLI vs plugin directories).
